...
Second review follow-up: 113 focused tests cover omitted entries in partial catalogs, known invalid selections becoming visible failed work without a launch command, and redirects preserving explicit choices while resolving destination presets/defaults for implicit choices. Browser environment-switch checks now include uninstalled, unavailable, errored, disabled and unauthenticated providers ahead of the usable provider. Existing work created before selection provenance was stored retains its saved selection on redirect because its original intent cannot be recovered reliably.

...
Run `vp run icons:export` from the repository root to regenerate the tracked Linux, Windows, and web assets. The development favicon exports are copied to `apps/web/public`; its Apple touch icon and splash screen use the production Pathway icon. Run `vp run icons:check` to verify that the generated assets and public copies match their sources without changing files.

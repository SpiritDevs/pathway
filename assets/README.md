...
The development and preview Icon Composer projects and the production Pathway PNG are the source
of truth for full application icons:
...
Run `vp run icons:export` from the repository root to regenerate the tracked iOS, Linux, Windows, and web assets. The development favicon exports are copied to `apps/web/public`; its Apple touch icon and splash screen use the production Pathway icon. Run `vp run icons:check` to verify that the generated assets and public copies match their sources without changing files.
...
```text
Use [@Computer](plugin://computer-use@openai-bundled) and the Icon Composer app to export the two Icon Composer-based macOS app icons in this repository.

---
name: test-pathway-mobile
description: Launch and test Pathway Mobile on an iOS Simulator or Android Emulator against disposable local Pathway environments, including Metro and dev-client reuse, native rebuild decisions, per-client pairing, seeded projects, semantic UI control, screenshots, and iOS serve-sim streaming. Use after mobile UI or native changes, when reproducing phone or tablet behavior, pairing an emulator to isolated state, or verifying mobile behavior on macOS, Linux, or Windows.
---

# Test Pathway Mobile

Run one focused, end-to-end mobile verification pass against disposable Pathway state. Use the sibling [`test-pathway-app`](../test-pathway-app/SKILL.md) skill as the detailed reference for pairing-token semantics and SQLite fixtures.
...
- App: `Pathway Dev`
- Bundle/package identifier: `com.spiritdevs.pathway.dev`
- URL scheme: `pathway-dev`
...
## Start one disposable Pathway environment

Run backend commands from the repository root. Use the ignored, worktree-local `.pathway` directory or create a fresh directory with the host OS's temporary-directory mechanism. An explicit base directory stores state in `<base-dir>/userdata`; never point testing at shared `~/.pathway` state.
...
Use direct SQLite mutation only for disposable projection fixtures. Follow `test-pathway-app` and stop the backend before writing.
...
1. Inspect any process on the intended Metro port and its `/status` response. Reuse it only when it is healthy, belongs to this worktree, and matches `APP_VARIANT=development`, `--dev-client`, and scheme `pathway-dev`.
...
- Workspace: `<repo>/apps/mobile/ios/PathwayDev.xcworkspace`
- Scheme: `PathwayDev`
- Configuration: `Debug`
- Simulator ID: the selected UDID
- Bundle ID: `com.spiritdevs.pathway.dev`

Check the installed client with:

```bash
xcrun simctl get_app_container <simulator-udid> com.spiritdevs.pathway.dev app
...
```bash
adb -s <emulator-serial> shell pm path com.spiritdevs.pathway.dev
...
```bash
PATHWAY_PORT=<server-port> node apps/server/src/bin.ts auth pairing create \
  --base-dir <base-dir> \
...
In PowerShell, set `$env:PATHWAY_PORT = "<server-port>"` first and run the `node ... auth pairing create` command without the leading assignment.
...
```bash
xcrun simctl openurl <simulator-udid> 'pathway-dev://connections/new'
adb -s <emulator-serial> shell am start -W \
  -a android.intent.action.VIEW \
  -d 'pathway-dev://connections/new' \
  com.spiritdevs.pathway.dev
```

Run only the command for the selected platform.

In Pathway Dev, open Add Environment and enter the complete `<mobile-origin>` and newly printed `Token`. Verify the expected seeded projects appear before exercising the affected flow.
...
Use `snapshot_ui` and current element references from XcodeBuildMCP for taps and typing. Stream the same UDID through `ios-simulator-browser` so the user can watch in Pathway when the host supports it. Use the stream as a visual feed rather than a reason to switch to fragile browser coordinates.
...
3. Remove the disposable environment from Pathway Dev.

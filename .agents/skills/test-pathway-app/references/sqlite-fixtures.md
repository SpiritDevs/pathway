...
Load this reference only when inspecting or seeding local Pathway state directly.
...
```bash
node apps/server/scripts/pathway-sqlite-state.ts query \
  --base-dir <base-dir> \
...
```bash
node apps/server/scripts/pathway-sqlite-state.ts exec \
  --base-dir <base-dir> \
  --file /tmp/pathway-seed.sql
```
...
Adapt any copied fixture column set to the current migrations instead of assuming old SQL remains
current.

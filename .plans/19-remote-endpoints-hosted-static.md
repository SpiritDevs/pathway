...
- users can expose a backend through LAN, their own Tailscale, MagicDNS, a manual HTTPS endpoint, or later Pathway Connect
- users can generate a hosted pairing link for `app.spiritdevs.com`
...
This plan intentionally leaves the paid Pathway cloud tunnel fabric out of scope. It defines the OSS foundation that Pathway Connect should later plug into.
...
- `https://app.spiritdevs.com` cannot reliably call `http://...` or `ws://...` endpoints because browsers block mixed content.
...
- Generate `app.spiritdevs.com` links only from endpoints that are likely hosted-browser compatible, or show a warning with an explicit fallback.
...
```text
https://app.spiritdevs.com/pair?host=<encoded endpoint httpBaseUrl>#token=<one-time token>
```
...
- HTTP private-network endpoints are marked incompatible with `app.spiritdevs.com`.
...
- `app.spiritdevs.com` works as a real client shell.
...
- `app.spiritdevs.com` can pair a reachable HTTPS backend and reconnect after reload.
- A saved environment can be used without any backend at `app.spiritdevs.com`.
...
## Phase 4: Future Pathway Connect Provider
...
- create endpoint with `provider: "pathway-tunnel"`
- surface tunnel status
- provide stable HTTPS URL
- use existing backend pairing/session auth
- never bypass server auth

The tunnel fabric can later be Pipenet-derived, Tailscale-derived, or another reverse tunnel implementation. The rest of Pathway should only see an `AdvertisedEndpoint`.

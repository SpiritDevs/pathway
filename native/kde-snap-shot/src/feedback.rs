...
const OBJECT: &str = "/com/spiritdevs/KdeCapture/Feedback";
...
#[zbus::interface(name = "com.spiritdevs.KdeCapture.Feedback")]
impl Bridge {
...
pub(super) fn run(connection: &Connection, directory: &Path, options: &str) -> Result<()> {
    ...
    let name = format!("pathway-capture-feedback-{}", std::process::id());
...
mod tests {
    ...
    fn only_kwin_can_read_commands_or_acknowledge_feedback() {
        ...
        let unrelated = Proxy::new(
            ...
            destination.as_str(),
            OBJECT,
            "com.spiritdevs.KdeCapture.Feedback",
        )
        .unwrap();

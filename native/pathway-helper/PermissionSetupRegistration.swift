...
func verifyPermissionSetupRegistration(
    ...
    guard let bundleIdentifier, !bundleIdentifier.isEmpty else {
        throw PathwayHelperFailure(code: "permission_setup_bundle_unavailable", message: permissionSetupRegistrationMessage)
    }
    ...
    guard let resolvedURL = resolveApplication(bundleIdentifier) else {
        throw PathwayHelperFailure(code: "permission_setup_registration_unresolved", message: permissionSetupRegistrationMessage)
    }
    guard matches(resolvedURL) else {
        throw PathwayHelperFailure(code: "permission_setup_identity_mismatch", message: permissionSetupIdentityMismatchMessage)
    }
}

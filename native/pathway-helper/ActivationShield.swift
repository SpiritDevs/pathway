...
/// Masked-activation shield: a Pathway-owned overlay that veils the target
...
final class ShieldContentView: NSView {
    init(frame frameRect: NSRect, label: String?) {
        ...
        let text = label.flatMap { $0.isEmpty ? nil : $0 } ?? "Pathway is activating this window"
...
final class ShieldCommandListener {
    ...
    private func invalid(_ message: String, id: String? = nil) {
        emitter.emitError(
            PathwayHelperFailure(code: "invalid_request", message: message),
            capturedAt: pathwayHelperTimestamp(),
            id: id
        )
    }

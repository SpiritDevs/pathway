...
/// ScreenCaptureKit owner for Computer Use's live preview. Frames remain
/// native; this class never encodes or stores images.
final class WindowFrameStream: NSObject, SCStreamOutput, SCStreamDelegate {
    ...
    private let onFailure: (PathwayHelperFailure) -> Void
    private let queue = DispatchQueue(label: "com.spiritdevs.pathway.window-frame-stream")
    ...
    init(
        ...
        onFailure: @escaping (PathwayHelperFailure) -> Void
    ) {
    ...
    func start() {
        queue.async { [self] in
            ...
            guard CGPreflightScreenCaptureAccess() else {
                fail("screen_recording_required", "Screen Recording is required for the preview. Enable it in Pathway’s permission setup.")
                return
            }
    ...
    private func fail(_ code: String, _ message: String) {
        guard !stopped else { return }
        stopOnQueue()
        onFailure(PathwayHelperFailure(code: code, message: message))
    }

...
final class ComputerFrameTap {
    ...
    private let queue = DispatchQueue(label: "com.spiritdevs.pathway.computer-frame-tap")
    ...
    private func emitConnectFailure(code: String, message: String) {
        emitter.emitError(
            PathwayHelperFailure(code: code, message: message),
    ...
    private func fail(_ failure: PathwayHelperFailure) {
        queue.async { [self] in
            stateLock.lock()
            guard !stopped else {
                stateLock.unlock()

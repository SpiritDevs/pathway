...
Insertion resolves a verified editable, non-password field and rechecks focus, selection, and modifiers before dispatch. macOS sends one Command-V paste through the editor's paste handler, preserving the previous clipboard. It does not use AX selected-text replacement because Chromium/Electron editors can advertise writable text without updating their content. Windows validates a UI Automation text selection, then sends one Control-V paste. Neither platform synthesizes Enter or changes application focus. Unsupported editors and elevated Windows targets may require manual copying. An OS write timeout or unconfirmed paste never triggers another write.

# Pathway native Cua revision

`0001-pathway-native.patch` applies to the exact upstream commit in
...
aborting connection tasks. Pathway refuses to kill or replace an active generation
...
`PATHWAY_CUA_BACKGROUND_OBSERVATION_MS` — the same env-var mechanism foreground
...
exact PID and window. Pathway already orders same-window requests in the server;
...
Pathway's admission-gate wrapping, single-transport event posting, exact-target
...
Pathway prefers this observed settle after a mutation whose window is known
...
`HIToolbox/Events.h` codes. Pathway still refuses `insert`/`ins` (macOS has
no Insert key) before dispatch; unmapped spellings keep the honest
`unknown_key_name` refusal.

The gate applies to the SDK tool path admitted by Pathway's GUI host. It does not
...
Build using `scripts/provision-cua-driver.ts --source-checkout
...
use the same directory through `PATHWAY_CUA_ARTIFACT_DIR`. Signing changes the
...
When bumping the revision: the daemon stamps `pathway_native_revision` from a
...
`0001-pathway-native.patch` is self-contained: it carries the `select_text`
...
`scripts/provision-cua-driver.ts` now records
...
- Live-verified (`.unlazy/pathway-cu-codex-parity/L13-notebook.md`,
...
- Live-verified (`.unlazy/pathway-cu-codex-parity/L21-notebook.md`,
...
- Live-verified (`.unlazy/pathway-cu-codex-parity/L22-notebook.md`,
...
- `get_agent_cursor_state` adds `overlay_ready`, `render_visible` and
  ...
  Pathway now keeps the cursor parked through model turns, hides it at turn end,
...
The matching host changes reuse pathway-helper's permission checks and grant events.
...
observed `AXFocusedWindow`, independently of Pathway's selected target. This is
...
## Pathway SDK build target

The native patch builds `cua-driver-sdk` as an `rlib` only. The driver still uses
the SDK internally; Pathway does not ship or load its separate `cdylib` on macOS

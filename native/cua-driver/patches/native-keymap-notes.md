...
Companion to the Pathway-side `cuaKey` expansion. **Applied 2026-09-23**: the
...
`pathway_native_revision` 19 by hand (the rev-18 stream had already landed the
17→18 bump, exactly the concurrency case predicted below). The folded patch
`0001-pathway-native.patch` regenerates at sha
...
1. `platform-macos/src/input/keyboard.rs` — `key_name_to_code`:
   - underscore spellings on existing arms: `caps_lock`, `page_up`/`pgup`/`prior`,
     `page_down`/`pgdn`/`next` (Pathway already maps these itself, but direct
...
3. `cua-driver/src/serve.rs` — `pathway_native_revision` 17 → 18.
...
- `serve.rs` bump is **required**, not optional: the desktop host handshake
  ...
  `pathway_native_revision` differs from the pin.
- `packages/shared/src/cuaDriverRelease.json` must then carry
  ...
  exist yet.
- Pathway-side needs nothing for the new names: `cuaKey` passes them through
  untouched and the driver refusal was the only gate.

## Deliberately not added

- `insert` — macOS has no Insert key. `help` (114) is the honest physical key;
  Pathway keeps refusing `insert`/`ins` with `unsupported_operation`.
...
- Right-side modifiers (`shift_r`, `ctrl_r`, `alt_r`, `cmd_r`, `super_r`,
  ...
  `modifier_flags` updates plus Pathway's chord set; a left-spelling→generic

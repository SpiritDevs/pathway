...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("switches desktop packaging product names to nightly for nightly builds", () => {
    assert.equal(resolveDesktopProductName("0.0.17"), "Pathway (Alpha)");
    assert.equal(resolveDesktopProductName("0.0.17-nightly.20260413.42"), "Pathway (Nightly)");
  });
        ...
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                PATHWAY_DESKTOP_UPDATE_REPOSITORY: "coreybain/pathway",
              },
            }),
          ),
        ),
      ...
      const nightlyConfig = yield* resolveGitHubPublishConfig("nightly").pipe(
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                GITHUB_REPOSITORY: "coreybain/pathway",
              },
            }),
          ),
        ),
  ...
  it("derives macOS passkey signing configuration from the Clerk publishable key", () => {
    const configuration = resolveMacPasskeySigningConfiguration({
      ...
      PATHWAY_CLERK_PUBLISHABLE_KEY: `pk_test_${btoa("example.clerk.accounts.dev$")}`,
    });

    assert.deepStrictEqual(configuration, {
      appId: "com.t3tools.pathway",
      teamId: "ABC1234567",
  ...
  it("normalizes explicit macOS passkey RP domains and renders required entitlements", () => {
    const configuration = resolveMacPasskeySigningConfiguration({
      PATHWAY_APPLE_TEAM_ID: "ABC1234567",
      PATHWAY_MACOS_PROVISIONING_PROFILE: "/tmp/pathway.provisionprofile",
      PATHWAY_CLERK_PASSKEY_RP_DOMAINS:
    ...
    assert.include(entitlements, "<string>ABC1234567.com.t3tools.pathway</string>");
  ...
  it("rejects incomplete macOS passkey signing configuration", () => {
    ...
    assert.equal(
      ...
      "PATHWAY_MACOS_PROVISIONING_PROFILE must point to an Associated Domains provisioning profile.",
    );
    ...
    const invalidDomainError = captureError({
      PATHWAY_APPLE_TEAM_ID: "ABC1234567",
      PATHWAY_MACOS_PROVISIONING_PROFILE: "/tmp/pathway.provisionprofile",
      PATHWAY_CLERK_PASSKEY_RP_DOMAINS: unsafeDomain,
    });
    ...
    assert.throws(
      () =>
        resolveMacPasskeySigningConfiguration({
          PATHWAY_APPLE_TEAM_ID: "ABC1234567",
          PATHWAY_MACOS_PROVISIONING_PROFILE: "/tmp/pathway.provisionprofile",
          PATHWAY_CLERK_PASSKEY_RP_DOMAINS: "example.clerk.accounts.dev:8443",
        }),
    ...
    assert.equal(invalidPublishableKeyError.message, "PATHWAY_CLERK_PUBLISHABLE_KEY is invalid.");
  ...
  it.effect("adds passkey entitlements and both renderer protocols to signed macOS builds", () =>
    Effect.gen(function* () {
      ...
      assert.equal(config.appId, "com.t3tools.pathway");
          ...
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                ...
                PATHWAY_DESKTOP_MOCK_UPDATES: "true",
              },
            }),
          ),
        ),

...
export class DevRunnerInvalidPortOffsetError extends Schema.TaggedErrorClass<DevRunnerInvalidPortOffsetError>()(
  ...
  {
    configKey: Schema.Literal("PATHWAY_PORT_OFFSET"),
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("PATHWAY_PORT_OFFSET"),
  devInstance: optionalStringConfig("PATHWAY_DEV_INSTANCE"),
});
...
> {
  if (config.portOffset !== undefined) {
    ...
    return Effect.succeed({
      ...
      source: `PATHWAY_PORT_OFFSET=${config.portOffset}`,
    });
  }
  ...
  if (seed) {
    if (/^\d+$/.test(seed)) {
      return Effect.succeed({
        offset: Number(seed),
        source: `numeric PATHWAY_DEV_INSTANCE=${seed}`,
      });
    }

    const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
    return Effect.succeed({ offset, source: `hashed PATHWAY_DEV_INSTANCE=${seed}` });
  }
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    if (configuredBaseDir !== undefined) {
      output.PATHWAY_HOME = resolvedBaseDir;
    } else {
      delete output.PATHWAY_HOME;
    }
    ...
    if (!isDesktopMode) {
      ...
      if (mode === "dev" || mode === "dev:web") {
        ...
        output.PATHWAY_SINGLE_ORIGIN_DEV = "1";
      } else {
        output.VITE_HTTP_URL = `http://localhost:${serverPort}`;
        output.VITE_WS_URL = `ws://localhost:${serverPort}`;
        delete output.PATHWAY_SINGLE_ORIGIN_DEV;
      }
    } else {
      output.PATHWAY_PORT = String(serverPort);
      ...
      delete output.PATHWAY_SINGLE_ORIGIN_DEV;
      delete output.PATHWAY_MODE;
      delete output.PATHWAY_NO_BROWSER;
      delete output.PATHWAY_HOST;
    }

    if (!isDesktopMode && host !== undefined) {
      output.PATHWAY_HOST = host;
    }

    if (!isDesktopMode) {
      output.PATHWAY_NO_BROWSER = browser === true ? "0" : "1";
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.PATHWAY_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.PATHWAY_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.PATHWAY_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.PATHWAY_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.PATHWAY_MODE = "web";
      delete output.PATHWAY_DESKTOP_WS_URL;
    }
 ...
 * target reach it there. When `--host`/`PATHWAY_HOST` moves the backend onto
      ...
      Effect.mapError(
        (cause) =>
          new DevRunnerConfigurationError({
            configKeys: ["PATHWAY_PORT_OFFSET", "PATHWAY_DEV_INSTANCE"],
            cause,
          }),
      ),
    );
    ...
    if (input.share) {
      if (input.mode === "dev:server") {
        ...
        if (shared) {
          ...
          env.PATHWAY_DEV_ALLOWED_ORIGINS = [
            env.PATHWAY_DEV_ALLOWED_ORIGINS,
          ...
          // a few chunk requests. Only defaulted, so PATHWAY_BUNDLED_DEV=0
          // still opts a --share run back out.
          if (env.PATHWAY_BUNDLED_DEV === undefined) {
            env.PATHWAY_BUNDLED_DEV = "1";
          }
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to PATHWAY_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("PATHWAY_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to PATHWAY_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("PATHWAY_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to PATHWAY_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("PATHWAY_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to PATHWAY_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("PATHWAY_PORT")),
  ),

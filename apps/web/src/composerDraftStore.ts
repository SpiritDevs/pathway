...
export const COMPOSER_DRAFT_STORAGE_KEY = "pathway:composer-drafts:v1";
...
export const PersistedComposerImageAttachment = Schema.Struct({
  // Older image-only drafts have no discriminator and hydrate as images.
...
export interface ComposerThreadDraftState {
  ...
  /** Pathway issues attached as compact chips and materialized only when the turn is sent. */
...
export function composerAttachmentDedupKey(
  image: Pick<ComposerAttachment, "type" | "id" | "mimeType" | "sizeBytes" | "name">,
): string {
  if (image.type === "file") {
    ...
    const issueContexts = Array.isArray(draftCandidate.issueContexts)
      ? normalizeIssueContexts(
          draftCandidate.issueContexts.flatMap((entry) => {
            const normalized = normalizeIssueContext(entry);
            return normalized ? [normalized] : [];
          }),
        )
      : [];
...
function hydratePersistedComposerImageAttachment(
  ...
  if (attachment.dataUrl === undefined) return null;
...
export function hydrateImagesFromPersisted(
  ...
  return attachments.flatMap((attachment) => {
    ...
    if (!file && attachment.type !== "file") return [];
    ...
    return [
      attachment.type === "file"
        ? ({
            ...
            previewUrl: "",
            file,
            ...(attachment.attachmentId !== undefined && attachment.environmentId !== undefined
              ? {
                  uploadedAttachmentId: attachment.attachmentId,
                  uploadEnvironmentId: attachment.environmentId as EnvironmentId,
                }
              : {}),
...
function toHydratedThreadDraft(
  ...
  return {
    ...
    issueContexts: normalizeIssueContexts(persistedDraft.issueContexts ?? []),
    previewAnnotations:
      ...
      return {
        ...
        setFileUpload: (threadRef, fileId, environmentId, attachmentId) => {
          ...
          set((state) => {
            ...
            let changed = false;
            const images = current.images.map((attachment) => {
              if (attachment.type !== "file" || attachment.id !== fileId) return attachment;
              if (
        ...
        setIssueContexts: (threadRef, contexts) => {
          const threadKey = resolveComposerDraftKey(get(), threadRef);
          if (!threadKey) return;
          const issueContexts = normalizeIssueContexts(contexts);
          set((state) => {
            const existing = state.draftsByThreadKey[threadKey] ?? createEmptyThreadDraft();
            const nextDraft: ComposerThreadDraftState = { ...existing, issueContexts };
            const nextDraftsByThreadKey = { ...state.draftsByThreadKey };
            if (shouldRemoveDraft(nextDraft)) {
              delete nextDraftsByThreadKey[threadKey];
            } else {
        ...
        removeIssueContext: (threadRef, contextId) => {
          ...
          set((state) => {
            const current = state.draftsByThreadKey[threadKey];
            if (!current) return state;
            const issueContexts = current.issueContexts.filter((entry) => entry.id !== contextId);
            if (issueContexts.length === current.issueContexts.length) return state;
            const nextDraft: ComposerThreadDraftState = { ...current, issueContexts };
            const nextDraftsByThreadKey = { ...state.draftsByThreadKey };
            if (shouldRemoveDraft(nextDraft)) {
              delete nextDraftsByThreadKey[threadKey];
            } else {

...
export type OnboardingWorkspaceKind = "personal" | "organization";
...
/** Maps onboarding vocabulary onto the durable workspace model understood by Convex. */
...
/**
 * Provisions the user's first workspace over one HTTP request.
 *
 * This deliberately uses `ConvexHttpClient`: onboarding needs one mutation, not another
 * always-on WebSocket beside the cloud-sync runtime. The module is dynamically imported by the
 * stepper, so Convex is not added to the initial onboarding bundle either.
 */
...
/** Keeps the onboarding gate closed until both durable stores agree that setup is complete. */

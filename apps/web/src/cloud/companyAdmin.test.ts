...
describe("company admin function references", () => {
  ...
  it("names every public backend function exactly", () => {
    expect(
      ...
      createOrganizationWorkspace: "companies:createOrganizationWorkspace",
  ...
  it("creates an organization alongside the personal workspace", async () => {
    const fake = fakeClient();
    const admin = makeCompanyAdminClient({
      convexUrl: "https://example.convex.cloud",
    ...
    await admin.createOrganizationWorkspace({ name: "Spirit Devs" });

    expect(fake.calls).toEqual([
      {
        kind: "mutation",
        name: "companies:createOrganizationWorkspace",
        args: { name: "Spirit Devs" },
      },
    ]);
  });

  it("creates an additional company with a client-owned id", async () => {
    const fake = fakeClient();
    const admin = makeCompanyAdminClient({
      convexUrl: "https://example.convex.cloud",
      fetchToken: async () => "token",
    ...
    await admin.createCompany({ id: COMPANY_ID, name: "Spirit Devs" });

    expect(fake.calls).toEqual([
      {
        kind: "mutation",
        name: "companies:create",
        args: { id: COMPANY_ID, name: "Spirit Devs" },
      },
    ]);
  });
});

...
import { ALL_FOCUS_ID, type ActiveFocusId } from "@spiritdevs/client-runtime/state/focuses";
import type {
  AttentionEventKind,
  Focus,
  FocusAssignment,
...
export function buildFocusNotificationRows(input: {
  ...
  readonly focuses: ReadonlyArray<Pick<Focus, "id" | "name">>;
...
}): ReadonlyArray<FocusNotificationRowModel> {
  const focusById = new Map(input.focuses.map((focus) => [focus.id, focus] as const));

...
export function SnapShotSettings() {
  ...
  const updateSettings = persistClientSettingsPatch;
  ...
  const captureAvailable =
    Boolean(bridge) && accountId !== null && state !== null && state.mode !== "unavailable";
  ...
  const setup = useCallback(
    async (action: DesktopSnapShotSetupAction) => {
      if (!accountId || !bridge?.setupSnapShot || setupBusy) return;
      setSetupBusy(true);
      setSetupError(null);
      try {
        if (
    ...
    [accountId, bridge, refreshState, setupBusy, state, wizard],
  );
  ...
  const openSetup = async (requested: CaptureSetupStep | "resume" = "resume") => {
    if (!accountId || !state || setupBusy) return;
    stopRecording();
    shortcutCheckIdRef.current++;
    setCandidate(savedShortcut);
    setShortcutCheck({ status: "idle", availability: null });
  ...
  const enableForSetup = async () => {
    ...
    try {
      if (state?.macPermissions) {
        ...
        if (!bridge?.setupSnapShot) throw new Error("Restart Pathway to finish capture setup.");
  ...
  return (
    <SettingsPageContainer>
      ...
      <SnapShotPendingCaptures key={accountId} />
      {accountId && wizard && state ? (
        <SnapShotSetupDialog
          state={state}

...
export function EnvironmentConnectionSettings({
  excludeEnvironmentIds,
  renderEnvironmentSection,
}: {
  readonly excludeEnvironmentIds?: ReadonlySet<EnvironmentId>;
  ...
  const visibleSavedEnvironments = useMemo(
    () =>
      excludeEnvironmentIds === undefined
        ? savedEnvironments
        : savedEnvironments.filter(
            (environment) => !excludeEnvironmentIds.has(environment.environmentId),
          ),
    ...
    return (
      <>
        ...
        {desktopWslState.enabled ? (
          <SettingsRow
            title="WSL only"
            description="Stop the Windows backend and run only the WSL backend. Useful if you develop entirely inside WSL and don't want a second backend process. Pathway restarts when you change this."
            ...
            <AlertDialogPopup>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  {pendingWslChange?.kind === "disable"
                    ? pendingWslChange.wasWslOnly
                      ? "Pathway will restart on the Windows backend. Threads and projects opened against WSL stay safe inside the distro and become available again when you re-enable WSL."
                      : "The WSL backend will stop. Threads and projects opened against WSL stay safe inside the distro, but they'll be unavailable in Pathway until you re-enable WSL."
                    : pendingWslChange?.kind === "distro"
                      ? "Pathway will restart the WSL backend on the new distro. Sessions still running on the current distro will be interrupted."
                      : pendingWslChange?.kind === "enable"
                        ? "Run the WSL backend alongside the Windows one, or stop the Windows backend and use only WSL? You can change this later from Settings."
                        : pendingWslChange?.nextValue
                          ? "Pathway will restart and start only the WSL backend. Your Windows-side projects won't be accessible until you turn this off again."
                          : "Pathway will restart and bring the Windows backend back up alongside WSL."}
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose

...
const account = vi.hoisted(() => ({ id: "test-user" as string | null }));
vi.mock("../../lib/snapShotAccount", () => ({ useSnapShotAccountId: () => account.id }));
...
vi.mock("../../hooks/useSettings", () => ({
  ...
  persistClientSettingsPatch: (patch: ClientSettingsPatch) => settingsStore.update(patch),
}));
...
it("requires a successful macOS test capture before enabling and allows retry", async () => {
  ...
  bridge.setupSnapShot.mockRejectedValueOnce(
    new Error("Error invoking remote method 'pathway:snap-shot:setup': Error: Capture was denied"),
  );
...
it("keeps native capture setup disabled until the authenticated account is synchronized", async () => {
  account.id = null;
  const tree = await mount();
  const toggle = visitElements(
    tree,
  ...
  expect(wizard(tree)).toBeNull();
  expect(bridge.requestSnapShotPermissions).not.toHaveBeenCalled();
  expect(bridge.setupSnapShot).not.toHaveBeenCalled();
});

...
export function ThemeSearchSection({
  ...
  const requestSearchKeyRef = useRef<string | null>(null);
  ...
  useEffect(() => {
    ...
    requestSearchKeyRef.current = null;
    if (open) {
  ...
  const runSearch = useCallback(
    async (searchText: string, nextSort = sortBy) => {
      ...
      if (!trimmed) return;
      const searchKey = `${trimmed}\u0000${nextSort}`;
      requestRef.current?.abort();
      const controller = new AbortController();
      requestRef.current = controller;
      requestSearchKeyRef.current = searchKey;
      setError(null);
      setIsSearching(true);
      try {
  ...
  useEffect(() => {
    ...
    if (requestSearchKeyRef.current === searchKey) {
      // Enter can launch the current query before the debounce catches up.
      // Keep that request instead of aborting it and issuing the same search.
      return;
    }

...
import type { RelayClientEnvironmentRecord } from "@spiritdevs/contracts/relay";
...
/**
 * The user's Pathway Connect environments from relay discovery, each with a
  ...
  const connectEnvironment = async (environment: RelayClientEnvironmentRecord) => {
    ...
    if (result._tag === "Success") {
      toastManager.add({
        ...
        description: `Connecting to ${environment.label} through Pathway Connect.`,
      });
      return;
    }
    ...
    const message =
      cause instanceof Error ? cause.message : "Could not connect the Pathway Connect environment.";
    const traceId = findErrorTraceId(cause);
    console.error("[pathway-connect] Could not connect environment", { message, traceId, cause });
    toastManager.add({
      type: "error",

...
describe("window capture delivery", () => {
  it.each([
    ...
    async ({ target, accessibleText }) => {
      ...
      const delivery = deliverSnapShot(bridge, item, target, () => true);
  ...
  it.each([false, true])(
    ...
    async (staged) => {
      ...
      try {
        ...
        await expect(deliverSnapShot(bridge, capture, target, () => true)).rejects.toThrow(
          "could not be saved",
        );
        expect(acknowledgeSnapShot).not.toHaveBeenCalled();
        expect(
        ...
        await deliverSnapShot(bridge, capture, target, () => true);
...
describe("snapshot metadata during draft recovery", () => {
  it("keeps source metadata in a recovery snapshot before the persistence effect runs", () => {
    const target = scopeThreadRef(environmentId, ThreadId.make("recovery-thread"));
    const source = {
      kind: "snap-shot" as const,
      capturedAt: "2026-09-01T00:00:00.000Z",
      appName: "Editor",
    ...
    useComposerDraftStore.getState().addImage(target, {
      ...
      file: new File(["123"], "window.png", { type: "image/png" }),
      source,
    });
    expect(captureComposerDraft(target).attachments[0]?.source).toEqual(source);
  });

  it("retains the native capture if its draft is removed while the screenshot is loading", async () => {
    const target = DraftId.make("removed-draft");
    const store = useComposerDraftStore.getState();
    store.setProjectDraftThreadId(projectRef, target, {
      threadId: ThreadId.make("removed-thread"),
    });
    const source = {
      kind: "snap-shot" as const,
      capturedAt: "2026-09-01T00:00:00.000Z",
    ...
    await expect(deliverSnapShot(bridge, capture, target, () => true)).rejects.toThrow(
      "destination conversation closed",
    );
    expect(acknowledgeSnapShot).not.toHaveBeenCalled();
    expect(store.getComposerDraft(target)?.images ?? []).toHaveLength(0);
  });
});

it("lets Retry attach to a new conversation after the pinned draft is removed", async () => {
  const removed = DraftId.make("removed-destination");
  const replacement = DraftId.make("replacement-destination");
  const targets = new Map<string, Promise<typeof removed | null>>([
    ["capture", Promise.resolve(removed)],
  ]);
  const resolveTarget = vi.fn(async () => replacement);
  expect(await resolvePendingSnapShotTarget(targets, "capture", resolveTarget, null)).toBeNull();
  expect(resolveTarget).not.toHaveBeenCalled();
  useComposerDraftStore.getState().setProjectDraftThreadId(projectRef, replacement, {
    threadId: ThreadId.make("replacement-thread"),
  });
  expect(await resolvePendingSnapShotTarget(targets, "capture", resolveTarget, null)).toBe(
    replacement,
  );
  expect(resolveTarget).toHaveBeenCalledOnce();
});

it("does not finish a pending delivery after the authenticated account changes", async () => {
  const target = scopeThreadRef(environmentId, ThreadId.make("account-switch-thread"));
  const capture = {
    id: "12345678-1234-1234-1234-123456789abc",
    name: "window.png",
    mimeType: "image/png" as const,
    ...
    source: {
      ...
      capturedAt: "2026-09-01T00:00:00.000Z",
      appName: "Private app",
      windowTitle: "Account A",
    },
  };
  let finishRead!: (value: typeof capture) => void;
  const read = new Promise<typeof capture>((resolve) => {
    finishRead = resolve;
  });
  const acknowledgeSnapShot = vi.fn();
  const bridge = {
  ...
  await first.ready;
  const delivery = deliverSnapShot(bridge, capture, target, first.isCurrent);
  const second = bindSnapShotAccount(bridge, "account-b");
  await second.ready;
  finishRead(capture);
  await expect(delivery).rejects.toThrow("account changed");

...
describe("Pierre file icons", () => {
  ...
  it("extends Pierre with Pathway-specific exact filename icons", () => {
    assert.equal(
      resolvePierreIconForEntry("package.json", "file")?.name,
      "pathway-file-icon-package-json",
    );
    assert.equal(
      resolvePierreIconForEntry("config/tsconfig.json", "file")?.name,
      "pathway-file-icon-tsconfig",
    );
    assert.equal(resolvePierreIconForEntry("AGENTS.md", "file")?.name, "pathway-file-icon-agents");
    assert.equal(resolvePierreIconForEntry("CLAUDE.md", "file")?.name, "pathway-file-icon-claude");
    assert.equal(resolvePierreIconForEntry("README.md", "file")?.name, "pathway-file-icon-readme");
    assert.equal(
      resolvePierreIconForEntry("pnpm-lock.yaml", "file")?.name,
      "pathway-file-icon-pnpm",
    );
    assert.equal(
  ...
  it("ships every custom icon referenced by the extended resolver", () => {
    const customIconNames = new Set(Object.values(Pathway_PIERRE_ICONS.byFileName));
    for (const iconName of customIconNames) {
      assert.include(Pathway_PIERRE_ICONS.spriteSheet, `id="${iconName}"`);
    }
  });

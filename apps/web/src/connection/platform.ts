...
import { bootstrapRemoteBearerSession } from "@spiritdevs/client-runtime/authorization";
...
import { EnvironmentRpcRequestObserver } from "@spiritdevs/client-runtime/rpc";
import {
...
function clientMetadata() {
  ...
  return {
    label: desktop ? "Pathway Desktop" : "Pathway Web",
    ...
    const cloudSession = CloudSession.of({
      clerkToken: Effect.gen(function* () {
        ...
        if (session === null) {
          return yield* new ConnectionBlockedError({
            reason: "authentication",
            detail: "Sign in to Pathway Connect to connect this environment.",
          });
        }
        ...
        if (token === null) {
          return yield* new ConnectionBlockedError({
            reason: "authentication",
            detail: "The Pathway Connect session is unavailable.",
          });
        }
        return token;
      }),

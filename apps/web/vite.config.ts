...
const isSingleOriginDev = process.env.PATHWAY_SINGLE_ORIGIN_DEV === "1";
...
const configuredRelayUrl = repoEnv.VITE_PATHWAY_RELAY_URL?.trim() || "";
...
const configuredMapboxSearchToken = repoEnv.VITE_MAPBOX_SEARCH_TOKEN?.trim() || "";
...
const sourcemapEnv = process.env.PATHWAY_WEB_SOURCEMAP?.trim().toLowerCase();
...
export default defineConfig(() => {
  return {
    ...
    define: {
      ...
      "import.meta.env.VITE_PATHWAY_RELAY_URL": JSON.stringify(configuredRelayUrl),
      ...
      "import.meta.env.VITE_MAPBOX_SEARCH_TOKEN": JSON.stringify(configuredMapboxSearchToken),

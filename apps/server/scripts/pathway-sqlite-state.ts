...
export class SqliteStateDatabaseMissingError extends Schema.TaggedErrorClass<SqliteStateDatabaseMissingError>()(
  ...
  override get message(): string {
    return `Database does not exist at '${this.databasePath}'. Start Pathway once to run migrations.`;
  }
}
...
export class SqliteStateSharedHomeMutationError extends Schema.TaggedErrorClass<SqliteStateSharedHomeMutationError>()(
  ...
  override get message(): string {
    return "Refusing to mutate the shared ~/.pathway database. Use an isolated --base-dir.";
  }
}
...
export const pathwaySqliteStateCommand = Command.make(
  "pathway-sqlite-state",
  {
    ...
    baseDir: Flag.string("base-dir").pipe(
      Flag.withDescription("Explicit Pathway base directory containing userdata/state.sqlite."),
    ),
  ...
  Command.withDescription(
    "Inspect or seed an isolated Pathway SQLite database with automatic backups for writes.",
  ),
);

if (import.meta.main) {
  Command.run(pathwaySqliteStateCommand, { version: "0.0.0" }).pipe(

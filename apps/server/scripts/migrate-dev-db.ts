 ...
 * ~/.pathway database, then run this checkout's migrations against it.
 *
 * `vp run migrate-dev-db` from a worktree:
 *   1. Nukes `<worktree>/.pathway/userdata/state.sqlite`.
...
export class MigrateDevDbNotInWorktreeError extends Schema.TaggedErrorClass<MigrateDevDbNotInWorktreeError>()(
  ...
  override get message(): string {
    return "Not inside a linked git worktree. Pass --base-dir to target an isolated .pathway directory.";
  }
}
...
export class MigrateDevDbSharedHomeError extends Schema.TaggedErrorClass<MigrateDevDbSharedHomeError>()(
  ...
  override get message(): string {
    return "Refusing to rebuild the shared ~/.pathway database. Use an isolated --base-dir.";
  }
}
...
export interface RunMigrateDevDbInput {
  /** Isolated .pathway directory. Defaults to `<worktree>/.pathway` of the cwd. */
  readonly baseDir?: string | undefined;
  /** Source database. Defaults to `~/.pathway/userdata/state.sqlite`. */
...
export const runMigrateDevDb = Effect.fn("runMigrateDevDb")(function* (
  ...
  const sharedHome = path.resolve(options.sharedHome ?? path.join(NodeOS.homedir(), ".pathway"));
...
export const migrateDevDbCommand = Command.make(
  "migrate-dev-db",
  {
    ...
    source: Flag.string("source").pipe(
      Flag.optional,
      Flag.withDescription("Source database. Defaults to ~/.pathway/userdata/state.sqlite."),
    ),
  },
  ...
  Command.withDescription(
    "Rebuild the worktree dev database from a pruned snapshot of the real ~/.pathway data, then run migrations.",
  ),
);

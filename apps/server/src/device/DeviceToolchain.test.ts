...
it.effect(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const previous = process.env.PATHWAY_DEVICE_HUB_ARCHIVE;
      process.env.PATHWAY_DEVICE_HUB_ARCHIVE = archive;
      ...
      yield* Effect.gen(function* () {
        ...
        Effect.ensuring(
          Effect.sync(() => {
            if (previous === undefined) delete process.env.PATHWAY_DEVICE_HUB_ARCHIVE;
            else process.env.PATHWAY_DEVICE_HUB_ARCHIVE = previous;
          }),
        ),
      );
    }).pipe(Effect.scoped, Effect.provide(NodeServices.layer)),

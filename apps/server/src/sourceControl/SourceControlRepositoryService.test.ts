...
import { GitCommandError, SourceControlProviderError } from "@spiritdevs/contracts";
...
const CLONE_URLS = {
  ...
  url: "https://github.com/octocat/pathway",
  sshUrl: "git@github.com:octocat/pathway.git",
};
...
it.effect("looks up repositories through the requested provider without search", () => {
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(calls, [{ cwd: "/workspace", repository: "octocat/pathway" }]);
  ...
  Effect.gen(function* () {
    ...
    yield* Effect.gen(function* () {
      ...
      assert.deepStrictEqual(cloneCalls, [
        {
          cwd: parent,
          args: ["clone", CLONE_URLS.url, "pathway"],
        },
      ]);
    }).pipe(
      Effect.provide(
...
it.effect("preserves destination probe failures instead of treating them as missing paths", () => {
  const fileSystemCause = PlatformError.systemError({
    ...
    module: "FileSystem",
    method: "exists",
    pathOrDescriptor: "/restricted/pathway",
  });
  ...
  return Effect.gen(function* () {
    ...
    const error = yield* Effect.flip(
      service.cloneRepository({
        ...
        destinationPath: "/restricted/pathway",
      }),
    );
...
it.effect("publishes by creating the repository, adding a remote, and pushing upstream", () => {
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(createCalls, [
      { cwd: "/workspace", repository: "octocat/pathway", visibility: "private" },
    ]);

...
import { GitCommandError } from "@spiritdevs/contracts";
...
const bitbucketPullRequest = {
  ...
  links: {
    html: {
      href: "https://bitbucket.org/coreybain/pathway/pull-requests/42",
    },
  },
  source: {
    branch: { name: "feature/source-control" },
    repository: {
      full_name: "octocat/pathway",
...
const repositoryJson = {
  full_name: "coreybain/pathway",
  links: {
    html: { href: "https://bitbucket.org/coreybain/pathway" },
    clone: [
      { name: "https", href: "https://bitbucket.org/coreybain/pathway.git" },
      { name: "ssh", href: "git@bitbucket.org:coreybain/pathway.git" },
    ],
  },
...
function makeLayer(input: {
  ...
  const gitMock = {
    readConfigValue: vi.fn<GitVcsDriver.GitVcsDriver["Service"]["readConfigValue"]>(() =>
      Effect.succeed<string | null>("git@bitbucket.org:coreybain/pathway.git"),
    ),
    ...
    listRemotes: () =>
      Effect.succeed({
        remotes: [
          {
            name: "origin",
            url: "git@bitbucket.org:coreybain/pathway.git",
            pushUrl: Option.none(),
            isPrimary: true,
          },
        ],
    ...
    Layer.provide(
      ConfigProvider.layer(
        ConfigProvider.fromEnv({
          env: {
            PATHWAY_BITBUCKET_API_BASE_URL: "https://api.test.local/2.0",
            PATHWAY_BITBUCKET_EMAIL: "user@example.com",
            PATHWAY_BITBUCKET_API_TOKEN: "token",
          },
        }),
      ),
    ),
...
it.effect("parses pull request responses from the Bitbucket REST API", () => {
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(result, {
      number: 42,
      title: "Add Bitbucket provider",
      url: "https://bitbucket.org/coreybain/pathway/pull-requests/42",
      baseRefName: "main",
      headRefName: "feature/source-control",
      state: "open",
      updatedAt: Option.some(DateTime.makeUnsafe("2026-01-02T00:00:00.000Z")),
      isCrossRepository: true,
      headRepositoryNameWithOwner: "octocat/pathway",
      headRepositoryOwnerLogin: "octocat",
    });
    assert.strictEqual(
      execute.mock.calls[0]?.[0].url,
      "https://api.test.local/2.0/repositories/coreybain/pathway/pullrequests/42",
    );
...
it.effect("lists pull requests with Bitbucket state and source branch query params", () => {
        ...
        values: [
          {
            ...
            source: {
              ...
              repository: { full_name: "coreybain/pathway" },
            },
          },
        ],
      }),
  ...
  return Effect.gen(function* () {
    ...
    assert.strictEqual(
      request?.url,
      "https://api.test.local/2.0/repositories/coreybain/pathway/pullrequests",
    );
...
it.effect("reads repository clone URLs and default branch", () => {
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(cloneUrls, {
      nameWithOwner: "coreybain/pathway",
      url: "https://bitbucket.org/coreybain/pathway.git",
      sshUrl: "git@bitbucket.org:coreybain/pathway.git",
    });
  ...
  () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.deepStrictEqual(
        ...
        [
          "https://api.test.local/2.0/repositories/coreybain/pathway",
          "https://api.test.local/2.0/repositories/coreybain/pathway/branching-model",
        ].toSorted(),
      );
    }).pipe(Effect.provide(layer));
  },
...
it.effect("creates repositories through the Bitbucket REST API", () => {
  ...
  return Effect.gen(function* () {
    ...
    const request = execute.mock.calls[0]?.[0];
    assert.strictEqual(request?.url, "https://api.test.local/2.0/repositories/coreybain/pathway");
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(JSON.parse(new TextDecoder().decode(rawBody)), {
      ...
      source: {
        ...
        repository: { full_name: "owner/pathway" },
      },
      destination: {
        branch: { name: "main" },
      },
  ...
  return Effect.gen(function* () {
    ...
    yield* bitbucket.checkoutPullRequest({
      ...
      context: {
        ...
        remoteName: "origin",
        remoteUrl: "git@bitbucket.org:coreybain/pathway.git",
      },
      reference: "42",
      force: true,
    });
...
it.effect("checks out fork pull requests through an ensured fork remote", () => {
  const { git, layer } = makeLayer({
    response: (request) => {
      if (request.url.endsWith("/repositories/octocat/pathway")) {
        return Response.json({
          ...repositoryJson,
          full_name: "octocat/pathway",
          links: {
            html: { href: "https://bitbucket.org/octocat/pathway" },
            clone: [
              { name: "https", href: "https://bitbucket.org/octocat/pathway.git" },
              { name: "ssh", href: "git@bitbucket.org:octocat/pathway.git" },
            ],
          },
        });
      }
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(git.ensureRemote.mock.calls[0]?.[0], {
      ...
      url: "git@bitbucket.org:octocat/pathway.git",
    });
    assert.deepStrictEqual(git.fetchRemoteBranch.mock.calls[0]?.[0], {
      cwd: "/repo",
      remoteName: "octocat",
      remoteBranch: "main",
      localBranch: "pathway/pr-42/main",
    });
    assert.deepStrictEqual(git.setBranchUpstream.mock.calls[0]?.[0], {
      cwd: "/repo",
      branch: "pathway/pr-42/main",
      remoteName: "octocat",
      remoteBranch: "main",
    });
    assert.deepStrictEqual(git.switchRef.mock.calls[0]?.[0], {
      cwd: "/repo",
      refName: "pathway/pr-42/main",
    });
  }).pipe(Effect.provide(layer));
});

...
import { sanitizeBranchFragment } from "@spiritdevs/shared/git";
...
const BitbucketApiEnvConfig = Config.all({
  baseUrl: Config.string("PATHWAY_BITBUCKET_API_BASE_URL").pipe(
    Config.withDefault(DEFAULT_API_BASE_URL),
  ),
  accessToken: Config.string("PATHWAY_BITBUCKET_ACCESS_TOKEN").pipe(Config.option),
  email: Config.string("PATHWAY_BITBUCKET_EMAIL").pipe(Config.option),
  apiToken: Config.string("PATHWAY_BITBUCKET_API_TOKEN").pipe(Config.option),
});
...
>()("@spiritdevs/pathway/sourceControl/BitbucketApi") {}
...
function checkoutBranchName(input: {
  ...
  return `pathway/pr-${input.pullRequestId}/${sanitizeBranchFragment(input.headBranch)}`;
}
...
function authFromConfig(
  ...
  return {
    ...
    host: Option.some("bitbucket.org"),
    detail: Option.some(
      "Set PATHWAY_BITBUCKET_EMAIL and PATHWAY_BITBUCKET_API_TOKEN, or PATHWAY_BITBUCKET_ACCESS_TOKEN.",
    ),
  };
}

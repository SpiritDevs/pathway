...
/**
 * A pinned runtime is an exact `@spiritdevs/pathway@<version>` npm-installed into
 ...
 * switching over, never `npx @spiritdevs/pathway`, whose cache is ephemeral and whose
...
export function pinnedRuntimePaths(
  ...
  return {
    versionDir,
    entryPath: path.join(versionDir, "node_modules", "@spiritdevs", "pathway", "dist", "bin.mjs"),
...
/**
 * Installs `@spiritdevs/pathway@<version>` into the pinned runtime directory unless a complete
...
const installPinnedRuntime = Effect.fn("cloud.pinned_runtime.ensure_installed")(function* (
  ...
  return yield* Effect.gen(function* () {
    const installStep = "installing the pinned Pathway runtime (this can take a few minutes)";
    yield* runner
      .run({
        command: "npm",
        args: [
          ...
          "--no-fund",
          "--no-audit",
          `@spiritdevs/pathway@${input.version}`,
        ],

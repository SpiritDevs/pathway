import { HostProcessExecutablePath, HostProcessPlatform } from "@spiritdevs/shared/hostProcess";
...
export function renderBootServiceUnit(plan: BootServicePlan): string {
  ...
  return [
    ...
    "Type=simple",
    "WorkingDirectory=%h",
    `Environment=PATHWAY_HOME=${quoteSystemdValue(plan.baseDir)}`,
...
export class BootServiceInstallError extends Schema.TaggedErrorClass<BootServiceInstallError>()(
  ...
  override get message(): string {
    return "Could not set up the Pathway background service.";
  }
}
...
>()("@spiritdevs/pathway/cloud/bootService") {}
            ...
            Effect.mapError(
              (cause) =>
                new PinnedRuntimeInstallError({
                  step: "verifying the pinned Pathway runtime",
                  cause,
                }),
            ),

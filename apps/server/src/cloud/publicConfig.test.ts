...
it.effect("prefers a runtime relay URL override over the statically injected value", () =>
  Effect.gen(function* () {
    const relayUrl = yield* makeRelayUrlConfig("https://embedded.example.test").pipe(
      provideEnv({ PATHWAY_RELAY_URL: "https://runtime.example.test///" }),
    );
...
it.effect("rejects an insecure runtime relay URL override", () =>
  makeRelayUrlConfig("https://embedded.example.test").pipe(
    provideEnv({ PATHWAY_RELAY_URL: "http://runtime.example.test" }),
    Effect.flip,
  ),
);
  ...
  Effect.gen(function* () {
    assert.equal(
      yield* hostedAppUrlConfig.pipe(
        provideEnv({ PATHWAY_HOSTED_APP_URL: "https://app.pathwayos.dev" }),
      ),
...
it.effect("rejects malformed or insecure hosted app URLs", () =>
  Effect.gen(function* () {
    for (const value of [
      "app.pathwayos.dev",
      "http://app.pathwayos.dev",
      "https://app.pathwayos.dev/nested",
      "https://app.pathwayos.dev?alias=true",
    ]) {
...
it.effect("prefers runtime Clerk OAuth config overrides over statically injected values", () =>
  Effect.gen(function* () {
    const config = yield* makeCloudCliOAuthConfig({
      ...
      provideEnv({
        PATHWAY_CLERK_PUBLISHABLE_KEY: "pk_test_cnVudGltZS5leGFtcGxlLnRlc3Qk",
        PATHWAY_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_client_runtime",
      }),
    );
...
it("resolves relay client tracing from runtime config with build-time fallback", () => {
  ...
  assert.deepEqual(
    resolveRelayClientTracingConfig(
      {
        PATHWAY_RELAY_CLIENT_OTLP_TRACES_URL: "https://runtime.example.test/v1/traces",
        PATHWAY_RELAY_CLIENT_OTLP_TRACES_DATASET: "runtime-dataset",
        PATHWAY_RELAY_CLIENT_OTLP_TRACES_TOKEN: "runtime-token",
      },
      fallback,
    ),
    {
  ...
  assert.equal(
    resolveRelayClientTracingConfig(
      {
        PATHWAY_RELAY_CLIENT_OTLP_TRACES_URL: "http://insecure.example.test/v1/traces",
        PATHWAY_RELAY_CLIENT_OTLP_TRACES_DATASET: "runtime-dataset",
        PATHWAY_RELAY_CLIENT_OTLP_TRACES_TOKEN: "runtime-token",
      },
      fallback,

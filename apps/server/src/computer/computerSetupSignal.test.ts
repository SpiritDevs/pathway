...
import { PATHWAY_DESKTOP_BUNDLE_ID_ENV } from "@spiritdevs/shared/computerGrants";
...
const previousBundleId = process.env[PATHWAY_DESKTOP_BUNDLE_ID_ENV];

afterEach(() => {
  if (previousBundleId === undefined) {
    delete process.env[PATHWAY_DESKTOP_BUNDLE_ID_ENV];
  } else {
    process.env[PATHWAY_DESKTOP_BUNDLE_ID_ENV] = previousBundleId;
  }
});

describe("responsibleDesktopBundleId", () => {
  it("reads the desktop shell's identifier, and treats blank as unknown", () => {
    expect(responsibleDesktopBundleId({ [PATHWAY_DESKTOP_BUNDLE_ID_ENV]: " com.x.pathway " })).toBe(
      "com.x.pathway",
    );
    expect(responsibleDesktopBundleId({ [PATHWAY_DESKTOP_BUNDLE_ID_ENV]: "   " })).toBeUndefined();
    expect(responsibleDesktopBundleId({})).toBeUndefined();
  });
});

describe("computerSetupSignal", () => {
  const setupError = new ComputerBackendError({ message: "denied", setupRequired: true });
  ...
  it("carries the responsible app onto every signal it raises", () => {
    ...
    // wrong one revokes a different Pathway's grants.
    process.env[PATHWAY_DESKTOP_BUNDLE_ID_ENV] = "com.spiritdevs.pathway.dev";
    expect(
      computerSetupSignal({
        availability: {
  ...
  it("omits the app entirely when no desktop shell is responsible for this server", () => {
    // A server started outside the desktop shell has no app behind it, and an absent field is what
  ...
  it("still reports nothing to do when no grant is missing and nothing failed", () => {
    process.env[PATHWAY_DESKTOP_BUNDLE_ID_ENV] = "com.spiritdevs.pathway";

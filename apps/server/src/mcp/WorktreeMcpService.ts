...
const make = Effect.gen(function* () {
  ...
  const performHandoff = Effect.fn("WorktreeMcpService.performHandoff")(function* (
    ...
    if (project.workspaceRoot === null) {
      return yield* failure(
        "invalid_request",
        `Project '${project.id}' has no workspace directory. Attach a directory before creating a worktree.`,
      );
    }
  ...
  const status: WorktreeMcpService["Service"]["status"] = Effect.fn("WorktreeMcpService.status")(
    function* (scope) {
      ...
      if (project.workspaceRoot === null) {
        return yield* failure(
          "invalid_request",
          `Project '${project.id}' has no workspace directory. Attach a directory before using worktree tools.`,
        );
      }

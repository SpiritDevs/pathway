// @effect-diagnostics nodeBuiltinImport:off - effect has no synchronous SHA-256; the guard digests keys inline on every check and record.
/**
 * Refuses confirmed computer-mutation retry loops before they reach the desktop.
 *
 * @module mcp/toolkits/computer/computerProgressGuard
 */
...
const digest = (value: string): string =>
  NodeCrypto.createHash("sha256").update(value).digest("base64url");
...
export class ComputerProgressGuard {
  ...
  private readonly maximumTargetsPerScope: number;

  constructor(maximumScopes = 256, maximumTargetsPerScope = 8) {
    this.maximumScopes = maximumScopes;
    this.maximumTargetsPerScope = maximumTargetsPerScope;

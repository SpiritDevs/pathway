/**
 * Agent-facing desktop perception and control tools: the `computer_*` family.
 *
 * Every handler is an Effect. Cancelling a call is interrupting its fiber;
 * nothing threads an abort signal. Per-thread state (screenshot frames,
 * element ref tables, digests, the progress guard, disclosures and guidance
 * cadence) lives in closures created when `makeComputerTools` runs.
 *
 * @module mcp/toolkits/computer/computerTools
 */
// @effect-diagnostics preferSchemaOverJson:off - tool results and refusal messages are JSON text the model reads.
import {
...
import {
  COMPUTER_APPROVAL_QUEUE_FULL_CODE,
...
/** Every failure a Computer tool body can end with; `handle` maps each to a result. */
export type ComputerToolFailure = ComputerOperationError | ToolInputError | ComputerToolError;

/** One Computer tool body. The handler wrapper owns approval, audit and result shaping. */
export type ComputerToolRun = (
  args: Record<string, unknown>,
  context: ToolContext,
) => Effect.Effect<unknown, ComputerToolFailure>;

// Root-class guards, compiled once. `Schema.is` on a subclass would accept its
// siblings, so subclasses keep their `instanceof` guards in computerErrors.
const isApprovalQueueFullError = Schema.is(ComputerApprovalQueueFullError);
const isComputerBackendError = Schema.is(ComputerBackendError);
const isComputerTargetError = Schema.is(ComputerTargetError);
const isComputerToolError = Schema.is(ComputerToolError);
 ...
 * the result resolved when one rode it.
 */
function computerAuditTarget(
  args: Record<string, unknown>,
  resultWindowId: string | undefined,
): ComputerAuditEntry["target"] | undefined {
  const windowId = peekWindowIdArg(args) ?? resultWindowId;
  const pid =
    typeof args.pid === "number" && Number.isSafeInteger(args.pid) && args.pid > 0
      ? args.pid
      : undefined;
  const app = typeof args.app === "string" && args.app.trim().length > 0 ? args.app : undefined;
...
export interface ComputerToolsOptions {
  ...
  /**
   * Asks the user before a gated call runs. Absent means nobody can answer, so
   * a call that raises a window or invokes a menu is refused instead.
   */
  readonly authorizeAction?: ComputerAuthorizeAction;
  /**
   * Called when a tool call failed because the OS is withholding a privacy
   * grant Pathway needs. The MCP server turns it into one actionable chat card;
 ...
 * MCP server photographs afterwards, or a result that already carries its own
 ...
 */
const validateInspectionArguments = Effect.fnUntraced(function* (
  value: unknown,
  definition: ToolEntry["definition"],
) {
  if (value === null || typeof value !== "object" || Array.isArray(value)) {
    return yield* invalidArgument('"arguments" must be an object.');
  }
  const args = value as Record<string, unknown>;
  const schema = definition.inputSchema;
  const properties = schema.properties as Record<string, Record<string, unknown>> | undefined;
  if (schema.type !== "object" || schema.additionalProperties !== false || !properties) {
    return yield* invalidArgument("The inspection schema cannot be validated.");
  }
  const required = schema.required as readonly string[] | undefined;
  for (const key of required ?? []) {
    if (!Object.hasOwn(args, key))
      return yield* invalidArgument(`Missing required argument "${key}".`);
  }
  for (const [key, property] of Object.entries(properties)) {
    ...
    if (
      ...
      return yield* invalidArgument(`Argument "${key}" must be a ${property.type}.`);
    }
  }
...
const readWindowIdArg = Effect.fnUntraced(function* (args: Record<string, unknown>) {
  return (yield* readStringArg(args, "window_id")) ?? (yield* readStringArg(args, "windowId"));
});

/**
 * The window id the call declared, read without failing - for the audit entry,
 * which records a call whose arguments may be the reason it was refused.
 */
function peekWindowIdArg(args: Record<string, unknown>): string | undefined {
  for (const key of ["window_id", "windowId"]) {
    const value = args[key];
 ...
 * preferring one.
 */
const readMenuTargetArg = Effect.fnUntraced(function* (
  args: Record<string, unknown>,
): Effect.fn.Return<ComputerMenuTarget, ToolInputError> {
  const windowId = yield* readWindowIdArg(args);
  const app = yield* readStringArg(args, "app");
  const pid = yield* readNumberArg(args, "pid");
  ...
  return yield* invalidArgument('"pid" must be a positive integer.');
});

/** The one-to-six-title menu path both the tool and a run step accept. */
const readMenuPathArg = Effect.fnUntraced(function* (
  args: Record<string, unknown>,
  subject: string,
) {
  const path = yield* readStringArrayArg(args, "path");
  if (!path?.length || path.length > 6 || path.some((title) => title.trim().length === 0)) {
    return yield* invalidArgument(`${subject} needs a path of one to six non-empty menu titles.`);
  }
  return path;
});

const readScreenshotIdArg = Effect.fnUntraced(function* (args: Record<string, unknown>) {
  return (
    (yield* readStringArg(args, "screenshot_id")) ?? (yield* readStringArg(args, "screenshotId"))
  );
});
...
const readScreenshotTarget = Effect.fnUntraced(function* (
  args: Record<string, unknown>,
): Effect.fn.Return<ScreenshotTarget, ToolInputError> {
  const x = yield* readNumberArg(args, "x");
  const y = yield* readNumberArg(args, "y");
  const screenshotId = yield* readScreenshotIdArg(args);
  ...
  const label = yield* readVerbatimStringArg(args, "label");
  const role = yield* readStringArg(args, "role");
  const windowId = yield* readWindowIdArg(args);
  const ref = yield* readNumberArg(args, "ref");
  if (ref !== undefined && (!Number.isSafeInteger(ref) || ref < 0)) {
    return yield* invalidArgument('Argument "ref" must be a non-negative integer.');
  }
  const refOrdinal =
    (yield* readNumberArg(args, "ref_ordinal")) ?? (yield* readNumberArg(args, "refOrdinal"));
  if (refOrdinal !== undefined && (!Number.isSafeInteger(refOrdinal) || refOrdinal < 0)) {
    return yield* invalidArgument('Argument "ref_ordinal" must be a non-negative integer.');
  }
  return {
    ...(x !== undefined ? { x } : {}),
    ...(y !== undefined ? { y } : {}),
...
const readNestedScreenshotTarget = Effect.fnUntraced(function* (
  args: Record<string, unknown>,
  name: string,
) {
  const value = yield* readRecordArg(args, name);
  if (!value) return yield* invalidArgument(`Missing required argument "${name}".`);
  return yield* readScreenshotTarget(value);
});

const readDelta = Effect.fnUntraced(function* (args: Record<string, unknown>, name: string) {
  const value = yield* readNumberArg(args, name);
  if (value === undefined) return yield* invalidArgument(`Missing required argument "${name}".`);
  return value;
});

const readScrollDelta = Effect.fnUntraced(function* (args: Record<string, unknown>) {
  const deltaX = (yield* readNumberArg(args, "delta_x")) ?? 0;
  const deltaY = (yield* readNumberArg(args, "delta_y")) ?? 0;
  if (deltaX === 0 && deltaY === 0) {
    return yield* invalidArgument('Scroll needs a nonzero "delta_x" or "delta_y".');
  }
 ...
 */
const readDragDurationMs = Effect.fnUntraced(function* (args: Record<string, unknown>) {
  const value = yield* readNumberArg(args, "duration_ms");
  if (value === undefined) return DEFAULT_DRAG_DURATION_MS;
  return Math.min(COMPUTER_DRAG_MAX_DURATION_MS, Math.max(0, value));
});

function readRawRequiredString(
  args: Record<string, unknown>,
  name: string,
  ...
  return typeof value === "string"
    ...
    : invalidArgument(`Argument "${name}" must be a string.`);
}

const readRequiredText = Effect.fnUntraced(function* (args: Record<string, unknown>) {
  const value = yield* readRawRequiredString(args, "text");
  if (value.length > COMPUTER_TEXT_MAX_LENGTH)
    return yield* invalidArgument('Argument "text" is too long.');
  return value;
});
 ...
 */
const readSetValueValue = Effect.fnUntraced(function* (args: Record<string, unknown>) {
  const value = yield* readRawRequiredString(args, "value");
  if (value.length > COMPUTER_TEXT_MAX_LENGTH)
    return yield* invalidArgument('Argument "value" is too long.');
  return value;
});
 ...
 */
const readSelectTextRange = Effect.fnUntraced(function* (
  args: Record<string, unknown>,
): Effect.fn.Return<ComputerTextRange, ToolInputError> {
  const start = yield* readNumberArg(args, "start");
  const length = yield* readNumberArg(args, "length");
  if (start === undefined || length === undefined) {
    return yield* invalidArgument('Arguments "start" and "length" are required.');
  }
 ...
 */
const readSelectTextTarget = Effect.fnUntraced(function* (
  args: Record<string, unknown>,
): Effect.fn.Return<ComputerTarget, ToolInputError> {
  const target = yield* readScreenshotTarget(args);
 ...
 */
const readClickGesture = Effect.fnUntraced(function* (
  args: Record<string, unknown>,
): Effect.fn.Return<
  { count?: 1 | 2 | 3; button?: "left" | "right" | "middle" } | undefined,
  ToolInputError
> {
  const count = yield* readNumberArg(args, "count");
  const button = yield* readStringArg(args, "button");
  if (count === undefined && button === undefined) return undefined;
  if (count !== undefined && (!Number.isSafeInteger(count) || count < 1 || count > 3)) {
    return yield* invalidArgument('Argument "count" must be 1, 2 or 3.');
  }
  if (button !== undefined && button !== "left" && button !== "right" && button !== "middle") {
    return yield* invalidArgument('Argument "button" must be "left", "right" or "middle".');
  }
  return {
 ...
 */
const readKeyOrChord = Effect.fnUntraced(function* (
  args: Record<string, unknown>,
): Effect.fn.Return<
  { readonly key: string; readonly chord: readonly string[] | undefined },
  ToolInputError
> {
  const key = yield* readStringArg(args, "key", { required: true });
...
const readActionName = Effect.fnUntraced(function* (args: Record<string, unknown>) {
  const value = yield* readStringArg(args, "action", { required: true });
...
const readClipboardText = Effect.fnUntraced(function* (args: Record<string, unknown>) {
  const value = yield* readRawRequiredString(args, "text");
  if (new TextEncoder().encode(value).byteLength > MAX_COMPUTER_CLIPBOARD_BYTES) {
    return yield* invalidArgument(
 ...
 */
const readCaptureRequest = Effect.fnUntraced(function* (
  args: Record<string, unknown>,
  mapRegion: (
    region: ComputerRect,
  ) => Effect.Effect<ComputerRect, ComputerTargetError | ToolInputError>,
): Effect.fn.Return<ScreenshotRequest, ToolInputError | ComputerTargetError> {
  const windowId = yield* readWindowIdArg(args);
  const present = CAPTURE_REGION_KEYS.filter(
    (key) => args[key] !== undefined && args[key] !== null,
  );
  const maxDimension = yield* readCaptureMaxDimension(args);
  ...
  const region = {
    x: (yield* readNumberArg(args, "x"))!,
    y: (yield* readNumberArg(args, "y"))!,
    width: (yield* readNumberArg(args, "width"))!,
    height: (yield* readNumberArg(args, "height"))!,
  };
  if (region.width <= 0 || region.height <= 0) {
    return yield* invalidArgument('Arguments "width" and "height" must be greater than zero.');
  }
  return { kind: "region", region: yield* mapRegion(region), ...limit };
});
 ...
 */
const readCaptureMaxDimension = Effect.fnUntraced(function* (args: Record<string, unknown>) {
  const value = yield* readNumberArg(args, "max_dimension");
  if (value === undefined) return undefined;
  if (value < 1) return yield* invalidArgument('Argument "max_dimension" must be at least 1.');
 ...
 * would say so.
 */
const readModifiers = Effect.fnUntraced(function* (
  args: Record<string, unknown>,
): Effect.fn.Return<readonly ComputerInputModifier[], ToolInputError> {
  const raw = yield* readStringArrayArg(args, "modifiers");
 ...
 * a long sleep.
 */
const readWaitDurationMs = Effect.fnUntraced(function* (args: Record<string, unknown>) {
  const value = yield* readNumberArg(args, "duration_ms");
  if (value === undefined)
    return yield* invalidArgument('Missing required argument "duration_ms".');
...
export function makeComputerTools(options: ComputerToolsOptions): ReadonlyArray<ToolEntry> {
  ...
  const withActionElementChanges = Effect.fnUntraced(function* (
    result: ComputerActionResult,
    context: ToolContext,
  ): Effect.fn.Return<ComputerActionResult, ComputerOperationError> {
    ...
    return yield* Effect.gen(function* () {
      ...
      const { root } = yield* manager.getState({
        windowId,
        includeTree: true,
        includeText: false,
      ...
      Effect.catch(() => assertDesktopOperationActive.pipe(Effect.as(result))),
    );
  });
   ...
   */
  const deliverScreenshot = Effect.fnUntraced(function* (
    threadId: string,
    payload: Record<string, unknown>,
    screenshot: ComputerScreenshot,
    windowId?: string,
  ): Effect.fn.Return<McpToolCallResult, ComputerOperationError | ToolInputError> {
    yield* assertDesktopOperationActive;
    if (windowId && screenshot.windowId && windowId !== screenshot.windowId) {
      return yield* invalidArgument("Screenshot identity differs from the requested window.");
    }
    windowId ??= screenshot.windowId;
    // PATHWAY_CUA_CAPTURE_REUSE: when the fresh capture is byte-for-byte the
  ...
  const resolveTarget = Effect.fnUntraced(function* (
    ...
    threadId: string,
  ): Effect.fn.Return<ComputerTarget, ToolInputError | ComputerTargetError> {
    ...
    const frame = yield* frames.resolve(threadId, screenshotId);
    if (frame.windowId && rest.windowId && frame.windowId !== rest.windowId)
      return yield* invalidArgument("Screenshot and action name different windows.");
    const resolved = {
      ...rest,
      ...(yield* screenshotPointToDesktop(frame, target.x, target.y)),
      ...(frame.windowId ? { windowId: frame.windowId, observedWindowBounds: frame.region } : {}),
    };
    return resolved;
  });

  const readTarget = (args: Record<string, unknown>, context: ToolContext) =>
    Effect.flatMap(readScreenshotTarget(args), (target) =>
      resolveTarget(target, context.callerThreadId),
    );

  const readNestedTarget = (args: Record<string, unknown>, name: string, context: ToolContext) =>
    Effect.flatMap(readNestedScreenshotTarget(args, name), (target) =>
      resolveTarget(target, context.callerThreadId),
    );
  ...
  const foregroundAuthorization = (
    ...
    if (resolve === undefined) return Effect.succeed(COMPUTER_FOREGROUND_NOT_AUTHORIZED);
    // A resolver that dies refuses too: the answer is never an inferred yes.
    return resolve(context).pipe(
  ...
  /**
   * Wraps one tool body with everything a Computer call shares: the progress
   * guard, the approval gate, the audit record, turn timing, the activity
   * badge, delivery mode, and the result shaping - setup notes and cards,
   * first-mutation disclosure and guidance cadence. Every failure the body
   * can end with becomes a tool result here; the handler itself never fails.
   */
  const handle =
    (name: string, run: ComputerToolRun): ToolHandler =>
    (args, context) =>
      Effect.suspend(() => {
        const guidance = guidanceCadence.shouldRefresh(context.callerThreadId)
          ? COMPUTER_TOOL_REFRESH_GUIDANCE
          : undefined;
        // The audit record's resolved field, filled as the call learns it:
        // the delivered window id after dispatch.
        ...
        const audit = (outcome: {
          ...
          if (!COMPUTER_AUDITED_TOOLS.has(name)) return Effect.void;
          const target = computerAuditTarget(args, resultWindowId);
        ...
        const settled = (
          result: McpToolCallResult,
        ): { result: McpToolCallResult; signal: ComputerSetupSignal | undefined } => ({
          result,
          signal: undefined,
        });
        ...
        const attempt = Effect.gen(function* () {
          ...
          if (mutating && context.callerTurnId) {
            ...
            if (blocked) {
              yield* audit({ effect: "refused", code: blocked.code });
              return settled({ ...mcpToolResultJson({ error: blocked }), isError: true });
            }
          }
          // A session with an approval gate asks it for every gated call. A
          // call that raises a window or invokes a menu is never allowed
          // without one, so a session that has none refuses those.
          if (
            ...
            if (!options.authorizeAction) {
              ...
              return settled(approvalUnavailableResult(name));
            }
            ...
            if (Result.isFailure(decision)) {
              ...
              if (isApprovalQueueFullError(error)) {
                yield* audit({ effect: "refused", code: COMPUTER_APPROVAL_QUEUE_FULL_CODE });
                return settled({
                  ...mcpToolResultJson({
                    error: {
                      code: COMPUTER_APPROVAL_QUEUE_FULL_CODE,
                      message: error.message,
                      retryable: error.retryable,
                    },
                  }),
              ...
              // ComputerApprovalPublishError: the card could not be posted, so
              // nobody could have answered it.
              yield* audit({ effect: "refused", code: "approval_unavailable" });
              return settled(approvalUnavailableResult(name));
            }
            if (decision.success === "denied") {
              yield* audit({ effect: "refused", code: "approval_denied" });
              return settled(
                mcpToolResultError("Computer action was denied or cancelled; no input was sent."),
              );
            }
            // The card is still open. Nothing was sent and nothing was decided,
            // so the call leaves no audit entry and no progress record.
            if (decision.success === "pending") {
              return settled(
                mcpToolResultJson({
          ...
          const body = Effect.suspend(() => run(args, context));
          const invoke = withComputerTask(
            {
              threadId: context.callerThreadId,
              ...(context.callerTurnId ? { turnId: context.callerTurnId } : {}),
            ...
            name === "computer_get_state" ||
              ...
              name === "computer_run"
              ? withModelDesktopObservation(body)
              : body,
          );
          const during = manager.cursorActivity.during(
            context.callerThreadId,
          ...
          const operationKey =
            ...
            manager.supportsFocusNeutralSemanticText
              ? yield* readWindowIdArg(args)
              : undefined;
          const value: unknown =
            name === "computer_wait"
              ? yield* context.assertCallerTurnActive().pipe(
                  Effect.andThen(during),
                  Effect.tap(() => context.assertCallerTurnActive()),
                )
                  ...
                  Effect.gen(function* () {
                    ...
                    const authorization =
                      foreground && !restoresOwnForeground
                        ? yield* foregroundAuthorization(context)
                        : undefined;
                    return yield* withDesktopDeliveryMode(
                      ...
                      // The excursion only speaks desktop errors, so the body's
                      // own failures ride through it as a value.
                      foreground && !restoresOwnForeground
                        ? manager
                            .withForegroundRestore(
                              context.callerThreadId,
                              Effect.result(during),
                              authorization,
                            )
          ...
          // take the same route to the same card as a refusal.
          //
          // Read rather than remembered: the read costs a round trip only
          ...
          const signal = computerSetupSignal({
            ...
            missing: yield* manager.missingPermissions(),
        ...
        const failed = Effect.fnUntraced(function* (error: ComputerToolFailure) {
          ...
          const backendError = isComputerBackendError(error) ? error : undefined;
          if (!backendError?.controlRevoked) {
            yield* audit(outcome);
          }
          ...
          if (progressAction && !(backendError?.setupRequired || backendError?.controlRevoked))
            progressGuard.record(progressAction, outcome);
          const cuaError = isCuaActionError(error) ? error : undefined;
          const failure: McpToolCallResult = backendError?.inputPause
            ? {
                ...mcpToolResultJson({
                  error: {
                    code: "computer_input_paused",
                    ...backendError.inputPause,
                    layer: cuaError ? cuaError.layer : "server-manager",
                    retryable: false,
                    ...(cuaError
                      ? { cause: cuaError.code, diagnostics: cuaError.diagnostics }
                      : {}),
                    requery_hint: INPUT_PAUSE_REQUERY_HINT,
                    ...(cuaError && cuaError.waitSeconds !== undefined
                      ? { wait_seconds: cuaError.waitSeconds }
                      : {}),
                  },
                  ...(cuaError ? { effect: cuaError.effect, retryAllowed: false } : {}),
                }),
                isError: true,
              }
            : cuaError
              ? {
                  ...mcpToolResultJson(cuaActionErrorPayload(cuaError)),
                  isError: true,
                }
              : isComputerTargetError(error)
                ? targetErrorResult(error)
                : isComputerLeaseError(error)
                  ? leaseErrorResult(error)
                  : isComputerToolError(error)
                    ? computerToolErrorResult(error)
          ...
          return yield* withSetupCard(
            name,
            context,
            signal,
            withSetupNoteOnResult(failure, signal),
          );
        });
        ...
        return attempt.pipe(
          Effect.matchEffect({
            onFailure: failed,
            onSuccess: ({ result, signal }) => withSetupCard(name, context, signal, result),
          }),
  ...
  /**
   * Direct MCP clients may call these tools by name; provider models use
  ...
  ): Effect.Effect<unknown, ComputerOperationError | ToolInputError> => {
    if (!capture) return Effect.succeed(result);
   ...
   */
  const observeAfterAction = Effect.fnUntraced(function* (
    args: Record<string, unknown>,
    result: ComputerActionResult,
    context: ToolContext,
  ): Effect.fn.Return<unknown, ComputerOperationError | ToolInputError> {
    if ((yield* readBooleanArg(args, "include_screenshot")) === false) return result;
    const label = yield* readVerbatimStringArg(args, "wait_for_label");
    const windowId = result.windowId;
    const readiness =
      label === undefined
        ? undefined
        : windowId === undefined
          ...
          : yield* waitForControl(
              ...
              Effect.catch((error) =>
                assertDesktopOperationActive.pipe(
                  Effect.as({ status: "unavailable", note: errorText(error) }),
                ),
              ),
            );
  ...
  const observedActionEntry = (
    ...
    run: (
      args: Record<string, unknown>,
      context: ToolContext,
    ) => Effect.Effect<ObservedActionOutcome, ComputerToolFailure>,
    annotations: Record<string, unknown> = WRITE_TOOL_ANNOTATIONS,
  ): ToolEntry =>
    actionEntry(
      name,
      ...
      Effect.fnUntraced(function* (args, context) {
        if (args.wait_for_label !== undefined) {
          if (!(yield* readStringArg(args, "wait_for_label"))?.trim())
            return yield* invalidArgument("wait_for_label must be a nonempty label.");
          if ((yield* readBooleanArg(args, "include_screenshot")) === false)
            return yield* invalidArgument("wait_for_label requires the action screenshot.");
        }
        const outcome = yield* run(args, context);
        ...
        const observed = [
          ...
          ? yield* withActionElementChanges(result, context)
          : result;
        return yield* "result" in outcome && args.wait_for_label === undefined
  ...
  interface PreparedRunStep {
    ...
    readonly run: Effect.Effect<unknown, ComputerToolFailure>;
  }

  /**
   * Parse one step into a ready-to-run effect. Every argument reader runs
   ...
   */
  const prepareRunStep = Effect.fnUntraced(function* (
    type: string,
    step: Record<string, unknown>,
    context: ToolContext,
  ): Effect.fn.Return<
    Effect.Effect<unknown, ComputerToolFailure>,
    ToolInputError | ComputerTargetError
  > {
    const threadId = context.callerThreadId;
    switch (type) {
      case "click": {
        const target = yield* readTarget(step, context);
        const modifiers = yield* readModifiers(step);
        const gesture = yield* readClickGesture(step);
      ...
      case "drag": {
        const from = yield* readNestedTarget(step, "from", context);
        const to = yield* readNestedTarget(step, "to", context);
        const durationMs = yield* readDragDurationMs(step);
      ...
      case "scroll": {
        ...
        const raw = yield* readScreenshotTarget(step);
        const frame = yield* frames.resolve(threadId, raw.screenshotId);
        const resolved = yield* resolveTarget(raw, threadId);
        const target =
          !hasTargetFields(resolved) && frame.windowId !== undefined
            ? { ...resolved, windowId: frame.windowId }
            : resolved;
        const requestedDelta = yield* readScrollDelta(step);
      ...
      case "type_text": {
        const text = yield* readRequiredText(step);
        const target = yield* readTarget(step, context);
      ...
      case "press_key": {
        const { key, chord } = yield* readKeyOrChord(step);
        const target = yield* readTarget(step, context);
      ...
      case "set_value": {
        const target = yield* readTarget(step, context);
        const value = yield* readSetValueValue(step);
        return manager.setValue(threadId, target, value);
      }
      case "perform_action": {
        const target = yield* readTarget(step, context);
        const action = yield* readActionName(step);
        return manager.performAction(threadId, target, action);
      }
      case "select_text": {
        const target = yield* resolveTarget(yield* readSelectTextTarget(step), threadId);
        const range = yield* readSelectTextRange(step);
        return manager.selectText(threadId, target, range);
      }
      case "wait": {
        const durationMs = yield* readWaitDurationMs(step);
        const absent = (yield* readBooleanArg(step, "absent")) === true;
        const raw = yield* readScreenshotTarget(step);
        const target =
          raw.ref !== undefined ||
          raw.label !== undefined ||
          ...
          raw.refOrdinal !== undefined
            ? yield* resolveTarget(raw, threadId)
            : undefined;
        if (target !== undefined) {
        ...
        if (absent) {
          return yield* invalidArgument('A "wait" step with "absent" requires an element target.');
        }
        return (durationMs > 0 ? Effect.sleep(durationMs) : Effect.void).pipe(
          Effect.as({ waitedMs: durationMs }),
        );
      }
      case "activate_window": {
        const windowId = yield* readWindowIdArg(step);
        if (windowId === undefined) {
          return yield* invalidArgument('Step "activate_window" requires "window_id".');
        }
        ...
        return Effect.flatMap(foregroundAuthorization(context), (authorization) =>
          withDesktopDeliveryMode(
            "foreground",
      ...
      case "launch_app": {
        const app = yield* readStringArg(step, "app", { required: true });
        const appArgs = (yield* readStringArrayArg(step, "arguments")) ?? [];
        const waitMs = (yield* readBooleanArg(step, "wait_for_window")) === false ? 0 : 2_000;
        ...
        const hidden = yield* readBooleanArg(step, "hidden");
        return Effect.gen(function* () {
          if (dialect !== "macos" && hidden === false) {
            const authorization = yield* foregroundAuthorization(context);
      ...
      case "write_clipboard": {
        const text = yield* readClipboardText(step);
      ...
      case "set_window_frame": {
        const windowId = yield* readWindowIdArg(step);
        if (!windowId)
          return yield* invalidArgument('Step "set_window_frame" requires "window_id".');
        const frame = {
          x: yield* readDelta(step, "x"),
          y: yield* readDelta(step, "y"),
          width: yield* readDelta(step, "width"),
          height: yield* readDelta(step, "height"),
        };
      ...
      case "invoke_menu": {
        const target = yield* readMenuTargetArg(step);
        const path = yield* readMenuPathArg(step, 'Step "invoke_menu"');
        return Effect.flatMap(foregroundAuthorization(context), (authorization) =>
          manager.invokeMenu(threadId, target, path, authorization),
        );
      }
      case "kill_app": {
        const windowId = yield* readWindowIdArg(step);
        if (!windowId) return yield* invalidArgument('Step "kill_app" requires "window_id".');
        return manager.killApp(threadId, windowId);
      }
      case "set_window_minimized": {
        const windowId = yield* readWindowIdArg(step);
        if (!windowId) {
          return yield* invalidArgument('Step "set_window_minimized" requires "window_id".');
        }
        const minimized = yield* readBooleanArg(step, "minimized");
        if (minimized === undefined) {
          return yield* invalidArgument(
            'Step "set_window_minimized" requires a boolean "minimized".',
          );
        }
        return manager.setWindowMinimized(threadId, windowId, minimized);
      }
      case "set_app_visibility": {
        const pid = yield* readNumberArg(step, "pid");
        if (pid === undefined || !Number.isSafeInteger(pid) || pid <= 0) {
          return yield* invalidArgument(
            'Step "set_app_visibility" requires a positive integer "pid".',
          );
        }
        const hidden = yield* readBooleanArg(step, "hidden");
        if (hidden === undefined) {
          return yield* invalidArgument('Step "set_app_visibility" requires a boolean "hidden".');
        }
      ...
      case "get_state": {
        const windowId = yield* readWindowIdArg(step);
        const labelContains =
          (yield* readVerbatimStringArg(step, "label_contains")) ??
          (yield* readVerbatimStringArg(step, "labelContains"));
      ...
      case "verify_state": {
        const windowId = yield* readWindowIdArg(step);
        if (!windowId) return yield* invalidArgument('Step "verify_state" requires "window_id".');
      ...
      default:
        return yield* invalidArgument(`Unknown run step type ${JSON.stringify(type)}.`);
    }
  });
   ...
   */
  const readStepElementCondition = Effect.fnUntraced(function* (
    step: Record<string, unknown>,
    name: string,
    threadId: string,
  ): Effect.fn.Return<ComputerTarget | undefined, ToolInputError | ComputerTargetError> {
    const value = yield* readRecordArg(step, name);
    if (value === undefined) return undefined;
    const target = yield* resolveTarget(yield* readScreenshotTarget(value), threadId);
    if (target.label === undefined) {
      return yield* invalidArgument(`"${name}" needs a label, or a ref whose element has one.`);
    }
    return target;
  });
   ...
   */
  const elementConditionPresent = Effect.fnUntraced(function* (target: ComputerTarget) {
    ...
    return yield* resolveComputerSemanticTarget(state.root, target).pipe(
      Effect.as(true),
      Effect.catch((error) => Effect.succeed(error.code === "computer_target_ambiguous")),
    );
  });
   ...
   */
  const runStepError = (error: ComputerToolFailure): Record<string, unknown> => {
    const backendError = isComputerBackendError(error) ? error : undefined;
    const cuaError = isCuaActionError(error) ? error : undefined;
    return backendError?.inputPause
      ? {
          code: "computer_input_paused",
          ...backendError.inputPause,
          layer: cuaError ? cuaError.layer : "server-manager",
          ...(cuaError ? { effect: cuaError.effect } : {}),
          requery_hint: INPUT_PAUSE_REQUERY_HINT,
          ...(cuaError ? { cause: cuaError.code, diagnostics: cuaError.diagnostics } : {}),
          ...(cuaError && cuaError.waitSeconds !== undefined
            ? { wait_seconds: cuaError.waitSeconds }
            : {}),
        }
      : cuaError
        ? {
            code: cuaError.code,
            effect: cuaError.effect,
            message: cuaError.message,
            retryAllowed: false,
            ...(cuaError.diagnostics ? { diagnostics: cuaError.diagnostics } : {}),
            ...(cuaError.layer ? { layer: cuaError.layer } : {}),
          }
        : isComputerTargetError(error)
          ? {
              code: error.code,
          ...
          : isComputerLeaseError(error)
            ...
            : isToolInputError(error)
              ? { code: "invalid_step", message: error.message }
              : {
                  code: "step_failed",
                  message: errorText(error),
                  ...(backendError?.retryable ? { retryable: true } : {}),
                };
  };

  const runComputerBatch = Effect.fnUntraced(function* (
    args: Record<string, unknown>,
    context: ToolContext,
  ): Effect.fn.Return<unknown, ComputerToolFailure> {
    const threadId = context.callerThreadId;
    const rawSteps = args.steps;
    if (!Array.isArray(rawSteps) || rawSteps.length === 0) {
      return yield* invalidArgument('"steps" must be a nonempty array of step objects.');
    }
    ...
    const prepared: PreparedRunStep[] = [];
    for (const [index, entry] of rawSteps.entries()) {
      if (typeof entry !== "object" || entry === null || Array.isArray(entry)) {
        return yield* invalidArgument(`Step ${index} must be an object with a "type" field.`);
      }
      const step = entry as Record<string, unknown>;
      const type = yield* readStringArg(step, "type");
      ...
      prepared.push({
        type,
        step,
        ifElement: yield* readStepElementCondition(step, "if_element", threadId),
        unlessElement: yield* readStepElementCondition(step, "unless_element", threadId),
        continueOnError: (yield* readBooleanArg(step, "continue_on_error")) === true,
        run: yield* prepareRunStep(type, step, context),
      });
    }
    ...
    for (const [index, preparedStep] of prepared.entries()) {
      ...
      if (
        ...
        !(yield* elementConditionPresent(preparedStep.ifElement))
      ) {
        skippedReason = "if_element_absent";
      } else if (
        preparedStep.unlessElement !== undefined &&
        (yield* elementConditionPresent(preparedStep.unlessElement))
      ) {
        skippedReason = "unless_element_present";
      }
      ...
      if (Result.isFailure(outcome)) {
        ...
        steps.push({
          ...
          type: preparedStep.type,
          ok: false,
          error: runStepError(outcome.failure),
        });
    ...
    const stateFields: Record<string, unknown> = yield* Effect.gen(function* () {
      yield* assertDesktopOperationActive;
      const state = yield* manager.getState({
        includeTree: true,
        includeText: false,
      ...
      Effect.catch((error) =>
        assertDesktopOperationActive.pipe(Effect.as({ stateError: errorText(error) })),
      ),
    );
    ...
    if ((yield* readBooleanArg(args, "include_screenshot")) !== true) return payload;
    return yield* Effect.gen(function* () {
      const screenshot =
        lastWindowId === undefined
          ? (yield* manager.captureFocusedWindow(COMPUTER_ACTION_OBSERVATION_MAX_DIMENSION))
              .screenshot
      ...
      return yield* deliverScreenshot(threadId, payload, screenshot, lastWindowId);
    }).pipe(
      Effect.catch((error) =>
        assertDesktopOperationActive.pipe(
          Effect.as({ ...payload, screenshotError: errorText(error) }),
        ),
      ),
    );
  });
  ...
  const entries: ToolEntry[] = [
    ...
    }),
    {
      ...
      handler: handle(
        ...
        Effect.fnUntraced(function* (args) {
          const app = (yield* readStringArg(args, "app"))?.toLocaleLowerCase();
          const result = yield* manager.listWindows();
          return app
            ? {
                ...result,
    ...
    },
    {
      ...
      handler: handle(
        ...
        Effect.fnUntraced(function* (args, context) {
          ...
          const wantText = (yield* readBooleanArg(args, "include_text")) ?? false;
          const windowId = yield* readWindowIdArg(args);
          const labelContains =
            (yield* readVerbatimStringArg(args, "label_contains")) ??
            (yield* readVerbatimStringArg(args, "labelContains"));
          const wantDiff = (yield* readBooleanArg(args, "diff")) ?? false;
          const state = yield* manager.getState({
            includeScreenshot: (yield* readBooleanArg(args, "include_screenshot")) ?? false,
          ...
          return yield* deliverScreenshot(context.callerThreadId, payload, screenshot);
        }),
      ),
    },
    {
      ...
      handler: handle(
        ...
        Effect.fnUntraced(function* (args, context) {
          const threadId = context.callerThreadId;
          const request = yield* readCaptureRequest(args, (region) =>
            readScreenshotIdArg(args).pipe(
              Effect.flatMap((screenshotId) => frames.resolve(threadId, screenshotId)),
              Effect.flatMap((frame) => screenshotRectToDesktop(frame, region)),
            ),
          );
          if (request.kind === "focused") {
            const capture = yield* manager.captureFocusedWindow(request.maxDimension);
            return yield* deliverScreenshot(
              threadId,
              { computerId: manager.computerId },
              capture.screenshot,
              capture.windowId,
          ...
          return yield* capturedScreenshotResult(
            threadId,
            request,
            yield* manager.captureScreenshot(request),
          );
        }),
      ),
    },
    {
      ...
      handler: handle("computer_get_screen_size", () => manager.getScreenSize()),
    },
    {
      requiredCapability: COMPUTER_CONTROL_CAPABILITY,
      requiresActiveTurn: true,
      ...
      handler: handle(
        "computer_wait",
        Effect.fnUntraced(function* (args, context) {
          const durationMs = yield* readWaitDurationMs(args);
          if (args.label !== undefined && (yield* readBooleanArg(args, "settle")) === true)
            return yield* invalidArgument(
              "A settle wait cannot combine with label; pick one observation mode.",
            );
          if (args.label !== undefined) {
            const target = yield* readTarget(args, context);
            if (!target.windowId || !target.label?.trim()) {
              return yield* invalidArgument(
                "A conditional wait requires a nonempty label and window_id.",
              );
            }
            const windowId = target.windowId;
            const readiness = yield* waitForControl(
              manager.withAgentActivity(
                context.callerThreadId,
                context
                  .assertCallerTurnActive()
                  .pipe(Effect.andThen(manager.getState({ includeTree: true, windowId }))),
                undefined,
                context.callerTurnId ?? undefined,
              ),
              target,
            ...
            if (
              (yield* readBooleanArg(args, "include_screenshot")) === false ||
              readiness.status === "closed"
            ) {
              return result;
            }
            const screenshot = yield* manager.withAgentActivity(
              context.callerThreadId,
            ...
            return yield* deliverScreenshot(context.callerThreadId, result, screenshot, windowId);
          }
          if ((yield* readBooleanArg(args, "settle")) === true) {
            const windowId = (yield* readTarget(args, context)).windowId;
            if (!windowId) {
              return yield* invalidArgument("A settle wait requires window_id.");
            }
            const verdict = yield* manager.withAgentActivity(
              context.callerThreadId,
              context
                .assertCallerTurnActive()
                .pipe(Effect.andThen(manager.waitForSettle(windowId, durationMs))),
              undefined,
              context.callerTurnId ?? undefined,
            );
            return { computerId: manager.computerId, ...verdict };
          }
          if (durationMs > 0) yield* Effect.sleep(durationMs);
    ...
    },
    {
      ...
      handler: handle("computer_read_clipboard", (_args, context) =>
    ...
    actionEntry(
      ...
      Effect.fnUntraced(function* (args, context) {
        const hidden = yield* readBooleanArg(args, "hidden");
        ...
        const result = yield* manager.launchApp(
          context.callerThreadId,
          yield* readStringArg(args, "app", { required: true }),
          (yield* readStringArrayArg(args, "arguments")) ?? [],
          (yield* readBooleanArg(args, "wait_for_window")) === false ? 0 : 2_000,
          hidden !== undefined ? { hidden } : undefined,
        );
    ...
    ),
    {
      ...
      handler: handle("computer_list_apps", () => manager.listApps()),
    },
    {
      requiredCapability: COMPUTER_CONTROL_CAPABILITY,
      requiresActiveTurn: true,
      ...
      handler: handle(
        ...
        Effect.fnUntraced(function* (args) {
          const windowId = yield* readWindowIdArg(args);
          if (!windowId) return yield* invalidArgument("window_id is required.");
          const raw = args.expect;
          if (!Array.isArray(raw) || raw.length === 0 || raw.length > 8)
            return yield* invalidArgument("expect must be an array of one to eight predicates.");
          for (const predicate of raw)
            if (!predicate || typeof predicate !== "object" || Array.isArray(predicate))
              return yield* invalidArgument("Each expect predicate must be an object.");
          return yield* manager.verifyState(windowId, raw as Record<string, unknown>[]);
        }),
      ),
    },
    {
      ...
      handler: handle(
        ...
        Effect.fnUntraced(function* (args) {
          ...
          if (!windowId) return yield* invalidArgument("window_id is required.");
          const region = {
            x: yield* readDelta(args, "x"),
            y: yield* readDelta(args, "y"),
            width: yield* readDelta(args, "width"),
            height: yield* readDelta(args, "height"),
          };
          if (
            ...
            return yield* invalidArgument("x/y/width/height must be finite, with positive size.");
          const zoom = yield* manager.zoomWindow(windowId, region);
    ...
    },
    {
      ...
      handler: handle("computer_get_accessibility_tree", (args) =>
        Effect.flatMap(readWindowIdArg(args), (windowId) => manager.getAccessibilityTree(windowId)),
      ),
    },
    {
      requiredCapability: COMPUTER_CONTROL_CAPABILITY,
      ...
      handler: handle("computer_get_cursor_position", (args) =>
        Effect.flatMap(readWindowIdArg(args), (windowId) => manager.getCursorPosition(windowId)),
      ),
    },
    {
      requiredCapability: COMPUTER_CONTROL_CAPABILITY,
      ...
      handler: (args, context) =>
        Effect.gen(function* () {
          if (Object.keys(args).some((key) => key !== "tool" && key !== "arguments")) {
            return yield* invalidArgument('computer_inspect accepts only "tool" and "arguments".');
          }
          const name = args.tool;
          if (typeof name !== "string" || !isInspectionToolName(name)) {
            return yield* invalidArgument("Unknown Computer inspection tool.");
          }
          const entry = entries.find((candidate) => candidate.definition.name === name)!;
          const toolArgs = yield* validateInspectionArguments(
          ...
          return yield* entry.handler(toolArgs, context);
    ...
    },
    {
      ...
      handler: handle(
        "computer_help",
        Effect.fnUntraced(function* (args) {
          const toolName = yield* readStringArg(args, "tool");
          const topic = yield* readStringArg(args, "topic");
          if (toolName !== undefined) {
            if (topic !== undefined) {
              return yield* invalidArgument("Use either tool or topic, not both.");
            }
    ...
    discoveryOnly(
      actionEntry(
        ...
        Effect.fnUntraced(function* (args, context) {
          ...
          if (!Object.values(frame).every(Number.isFinite) || frame.width <= 0 || frame.height <= 0)
            return yield* invalidArgument("x/y/width/height must be finite, with positive size.");
          return yield* manager.setWindowFrame(context.callerThreadId, windowId, frame);
        }),
      ),
    ),
    discoveryOnly(
      actionEntry(
        ...
        Effect.fnUntraced(function* (args, context) {
          const target = yield* readMenuTargetArg(args);
          const path = yield* readMenuPathArg(args, "computer_invoke_menu");
          return yield* withActionElementChanges(
            yield* manager.invokeMenu(
              context.callerThreadId,
              target,
              path,
              yield* foregroundAuthorization(context),
    ...
    discoveryOnly(
      actionEntry(
        ...
        Effect.fnUntraced(function* (args, context) {
          const windowId = yield* readWindowIdArg(args);
          if (!windowId) return yield* invalidArgument("window_id is required.");
          return yield* manager.killApp(context.callerThreadId, windowId);
        }),
      ),
    ),
    {
      ...
      handler: handle(
        "computer_set_window_minimized",
        Effect.fnUntraced(function* (args, context) {
          if ((yield* readStringArg(args, "delivery_mode")) === "foreground")
            return yield* invalidArgument(
              "computer_set_window_minimized never activates; it takes no delivery_mode.",
            );
          const windowId = yield* readWindowIdArg(args);
          if (!windowId) return yield* invalidArgument("window_id is required.");
          const minimized = yield* readBooleanArg(args, "minimized");
          if (minimized === undefined)
            return yield* invalidArgument("minimized is required and must be a boolean.");
          return yield* manager.setWindowMinimized(context.callerThreadId, windowId, minimized);
        }),
      ),
    },
    {
      ...
      handler: handle(
        ...
        Effect.fnUntraced(function* (args, context) {
          ...
          const pid = yield* readNumberArg(args, "pid");
          if (pid === undefined || !Number.isSafeInteger(pid) || pid <= 0)
            return yield* invalidArgument("pid is required and must be a positive integer.");
          const hidden = yield* readBooleanArg(args, "hidden");
          if (hidden === undefined)
            return yield* invalidArgument("hidden is required and must be a boolean.");
          return yield* manager.setAppVisibility(context.callerThreadId, pid, hidden);
        }),
      ),
    },
    observedActionEntry(
      ...
      clickSchema,
      Effect.fnUntraced(function* (args, context) {
        return yield* manager.click(
          context.callerThreadId,
          yield* readTarget(args, context),
          yield* readModifiers(args),
          yield* readClickGesture(args),
        );
      }),
    ),
    discoveryOnly(
      observedActionEntry(
        ...
        targetSchema,
        (args, context) =>
          Effect.flatMap(readTarget(args, context), (target) =>
            manager.moveCursor(context.callerThreadId, target),
          ),
    ...
    discoveryOnly(
      observedActionEntry(
        ...
        Effect.fnUntraced(function* (args, context) {
          return yield* manager.drag(
            context.callerThreadId,
            yield* readNestedTarget(args, "from", context),
            yield* readNestedTarget(args, "to", context),
            yield* readDragDurationMs(args),
          );
        }),
      ),
    ),
    observedActionEntry(
      ...
      Effect.fnUntraced(function* (args, context) {
        const threadId = context.callerThreadId;
        const raw = yield* readScreenshotTarget(args);
        const frame = yield* frames.resolve(threadId, raw.screenshotId);
        const resolved = yield* resolveTarget(raw, threadId);
        const target =
          !hasTargetFields(resolved) && frame.windowId !== undefined
        ...
        const requestedDelta = yield* readScrollDelta(args);
        const delta = screenshotDeltaToDesktop(frame, requestedDelta.deltaX, requestedDelta.deltaY);
        ...
        const modifiers = yield* readModifiers(args);
        ...
        const outcome = yield* manager.scrollCalibrated(
          threadId,
          hasTargetFields(target) ? target : null,
          limited.deltaX,
          limited.deltaY,
          {
            observe: (yield* readBooleanArg(args, "include_screenshot")) !== false,
    ...
    observedActionEntry(
      ...
      Effect.fnUntraced(function* (args, context) {
        const target = yield* readTarget(args, context);
        return yield* target.label !== undefined || target.role !== undefined
          ? manager.typeTextAt(context.callerThreadId, yield* readRequiredText(args), target)
          : manager.typeText(
              context.callerThreadId,
              yield* readRequiredText(args),
              target.windowId,
            );
      }),
    ),
    observedActionEntry(
      ...
      Effect.fnUntraced(function* (args, context) {
        const { key, chord } = yield* readKeyOrChord(args);
        const target = yield* readTarget(args, context);
        const exact = target.label !== undefined || target.role !== undefined ? target : undefined;
        return yield* chord === undefined
    ...
    discoveryOnly(
      actionEntry(
        ...
        (args, context) =>
          Effect.flatMap(readClipboardText(args), (text) =>
            manager.writeClipboard(context.callerThreadId, text),
          ),
      ),
    ),
    observedActionEntry(
      ...
      Effect.fnUntraced(function* (args, context) {
        return yield* manager.paste(
          context.callerThreadId,
          yield* readClipboardText(args),
          yield* readWindowIdArg(args),
        );
      }),
    ),
    actionEntry(
      ...
      Effect.fnUntraced(function* (args, context) {
        const windowId = yield* readWindowIdArg(args);
        if (windowId === undefined) {
          return yield* invalidArgument('Missing required argument "window_id".');
        }
        ...
        return yield* manager.foregroundWithRestore(
          context.callerThreadId,
          windowId,
          undefined,
          yield* foregroundAuthorization(context),
    ...
    observedActionEntry(
      ...
      Effect.fnUntraced(function* (args, context) {
        return yield* manager.setValue(
          context.callerThreadId,
          yield* readTarget(args, context),
          yield* readSetValueValue(args),
        );
      }),
    ),
    discoveryOnly(
      observedActionEntry(
        ...
        Effect.fnUntraced(function* (args, context) {
          return yield* manager.performAction(
            context.callerThreadId,
            yield* readTarget(args, context),
            yield* readActionName(args),
          );
        }),
      ),
    ),
    discoveryOnly(
      observedActionEntry(
        ...
        Effect.fnUntraced(function* (args, context) {
          return yield* manager.selectText(
            context.callerThreadId,
            yield* resolveTarget(yield* readSelectTextTarget(args), context.callerThreadId),
            yield* readSelectTextRange(args),
          );
        }),
      ),
    ),

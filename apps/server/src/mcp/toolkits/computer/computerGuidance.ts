 ...
 *
 * Perception and mutations share the task's computer capability. Disabled
 * sessions receive neither this block nor the Computer tool catalog.
 *
 * @module mcp/toolkits/computer/computerGuidance
 */
...
export const COMPUTER_HELP_SECTIONS = {
  ...
  spaces:
    "Read computer_help tool computer_spaces for computer_inspect fields. macOS list reads managed Spaces, including empty ones; window spaceIds alone are membership. Reserve an existing noncurrent desktop Space only after the user says: Use Space ID 42 for this task. Use its real ID, not Mission Control position. reserve can include window_id; select picks an exact existing window without activation. peek reads metadata or window text. Reservations last this turn; current-Space entry or identity/window changes block input. They coordinate Pathway tasks, not OS ownership or continuous isolation. Native create, switch, move and follow are unsupported. Launch, app-wide or visibility changes cannot target reservations. Drive the selected window through supported background routes; refusals never authorize activation. Release changes no desktop state. Linux has no managed-Space inventory.",
  linux:
...
export function computerToolInstructions(): string {
  return [
    ...
    "The computer_* tools are live on this session. Use them directly for the requested desktop or browser work. If your harness defers advertised tools, look them up by exact name; never list the whole catalog. Never substitute shell or AppleScript to get around a refusal. For Pathway's own in-app browser use browser_*.",

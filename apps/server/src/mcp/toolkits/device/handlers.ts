...
import { nodeRuntimeUnavailableMessage } from "@spiritdevs/shared/nodeRuntime";
...
const requireDeviceAccess = Effect.gen(function* () {
  const invocation = yield* McpInvocationContext.McpInvocationContext;
  const settings = yield* ServerSettings.ServerSettingsService;
  const current = yield* settings.getSettings.pipe(
    Effect.mapError(
      (cause) =>
        new DeviceToolUnavailableError({ reason: "Device settings are unavailable.", cause }),
    ),
  );
  if (
    !invocation.capabilities.has("device") ||
    !current.enableDeviceSupport ||

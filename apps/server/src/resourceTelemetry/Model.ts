...
export interface TelemetryCounters {
  ...
  readonly allPathway: GroupCounters;
}
...
export interface MergeProcessesResult {
  ...
  readonly groups: {
    ...
    readonly allPathway: ResourceTelemetryAggregate;
  };
...
function applyLifecycleCounters(input: {
  ...
  let allPathway = input.counters.allPathway;
  for (const processDelta of input.deltas) {
    ...
    allPathway = incrementCounters(allPathway, processDelta);
  }
  ...
  for (const [identityKey, current] of input.current) {
    ...
    allPathway = incrementCounters(allPathway, { processStarts: 1 });
  }
  ...
  for (const [identityKey, previous] of input.previous) {
    ...
    allPathway = incrementCounters(allPathway, { processExits: 1 });
  }

  return { backend, electron, monitor, allPathway };
}
...
export function mergeProcesses(input: MergeProcessesInput): MergeProcessesResult {
  ...
  return {
    ...
    groups: {
      ...
      allPathway: aggregate(ordered, counters.allPathway),
    },
    deltas: processDeltas,
  };
}

...
import { inferFileExtension, inferImageExtension } from "../imageMime.ts";
...
const claimsJson = Schema.fromJsonString(AttachmentUploadClaims);
const decodeClaimsOption = Schema.decodeUnknownOption(claimsJson);
const encodeClaims = Schema.encodeSync(claimsJson);

function decodeClaims(encodedPayload: string): AttachmentUploadClaims | null {
  try {
    return Option.getOrNull(decodeClaimsOption(base64UrlDecodeUtf8(encodedPayload)));
  } catch {
    return null;
  }
}
...
export const issueAttachmentUploadUrl = Effect.fn("AttachmentUpload.issueUrl")(function* (
  ...
  const extension =
    input.type === "file"
      ? inferFileExtension({ mimeType: input.mimeType, fileName: input.name })
...
export const validateAttachmentUploadToken = Effect.fn("AttachmentUpload.validateToken")(function* (
  ...
  if (!encodedPayload || !signature || unexpectedSegment) return null;
  const secret = yield* loadSigningSecret.pipe(
  ...
  return !claims || claims.expiresAt <= (yield* Clock.currentTimeMillis) ? null : claims;
});
...
export const storeAttachmentUpload = Effect.fn("AttachmentUpload.store")(function* (
  ...
  const extension =
    claims.type === "file"
      ? inferFileExtension({ mimeType: claims.mimeType, fileName: claims.name })
  ...
  return yield* Effect.gen(function* () {
    ...
    Effect.catch((cause) =>
      fileSystem.remove(partPath, { force: true }).pipe(
        ...
        Effect.andThen(Effect.logError("Failed to persist attachment upload.", { cause })),
        Effect.as({
          ok: false,
          status: 500,

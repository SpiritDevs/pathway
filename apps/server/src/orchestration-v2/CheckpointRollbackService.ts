  ...
  Effect.gen(function* () {
    ...
    const execute = Effect.fn("orchestrationV2.checkpointRollback.execute")(function* (input: {
      ...
      for (const staleScope of projection.checkpointScopes) {
        const checkpointsInScope = staleCheckpoints.filter(
          (candidate) => candidate.scopeId === staleScope.id,
        );
        if (checkpointsInScope.length > 0) {
          yield* checkpoints.deleteStaleRefs({
            scope: staleScope,
            checkpoints: checkpointsInScope,
          });
        }
      }
      ...
      for (const run of runsToRollback) {
        const runNodes = projection.nodes.filter((candidate) => candidate.runId === run.id);
        const rootNode = runNodes.find((candidate) => candidate.id === run.rootNodeId);
        events.push(
          yield* makeEvent({
            type: "run.updated",
            threadId: input.threadId,
        ...
        // Provider-native and app-owned subtrees can both outlive their root
        // node in the projection. Terminalize every node in the rolled-back
        // run so background/subagent descendants cannot remain actionable.

...
export function handleTerminalEventWithPoisonIsolation<E>(input: {
  ...
  readonly handle: (stored: OrchestrationV2StoredEvent) => Effect.Effect<void, E>;
}) {
  return input.handle(input.stored).pipe(
    Effect.catchCause((cause) =>
      Effect.logError("Skipping terminal V2 run event after handler failure", {
    ...
    // A projection-backed terminal event is immutable. Retrying it forever
    // globally wedges every later thread, so isolate the poison event after
    // recording its exact identity and let recovery/reconciliation repair its
    // derived side effects on restart.
    Effect.andThen(Ref.set(input.cursor, input.stored.sequence)),
  );
}
...
const makeOrchestrator = Effect.fn("orchestrationV2.Orchestrator.layer")(function* () {
    ...
    function* (
      ...
      yield* emitEvent({
        ...
        payload: {
          ...
          ...(command.commitSha === undefined ? {} : { commitSha: command.commitSha }),
        ...
        if (nativeActiveTurn !== undefined) {
          ...
          yield* emitEvent({
            ...
            payload: {
              ...
              role: "user",
              createdBy: command.answeredBy ?? "user",
  ...
  const dispatchMessageEditAndRestart = (
    command: Extract<OrchestrationV2Command, { readonly type: "message.edit-and-restart" }>,
    events: Ref.Ref<Array<OrchestrationV2DomainEvent>>,
    effects: Ref.Ref<Array<PendingOrchestrationEffectV2>>,
  ) =>
    Effect.gen(function* () {
      if (command.text.trim().length === 0) {
        return yield* new OrchestratorDispatchError({
          commandId: command.commandId,
          commandType: command.type,
          cause: "A restarted message cannot be empty.",
        });
      }
      const projection = yield* loadProjectionForCommand(command);
      if (projection.runs.some(isBlockingRun)) {
        return yield* new OrchestratorDispatchError({
          commandId: command.commandId,
          commandType: command.type,
          cause: "The current run must stop before its message can be edited and restarted.",
        });
      }
      if (projection.messages.some((candidate) => candidate.id === command.replacementMessageId)) {
        return yield* new OrchestratorDispatchError({
          commandId: command.commandId,
          commandType: command.type,
          cause: `Replacement message ${command.replacementMessageId} already exists.`,
        });
      }
      const latestUserMessage = projection.messages.findLast((candidate) => {
        if (candidate.role !== "user") return false;
        const run = projection.runs.find((candidateRun) => candidateRun.id === candidate.runId);
        return run?.status !== "rolled_back" && run?.status !== "cancelled";
      });
      if (
        latestUserMessage?.id !== command.messageId ||
        command.createdBy !== "user" ||
        latestUserMessage.createdBy !== "user" ||
        latestUserMessage.runId === null
      ) {
        return yield* new OrchestratorDispatchError({
          commandId: command.commandId,
          commandType: command.type,
          cause: "Only the latest user-authored message can be edited and restarted.",
        });
      }
      const targetRun = projection.runs.find(
        (candidate) => candidate.id === latestUserMessage.runId,
      );
      if (
        targetRun === undefined ||
        ...
        return yield* new OrchestratorDispatchError({
          commandId: command.commandId,
          commandType: command.type,
          cause: `Run for message ${command.messageId} has not stopped.`,
        });
      }
      if (!modelSelectionsEqual(projection.thread.modelSelection, targetRun.modelSelection)) {
        return yield* new OrchestratorDispatchError({
          commandId: command.commandId,
          commandType: command.type,
          cause: "Restore the message's model selection before editing and restarting it.",
        });
      }
      const changedFilesAfterMessage = projection.checkpoints.some(
        (checkpoint) =>
          checkpoint.status === "ready" &&
          checkpoint.files.length > 0 &&
          checkpoint.appRunOrdinal !== null &&
          checkpoint.appRunOrdinal >= targetRun.ordinal,
      );
      if (changedFilesAfterMessage) {
        return yield* new OrchestratorDispatchError({
          commandId: command.commandId,
          commandType: command.type,
          cause: "This message cannot be edited because its run changed files.",
        });
      }
      ...
      const firstRunRootScope =
        targetRun.ordinal === 1
          ? projection.checkpointScopes.find(
              (scope) => scope.runId === targetRun.id && scope.kind === "root_run",
            )
          : undefined;
      const baselineCheckpoint = projection.checkpoints.findLast(
        (checkpoint) =>
          checkpoint.status === "ready" &&
          (targetRun.ordinal === 1
            ? firstRunRootScope !== undefined &&
              checkpoint.scopeId === firstRunRootScope.id &&
              checkpoint.ordinalWithinScope === 0 &&
              checkpoint.appRunOrdinal === null
            : checkpoint.appRunOrdinal === targetRun.ordinal - 1),
      );
      if (baselineCheckpoint === undefined) {
        return yield* new OrchestratorDispatchError({
          commandId: command.commandId,
          commandType: command.type,
          cause: `No rollback checkpoint exists before message ${command.messageId}.`,
        });
      }

      yield* dispatchCheckpointRollback(
        {
          type: "checkpoint.rollback",
          commandId: command.commandId,
          threadId: command.threadId,
          scopeId: baselineCheckpoint.scopeId,
      ...
      const replacementRun = plannedEvents.find(
        (event): event is Extract<OrchestrationV2DomainEvent, { readonly type: "run.created" }> =>
          event.type === "run.created" &&
          event.threadId === command.threadId &&
          event.payload.userMessageId === command.replacementMessageId,
      );
      const plannedEffects = yield* Ref.get(effects);
      const rollbackEffect = plannedEffects.find(
        (effect) => effect.request.type === "provider-thread.rollback",
      );
      const startEffect = plannedEffects.find(
        (effect) =>
          effect.request.type === "provider-turn.start" &&
          effect.request.runId === replacementRun?.payload.id,
      );
      if (
        replacementRun?.type !== "run.created" ||
        ...
        return yield* new OrchestratorDispatchError({
          commandId: command.commandId,
          commandType: command.type,
          cause: "The edit-and-restart command did not produce a rollback and replacement start.",
        });
      }
      yield* Ref.set(effects, [
        ...plannedEffects.filter(
          (effect) => effect.id !== rollbackEffect.id && effect.id !== startEffect.id,
        ),
        {
          id: `effect:${command.commandId}:provider-thread.rollback-and-start:${replacementRun.payload.id}`,
          commandId: command.commandId,
          threadId: command.threadId,
          request: {
            type: "provider-thread.rollback-and-start",
            providerThreadId: rollbackEffect.request.providerThreadId,
            checkpointId: rollbackEffect.request.checkpointId,
            scopeId: rollbackEffect.request.scopeId,
            runId: replacementRun.payload.id,
          },
        } satisfies PendingOrchestrationEffectV2,
      ]);
    });
  ...
  const terminalEventCursor = yield* Ref.make(terminalEventsAfterSequence);
  const consumeTerminalEvents = Ref.get(terminalEventCursor).pipe(
      ...
      eventSink.stream({ afterSequence }).pipe(
        ...
        Stream.runForEach((stored) =>
          handleTerminalEventWithPoisonIsolation({
            ...
            handle: handleTerminalRun,
          }),
        ),
      ),
    ),
    Effect.catchCause((cause) =>
      Ref.get(terminalEventCursor).pipe(
        Effect.flatMap((afterSequence) =>
          Effect.logWarning("Terminal V2 run subscriber failed; restarting", {
            afterSequence,
            cause,
          }),
        ),
    ...
    // Also back off if a stream ends cleanly; an unexpected closed stream
    // must not turn the supervisor into a hot loop.
    Effect.andThen(Effect.sleep("250 millis")),
  );
  yield* consumeTerminalEvents.pipe(Effect.forever, Effect.forkDetach);

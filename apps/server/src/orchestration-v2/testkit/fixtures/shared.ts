  ...
  return Effect.gen(function* () {
    ...
    for (const [stepIndex, step] of input.fixtureInput.steps.entries()) {
      switch (step.type) {
        ...
        case "rollback":
          {
            const checkpointRunOrdinal = Number(step.checkpointSuffix);
            const scopeName =
              step.checkpointScopeSuffix === "root" &&
              Number.isSafeInteger(checkpointRunOrdinal) &&
              checkpointRunOrdinal > 0
                ? `root:${runIdFor(checkpointRunOrdinal)}`
                : step.checkpointScopeSuffix;

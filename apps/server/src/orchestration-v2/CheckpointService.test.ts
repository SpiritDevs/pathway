...
it.effect("allocates a distinct checkpoint scope for every root run", () => {
  ...
  return Effect.gen(function* () {
    ...
    const shared = {
      threadId: ThreadId.make("thread:root-scopes"),
      providerThreadId: ProviderThreadId.make("provider-thread:root-scopes"),
      cwd: "/repo",
      createdAt: DateTime.makeUnsafe("2026-08-12T00:00:00.000Z"),
    };
    const first = yield* checkpoints.prepareRootRunScope({
      ...shared,
      runId: RunId.make("run:root-scopes:1"),
      rootNodeId: NodeId.make("node:root-scopes:1"),
    });
    const second = yield* checkpoints.prepareRootRunScope({
      ...shared,
      runId: RunId.make("run:root-scopes:2"),
      rootNodeId: NodeId.make("node:root-scopes:2"),
    });

    assert.notEqual(first.id, second.id);
    assert.equal(first.runId, "run:root-scopes:1");
    assert.equal(second.runId, "run:root-scopes:2");

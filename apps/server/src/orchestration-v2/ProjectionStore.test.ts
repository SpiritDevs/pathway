...
it.layer(TestLayer)("ProjectionStoreV2", (it) => {
  ...
  it.effect("does not inherit runless items into fork shells", () =>
    Effect.gen(function* () {
      const projectionStore = yield* ProjectionStoreV2;
      const now = yield* DateTime.now;
      ...
      assert.equal(targetShell.visibleItemCount, 2);
      assert.equal(targetProjection.visibleTurnItems.length, 2);
    }),
  );

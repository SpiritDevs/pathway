...
- Codex same-provider fork is lazy: `thread.fork` records lineage and pending transfer, and first
  dispatch resolves native Codex fork. Earlier source-point forks pass the inclusive native
  `lastTurnId` boundary directly to `thread/fork`.
...
- Custom pathway tools/mcp_server that lets agents spawn subagents of other providers powered by the Pathway Orchestrator

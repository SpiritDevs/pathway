...
function makeRootRunScope(input: {
  ...
  return Effect.gen(function* () {
    const scopeId = yield* input.idAllocator.allocate.checkpointScope({
      ...
      // A checkpoint scope is owned by one root run. Including the run id
      // prevents later runs from reusing the same deterministic scope id and
      // overwriting its run/node metadata in the projection.
      name: `${ROOT_CHECKPOINT_SCOPE_NAME}:${input.runId}`,
    });
    return {
      id: scopeId,
      threadId: input.threadId,
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const isGitCheckpointable = (scope: OrchestrationV2CheckpointScope) =>
      checkpointStore
        .isGitRepository(scope.cwd, scope.repositoryRootOnly)
    ...
    const captureBaseline: CheckpointServiceV2Shape["captureBaseline"] = (input) =>
      withWorkspaceLock(
        input.scope.cwd,
        Effect.gen(function* () {
          if (!(yield* isGitCheckpointable(input.scope))) {
            return;
          }
      ...
      (input) =>
        withWorkspaceLock(
          ...
          Effect.gen(function* () {
            ...
            const checkpointable = yield* isGitCheckpointable(input.scope);

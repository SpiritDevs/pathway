...
> {
  return Effect.gen(function* () {
    const remoteDir = yield* makeTempDir("pathway-git-remote-");
...
function createGitHubCliWithFakeGh(scenario: FakeGhScenario = {}): {
  ...
  return {
    service: {
      ...
      listRepositoryOwners: () => Effect.succeed([]),
...
it.layer(GitManagerTestLayer)("GitManager", (it) => {
  ...
  it.effect("status returns an explicit non-repo result for non-git directories", () =>
    Effect.gen(function* () {
      const cwd = yield* makeTempDir("pathway-git-manager-non-repo-");
      const { manager } = yield* makeManager();
  ...
  it.effect("status returns an explicit non-repo result for deleted directories", () =>
    Effect.gen(function* () {
      const rootDir = yield* makeTempDir("pathway-git-manager-missing-dir-");
            ...
            JSON.stringify([
              {
                ...
                url: "https://github.com/coreybain/pathway/pull/214",
                baseRefName: "main",
              ...
              JSON.stringify([
                {
                  ...
                  url: "https://github.com/coreybain/pathway/pull/1661",
                  baseRefName: "main",
                  headRefName: "main",
                  state: "OPEN",
                  updatedAt: "2026-04-01T15:00:00Z",
                  ...
                  headRepository: {
                    nameWithOwner: "lnieuwenhuis/pathway",
                  },
                  headRepositoryOwner: {
                    login: "lnieuwenhuis",
                  },
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* runGit(repoDir, ["checkout", "-b", "pathway/pr-488/statemachine"]);
        ...
        const status = yield* manager.status({ cwd: repoDir });
        expect(status.refName).toBe("pathway/pr-488/statemachine");
        expect(status.pr).toEqual({
          number: 488,
          title: "Rebase this PR on latest main",
          url: "https://github.com/pingdotgg/codething-mvp/pull/488",
              ...
              "effect-atom": JSON.stringify([
                {
                  number: 1618,
                  title: "Correct PR",
                  url: "https://github.com/coreybain/pathway/pull/1618",
                  baseRefName: "main",
              ...
              "upstream/effect-atom": JSON.stringify([
                {
                  number: 1518,
                  title: "Wrong PR",
                  url: "https://github.com/coreybain/pathway/pull/1518",
                  baseRefName: "main",
  ...
  it.effect("status finds a PR after a base-tracking branch is published explicitly", () =>
    Effect.gen(function* () {
      const repoDir = yield* makeTempDir("pathway-git-manager-");
      yield* initRepo(repoDir);
      const remoteDir = yield* createBareRemote();
      ...
      yield* runGit(repoDir, ["remote", "set-head", "origin", "main"]);
      yield* runGit(repoDir, ["checkout", "-b", "feature/from-main", "origin/main"]);
      yield* runGit(repoDir, ["push", "origin", "HEAD:refs/heads/feature/from-main"]);
      yield* configureVisibleRemoteUrlWithLocalRewrite(
        repoDir,
        "origin",
        "https://github.com/pingdotgg/codething-mvp.git",
      ...
      const { manager, ghCalls } = yield* makeManager({
        ghScenario: {
          prListByHeadSelector: {
            // @effect-diagnostics-next-line preferSchemaOverJson:off
            "feature/from-main": JSON.stringify([
              {
                number: 55,
      ...
      const status = yield* manager.status({ cwd: repoDir });
      expect(status.pr?.number).toBe(55);
      expect(ghCalls.some((call) => call.includes("pr list --head feature/from-main"))).toBe(true);
    }),
  );

  it.effect("status still queries an unresolved fork tracking its default branch", () =>
    Effect.gen(function* () {
      const repoDir = yield* makeTempDir("pathway-git-manager-");
      yield* initRepo(repoDir);
      const baseDir = yield* createBareRemote();
      const forkDir = yield* createBareRemote();
      yield* runGit(repoDir, ["remote", "add", "origin", baseDir]);
      yield* runGit(repoDir, ["push", "-u", "origin", "main"]);
      yield* runGit(repoDir, ["remote", "set-head", "origin", "main"]);
      yield* runGit(repoDir, ["remote", "add", "fork", forkDir]);
      yield* runGit(repoDir, ["push", "fork", "main"]);
      yield* runGit(repoDir, ["checkout", "-b", "review", "fork/main"]);
      yield* configureVisibleRemoteUrlWithLocalRewrite(
        repoDir,
        "origin",
      ...
      yield* configureVisibleRemoteUrlWithLocalRewrite(
        repoDir,
        "fork",
        "https://gitlab.example.com/contributor/project.git",
        forkDir,
      );
      ...
      const { manager, ghCalls } = yield* makeManager({
        ghScenario: {
          prListByHeadSelector: {
            // Non-GitHub providers do not necessarily expose repository-owner
            // metadata alongside the change request.
            // @effect-diagnostics-next-line preferSchemaOverJson:off
            "fork:main": JSON.stringify([
              {
                number: 56,
                title: "Fork default branch",
                url: "https://gitlab.example.com/team/project/-/merge_requests/56",
                baseRefName: "main",
                headRefName: "main",
                state: "OPEN",
                updatedAt: "2026-08-28T10:00:00Z",
      ...
      const status = yield* manager.status({ cwd: repoDir });
      expect(status.pr?.number).toBe(56);
      expect(ghCalls.some((call) => call.includes("pr list --head fork:main"))).toBe(true);
    }),
  );
      ...
      Effect.gen(function* () {
        ...
        const { manager, ghCalls } = yield* makeManager({
          ghScenario: {
            prListByHeadSelector: {
              // @effect-diagnostics-next-line preferSchemaOverJson:off
              "pathway/pr-142/statemachine": JSON.stringify([]),
              // @effect-diagnostics-next-line preferSchemaOverJson:off
  ...
  it.effect("accepts fork PR metadata when origin is the fork checkout remote", () =>
    Effect.sync(() => {
      const headContext = {
        headBranch: "pathway/git-audit-stability",
        headRepositoryNameWithOwner: "justsomelegs/pathway",
        ...
        GitManager.matchesBranchHeadContext(
          {
            ...
            url: "https://github.com/coreybain/pathway/pull/2284",
            baseRefName: "main",
            headRefName: "pathway/git-audit-stability",
            state: "open",
            updatedAt: Option.none(),
            isCrossRepository: true,
            headRepositoryNameWithOwner: "justsomelegs/pathway",
  ...
  it.effect("generates PR content against the remote base when the local base is stale", () =>
    Effect.gen(function* () {
      ...
      const peerDir = yield* makeTempDir("pathway-git-peer-");
  ...
  it.effect("preserves both branch materialization failures when the fallback also fails", () =>
    Effect.gen(function* () {
      ...
      expect(error).toMatchObject({
        ...
        localBranch: "pathway/pr-93/feature/missing-fork-branch",
      });
      ...
      const { manager } = yield* makeManager({
        ghScenario: {
          pullRequest: {
            ...
            url: "https://github.com/coreybain/pathway/pull/642",
            baseRefName: "main",
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(result.branch).toBe("pathway/pr-91/main");
        expect(result.worktreePath).not.toBeNull();
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(result.branch).toBe("pathway/pr-92/main");
        expect((yield* runGit(repoDir, ["rev-parse", "main"])).stdout.trim()).toBe(localMainBefore);
        expect(
          (yield* runGit(result.worktreePath as string, [
            "rev-parse",

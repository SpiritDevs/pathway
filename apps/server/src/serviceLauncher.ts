...
// `pathway service update`. Keep runtime imports limited to Node built-ins.
...
export class Launcher {
  ...
  async #startChild(version: string, role: ChildRole, update?: ServiceUpdateRecord): Promise<void> {
    ...
    if (!(await runtimeExists(this.#baseDir, version))) {
      throw new Error(`Selected @spiritdevs/pathway@${version} runtime is missing or incomplete.`);
    }
    if (this.#stopping) return;
...
async function main(): Promise<void> {
  const baseDir = process.env.PATHWAY_HOME?.trim();
  if (baseDir === undefined || baseDir === "") {
    throw new Error("PATHWAY_HOME is required by the Pathway service launcher.");
  }

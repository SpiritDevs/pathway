import { KeybindingCommand, KeybindingRule, KeybindingsConfig } from "@spiritdevs/contracts";
...
import { KeybindingsConfigError } from "@spiritdevs/contracts";
    ...
    Layer.provideMerge(
      Layer.fresh(
        ServerConfig.layerTest(process.cwd(), {
          prefix: "pathway-keybindings-test-",
        }),
      ),
    ),
  );
...
it.layer(NodeServices.layer)("keybindings", (it) => {
  ...
  it.effect("ships configurable thread navigation defaults", () =>
    Effect.sync(() => {
      ...
      assert.equal(defaultsByCommand.get("focus.cycle"), "mod+alt+g");
      ...
      assert.equal(defaultsByCommand.get("threadBrowser.toggle"), "mod+shift+b");

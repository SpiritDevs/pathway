...
import type { DesktopUpdateState, DesktopUpdateStatusReport } from "@spiritdevs/contracts";
...
const makeHarness = Effect.fn("test.make_desktop_app_update_harness")(function* (
  ...
  const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "pathway-desktop-app-update-test-" });
  ...
  return { service, requested: Deferred.await(requestIdDeferred), canceledRequestIds };
});
...
it.layer(NodeServices.layer)("desktop app update", (it) => {
  it.effect("is unavailable without desktop mode or the control fd", () =>
    Effect.gen(function* () {
      ...
      expect((yield* noFd.service.run(() => Effect.void).pipe(Effect.flip)).reason).toContain(
        "not started by the Pathway desktop app",
      );
  ...
  it.effect(
    "reports checking, downloading, and installation preparation without duplicate stages",
    () =>
      Effect.gen(function* () {
  ...
  it.effect("maps up-to-date and failed outcomes to readable errors", () =>
    Effect.gen(function* () {
      ...
      expect((yield* upToDate.service.run(() => Effect.void).pipe(Effect.flip)).reason).toBe(
        "The Pathway desktop app on this machine is already up to date on 1.2.3.",
      );
  ...
  it.effect("cancels the prepared download when the requesting connection is interrupted", () =>
    Effect.gen(function* () {
      const { service, requested, canceledRequestIds } = yield* makeHarness({ keepOpen: true });
      const running = yield* Effect.forkChild(service.run(() => Effect.void));
      const requestId = yield* requested;
      yield* Fiber.interrupt(running);
      expect(canceledRequestIds).toEqual([requestId]);
    }),
  );

  it.effect("cancels a stalled download when its preparation deadline expires", () =>
    Effect.gen(function* () {
      const { service, requested, canceledRequestIds } = yield* makeHarness({ keepOpen: true });
      const running = yield* Effect.forkChild(service.run(() => Effect.void).pipe(Effect.flip));
      const requestId = yield* requested;
      yield* TestClock.adjust("20 minutes");
      const error = yield* Fiber.join(running);
      expect(error.reason).toBe("The desktop app did not finish the update in time.");
      expect(canceledRequestIds).toEqual([requestId]);
    }),
  );

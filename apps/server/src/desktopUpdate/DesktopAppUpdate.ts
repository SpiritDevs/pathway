...
export const make = Effect.fn("desktopUpdate.desktopAppUpdate.make")(function* () {
  ...
  const consumeReports = Effect.fn("desktopUpdate.desktopAppUpdate.consumeReports")(function* (
    ...
    if (report.outcome === "up-to-date") {
      return yield* failWith(
        `The Pathway desktop app on this machine is already up to date on ${report.state.currentVersion}.`,
      );
    }
    return yield* failWith(
  ...
  const run: DesktopAppUpdate["Service"]["run"] = Effect.fn("desktopUpdate.desktopAppUpdate.run")(
    function* (reportProgress) {
      if (!available) {
        return yield* failWith(
          "This server was not started by the Pathway desktop app, so it cannot drive a desktop update.",
        );
      }
    ...
    const terminal = yield* Effect.scoped(
      Effect.gen(function* () {
        ...
        yield* receiver
          ...
          .pipe(
            Effect.mapError((error) => failWith("Could not reach the Pathway desktop app.", error)),
          );

...
const writeProjectFile = Effect.fn("writeProjectFile")(function* (cwd: string, contents: string) {
  ...
  yield* fileSystem.writeFileString(path.join(cwd, "pathway.json"), contents).pipe(Effect.orDie);
});

it.layer(TestLayer)("PathwayProjectFileLoader", (it) => {
  describe("load", () => {
    it.effect("loads and decodes a valid pathway.json", () =>
      Effect.gen(function* () {
        const loader = yield* PathwayProjectFileLoader.PathwayProjectFileLoader;
    ...
    it.effect("returns none when pathway.json is missing", () =>
      Effect.gen(function* () {
        const loader = yield* PathwayProjectFileLoader.PathwayProjectFileLoader;
        const cwd = yield* makeTempDir;

...
import { GitCommandError, type ReviewDiffFileContentsInput } from "@spiritdevs/contracts";
  ...
  describe("remote operations", () => {
    it.effect("ensureRemote reuses an existing remote across ssh/https transport variants", () =>
      Effect.gen(function* () {
        ...
        yield* git(cwd, ["remote", "add", "origin", "https://github.com/coreybain/pathway.git"]);

        const reusedForSsh = yield* driver.ensureRemote({
          cwd,
          preferredName: "pingdotgg",
          url: "git@github.com:coreybain/pathway.git",
        });
        ...
        const reusedForSshScheme = yield* driver.ensureRemote({
          cwd,
          preferredName: "pingdotgg",
          url: "ssh://git@github.com/coreybain/pathway",
        });
        ...
        const reusedForBareSshScheme = yield* driver.ensureRemote({
          cwd,
          preferredName: "pingdotgg",
          url: "ssh://github.com/coreybain/pathway",
        });
        ...
        const reusedForSshPort = yield* driver.ensureRemote({
          cwd,
          preferredName: "pingdotgg",
          url: "ssh://git@github.com:22/coreybain/pathway",
        });
        ...
        const reusedForSshWithPort = yield* driver.ensureRemote({
          cwd,
          preferredName: "pingdotgg",
          url: "ssh://git@github.com:22/coreybain/pathway.git",
        });
        ...
        const addedForFork = yield* driver.ensureRemote({
          ...
          url: "git@github.com:octocat/pathway.git",
        });
    ...
    it.effect("creates a worktree from the latest fetched remote commit", () =>
      Effect.gen(function* () {
        ...
        yield* driver.createWorktree({
          ...
          newRefName: "pathway/fetched-origin",
        ...
        assert.equal(
          yield* driver.readConfigValue(
            worktreePath,
            "branch.pathway/fetched-origin.gh-merge-base",
          ),
          initialBranch,
        );
        assert.equal(
          yield* driver.readConfigValue(worktreePath, "branch.pathway/fetched-origin.remote"),
          null,
        );

 ...
 * shared Pathway home.
 */
import { AuthStandardClientScopes, ExecutionEnvironmentDescriptor } from "@spiritdevs/contracts";
...
/**
 * Three outcomes: a Pathway descriptor, nothing answering, or another HTTP service.
 */
  ...
  Command.withHandler((flags) =>
    Effect.gen(function* () {
      ...
      if (isLoopbackHost(new URL(pairingBaseUrl).hostname)) {
        notes.push(
          "This URL is only reachable from this machine. Use Pathway Connect for remote access, or restart the server with a reachable --host.",
        );
      }

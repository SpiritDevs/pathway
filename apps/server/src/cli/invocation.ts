...
import { HostProcessArguments } from "@spiritdevs/shared/hostProcess";
 ...
 * Detection is best-effort; callers must fail closed to a plain `pathway` command.
 */
...
/**
 * The Pathway package spec to suggest. The literal spec the user typed (e.g.
 * `@spiritdevs/pathway@nightly`) is resolved away before our process starts, so re-derive it
...
export function suggestedPackageSpec(version: string): string {
  return version.includes("-nightly.") ? "@spiritdevs/pathway@nightly" : "@spiritdevs/pathway";
}

/**
 * Render a `pathway <subcommand>` suggestion that matches how this process was
 * launched, so copy/pasting it actually works: `npx @spiritdevs/pathway connect` suggests
 * `npx @spiritdevs/pathway serve`, a global install suggests `pathway serve`, and a nightly build
...
export function formatCliCommand(input: {
  ...
  if (runner === null) {
    return `pathway ${input.subcommand}`;
  }

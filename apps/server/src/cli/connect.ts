...
const authorizeCli = Effect.fn("cloud.cli.authorize")(function* (options: {
  ...
  const existing = yield* tokens.getExisting.pipe(
    Effect.catchTag("CloudCliCredentialRefreshError", () =>
      Console.log(
        "The stored Pathway Connect credential could not be refreshed; signing in again.",
...
function formatCloudStatus(status: CloudCliStatus, options?: { readonly json?: boolean }): string {
  ...
  const nextStep = !status.authenticated
    ? "Run `pathway connect link` to authorize and enable Pathway Connect."
    : !status.desired
      ? "Run `pathway connect link` to enable Pathway Connect."
      : !status.linked
        ? "Start Pathway to provision the environment link and launch its managed tunnel."
        : undefined;
...
const logCloudDisconnectFailure = (
  ...
  Effect.logWarning("Pathway Connect disconnect operation failed.").pipe(
    Effect.annotateLogs({
      operation,
      clearAuthorization,
...
export const reportCloudDisconnectResults = Effect.fn("cloud.cli.report_disconnect_results")(
  function* (input: {
    ...
    if (input.liveResult.status === "failed") {
      ...
      yield* Console.warn(
        "Pathway Connect is disabled, but the running server could not stop its tunnel.\nRestart that server to stop the connector.",
      );
    } else {
      yield* Console.log("Pathway Connect is disabled locally.");
    }
    ...
    if (Exit.isFailure(input.relayResult)) {
      ...
      yield* Console.warn(
        input.clearAuthorization
          ...
          : "Could not revoke the relay-side environment record yet.\nRun `pathway connect unlink` again when the relay is reachable.",
      );
...
const disconnectCloud = Effect.fn("cloud.cli.disconnect")(function* (options: {
  ...
  if (options.clearAuthorization) {
    yield* Console.log(
      "Signed out of Pathway Connect locally.\nThe background service is managed separately with `pathway service`.",
    );
  }
});
...
const connectLoginCommand = Command.make("login", {
  ...
  Command.withDescription("Authorize the Pathway Connect CLI without enabling remote access."),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
        yield* Console.log("Pathway Connect\n");
...
const connectLinkCommand = Command.make("link", {
  ...
  publishOnly: Flag.boolean("publish-only").pipe(
    Flag.withDescription(
      "Link to publish agent activity only — no managed tunnel. Reach this environment through a direct endpoint you manage.",
    ),
    Flag.withDefault(false),
  ),
}).pipe(
  Command.withDescription("Authorize this environment for Pathway Connect on next start."),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
        ...
        if (linked) {
          ...
          yield* Console.log(
            flags.publishOnly
              ? `✓ Authorized${connectedAs(linked.identity)}\n\nNext\n  Start Pathway to publish agent activity (no managed tunnel).`
...
const connectStatusCommand = Command.make("status", {
  ...
  Command.withDescription("Show persisted Pathway Connect and relay client state."),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
      ...
      Effect.gen(function* () {
        ...
        if (!enabled) {
          ...
          if (!linkedNow && (yield* CliState.readCliDesiredLinkMode) === "publish_only") {
            yield* CliState.setCliDesiredCloudLink(false);
            yield* Console.log("Cancelled the pending publish-only Pathway Connect link.");
          }
        ...
        // out of band without Pathway Connect.
        if (!(yield* tokens.hasCredential)) {
          yield* Console.log(
            "Run `pathway connect login` first so this environment can be authorized to publish.",
          );
          return;
        }
        // A link may already be desired (e.g. `pathway connect link` before the
        ...
        if (yield* CliState.readCliDesiredCloudLink) {
          yield* Console.log(
            "A Pathway Connect link is already pending. Start Pathway to finish provisioning it; publishing starts once it links.",
          );
          return;
        }
        yield* CliState.setCliDesiredCloudLink(true, "publish_only");
        yield* Console.log(
          "Restart Pathway to finish authorizing this environment to publish (no managed tunnel is created).",
        );
      }),
    ),
  ),
...
const connectUnlinkCommand = Command.make("unlink", {
  ...
  Command.withDescription("Disable Pathway Connect while retaining the stored authorization."),
  Command.withHandler((flags) =>
...
const connectLogoutCommand = Command.make("logout", {
  ...
  Command.withDescription("Disable Pathway Connect and clear the stored CLI authorization."),
  Command.withHandler((flags) =>
...
export const connectCommand = Command.make("connect", {
  ...
  Command.withDescription("Set up Pathway Connect for this machine."),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
        ...
        if (background) {
          yield* Console.log(
            "\n✓ Background service ready\n\nPathway will stay reachable after you log out.",
          );
          return;
        }
        const serveCommand = yield* resolveCliCommand("serve");

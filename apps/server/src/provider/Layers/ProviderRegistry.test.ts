...
import { deepMerge } from "@spiritdevs/shared/Struct";
import { createModelCapabilities } from "@spiritdevs/shared/model";
import { applyServerSettingsPatch } from "@spiritdevs/shared/serverSettings";
...
process.env.PATHWAY_CURSOR_ENABLED = "1";
  ...
  (it) => {
    ...
    describe("ProviderRegistryLive", () => {
      ...
      it.effect("propagates real Codex probe failures to the aggregator at boot", () =>
        Effect.gen(function* () {
          const missingBinary = `pathway_codex_missing_`;
          const serverSettings = yield* makeMutableServerSettingsService(
            decodeServerSettings(
              deepMerge(encodedDefaultServerSettings, {
                ...
                providerInstances: {
                  // Matches the shape the user had in `.pathway/dev/settings.json`
      ...
      it.effect("re-probes when settings change the codex binaryPath", () =>
        Effect.gen(function* () {
          const firstMissing = `pathway_codex_first_`;
          const secondMissing = `pathway_codex_second_`;
          ...
          Effect.gen(function* () {
            ...
            yield* Effect.gen(function* () {
              ...
              assert.strictEqual(
                ...
                "Cursor is disabled in Pathway settings.",
              );
      ...
      it.effect("skips codex probes entirely when the provider is disabled", () =>
        Effect.gen(function* () {
          ...
          assert.strictEqual(status.installed, false);
          assert.strictEqual(status.message, "Codex is disabled in Pathway settings.");
        }),
      );
    });
    ...
    describe("checkClaudeProviderStatus", () => {
      ...
      it.effect("includes Claude Opus 5.5 on supported Claude Code versions", () =>
        Effect.gen(function* () {
          ...
          assert.strictEqual(opus55?.isLegacy, undefined);
      ...
      it.effect("includes Claude Fable 5.1 on supported Claude Code versions", () =>
        Effect.gen(function* () {
          ...
          assert.strictEqual(fable51?.isLegacy, undefined);
          assert.strictEqual(
            status.message,
            "Claude Code v2.1.257 is too old for Claude Opus 5.5. Upgrade to v2.1.280 or newer to access it.",
          );
        }).pipe(
          Effect.provide(
            mockSpawnerLayer((args) => {
      ...
      it.effect("runs Claude status probes with the configured CLAUDE_CONFIG_DIR", () => {
        const claudeConfigDir = "/tmp/pathway-claude-home";

...
const shouldLaunchElectronAfterPack = process.env.PATHWAY_DESKTOP_DEV === "1";
  ...
  run: {
    tasks: {
      build: {
        ...
        dependsOn: ["@spiritdevs/pathway#build"],
        cache: false,
      },
      dev: {
        command:
          "node scripts/build-preview-annotation-css.mjs && cross-env PATHWAY_DESKTOP_DEV=1 vp pack --watch",
        dependsOn: ["@spiritdevs/pathway#build"],
        cache: false,
      },
      "dev:bundle": {
  ...
  pack: [
    {
      ...
      clean: true,
      deps: {
        alwaysBundle: (id) => id.startsWith("@spiritdevs/"),
      },

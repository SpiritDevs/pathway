...
it("activates only a window of the process authenticated by D-Bus", () => {
  const target = window(42, "Pathway");
  expect(findCaptureDestination([window(99, "Pathway"), target], 42, "Pathway")).toBe(target);
  expect(findCaptureDestination([window(99, "Pathway")], 42, "Pathway")).toBeUndefined();
...
it("does not guess between ambiguous windows from the same process", () => {
  expect(
    findCaptureDestination([window(42, "Pathway"), window(42, "Pathway")], 42, "Pathway"),
  ).toBeUndefined();

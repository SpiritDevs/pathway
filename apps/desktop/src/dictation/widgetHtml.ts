...
function mountDictationOverlay() {
  const bridge = (
    window as unknown as {
      dictationOverlay: DictationBridge & { resize(width: number, height: number): void };
    }
  ).dictationOverlay;
  ...
  function render(force = false) {
    ...
    if (recentOpen) renderRecent();
      ...
      root.append(pill);
      size(hovered ? 346 : 112, hovered ? 74 : 48);
    } else size(112, 48);

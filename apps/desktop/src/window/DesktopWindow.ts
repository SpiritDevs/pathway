...
export class DesktopWindow extends Context.Service<
  DesktopWindow,
  {
    ...
    // serves the renderer is ready. It is dismissed automatically once the real
    // main window reveals.
    readonly showConnectingSplash: (input?: { readonly label?: string }) => Effect.Effect<void>;
...
// serves the renderer) cold-boots. Inlined as a data URL so it needs no bundled
...
function buildConnectingSplashDataUrl(shouldUseDarkColors: boolean, label: string): string {
  ...
  const foreground = shouldUseDarkColors ? "#9ca3af" : "#6b7280";
  const accent = shouldUseDarkColors ? "#f8fafc" : "#1f2937";
  const track = shouldUseDarkColors ? "rgba(248,250,252,0.18)" : "rgba(31,41,55,0.18)";
  const html = `<!doctype html><html><head><meta charset="utf-8"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src 'unsafe-inline'"><style>html,body{margin:0;height:100%}body{background:${background};color:${foreground};font-family:system-ui,-apple-system,'Segoe UI',sans-serif;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;-webkit-user-select:none;user-select:none;-webkit-app-region:drag}.spinner{width:26px;height:26px;border:3px solid ${track};border-top-color:${accent};border-radius:50%;animation:spin .8s linear infinite}.label{font-size:13px}@keyframes spin{to{transform:rotate(360deg)}}</style></head><body><div class="spinner"></div><div class="label">${label}</div></body></html>`;
...
export const make = Effect.gen(function* () {
  ...
  const showConnectingSplash = Effect.fn("desktop.window.showConnectingSplash")(
    function* (input?: { readonly label?: string }) {
      ...
      void splash.loadURL(
        buildConnectingSplashDataUrl(
          shouldUseDarkColors,
          input?.label ?? "Starting the local server…",
        ),
      );

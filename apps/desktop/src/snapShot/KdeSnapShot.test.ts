...
beforeEach(async () => {
  directory = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "pathway-kde-test-"));
  paths = {
...
it("captures with verified identity, cleans private files, and activates only the owning Pathway PID/title", async () => {
  await setup.perform("install-kde-helper");

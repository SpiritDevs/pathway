...
// Recursive wire decoders need a depth bound even when a tree fits the byte and node limits.
const MAX_ACCESSIBILITY_TREE_DEPTH = 128;
...
function compactAccessibilityNodes(
  ...
  const children = node.children.flatMap((child) =>
    compactAccessibilityNodes(child, false, descendantLocationsReliable, depth + 1, onTruncated),
  );
...
export async function accessibleWindowElementTree(
  ...
  const visit = async (
    ...
    if (depth >= MAX_ACCESSIBILITY_TREE_DEPTH || !shouldContinue()) {
      truncated = true;
      return undefined;
    }
    ...
    for (const child of children) {
      const childNode = await visit(child, false, depth + 1);
      if (!childNode) break;
  ...
  const completedRoot = await visit(element, true, 0);
...
export function hideAndWaitForBlur(window: {
  ...
  return new Promise((resolve, reject) => {
    const timeout = setTimeout(() => {
      ...
      reject(new Error("Timed out waiting for Pathway to lose focus."));

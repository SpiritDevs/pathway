...
// oxlint-disable-next-line pathway/no-global-process-runtime -- Test collection checks the host before starting these Linux-native filesystem tests.
...
beforeEach(async () => {
  directory = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "pathway-capture-config-"));

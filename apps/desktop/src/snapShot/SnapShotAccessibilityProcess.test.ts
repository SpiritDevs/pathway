...
it("keeps Pathway alive when Zoom crashes accessibility extraction", async () => {
...
it("does not prewarm for a delayed accessibility read after capture is disabled", async () => {
  const standbyWorker = makeWorker();
  const captureWorker = makeWorker();
  const replacementWorker = makeWorker();
  forkMock
    .mockReset()
    .mockReturnValueOnce(standbyWorker)
    .mockReturnValueOnce(captureWorker)
    .mockReturnValueOnce(replacementWorker);
  const pool = makeSnapShotAccessibilityProcessPool("accessibility.cjs");
  try {
    pool.warm();
    pool.cool();
    assert.lengthOf(standbyWorker.kill.mock.calls, 1);
    const read = pool.read(request);
    captureWorker.emit("message", "ready");
    captureWorker.emit("message", "started");
    captureWorker.emit("message", { type: "result", context: { accessibleText: "Captured" } });

    assert.deepEqual(await read.result, { accessibleText: "Captured" });
    assert.lengthOf(forkMock.mock.calls, 2);
    pool.warm();
    assert.lengthOf(forkMock.mock.calls, 3);
  } finally {

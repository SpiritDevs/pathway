...
it.runIf(hasDbus)("captures through real D-Bus marshalling on a private bus", async () => {
  const directory = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "pathway-private-dbus-"));
  ...
  try {
    ...
    await server.requestName("org.gnome.Shell.Extensions.PathwaySnapShot", NameFlag.DO_NOT_QUEUE);
    ...
    expect(await portal.backend("com.spiritdevs.Pathway")).toBe("screenshot-portal");
    expect(await portal.capturePortal()).toEqual({ png });
    expect(target).toBe(8);
    portalVersion = 2;
    const extension = connect();
    expect(await extension.backend("com.spiritdevs.Pathway")).toBe("gnome-extension");
    expect(await extension.captureExtension("com.spiritdevs.Pathway")).toMatchObject({
      png,
      window: { processId: 42 },
    });
    const owner = await server.call(
      new Message({
        ...
        member: "GetNameOwner",
        signature: "s",
        body: ["com.spiritdevs.Pathway.SnapShot"],
      }),
    );
    ...
    expect(await updated.backend("com.spiritdevs.Pathway")).toBe("gnome-extension");
    const snapshot = await updated.captureExtension("com.spiritdevs.Pathway", {
      flash: true,
      animate: true,
    });
    expect(snapshot.feedback?.animationStarted).toBe(true);
    await snapshot.feedback!.activate("Pathway");
    ...
    expect(activateTitle).toBe("Pathway");
    expect(animateFrame).toEqual([0.1, 0.8, 0.2, 0.1]);
    vi.stubEnv("XDG_CURRENT_DESKTOP", "KDE");
    const kde = connect();
    expect(await kde.backend("com.spiritdevs.Pathway")).toBe("kde");
    ...
    stopNiriShortcut = await startNiriCaptureShortcut(
      "com.spiritdevs.Pathway.NiriTest",
      triggered,
      failed,
      niriBus,
    );
    const command = [
      ...
      "--session",
      "--dest",
      "com.spiritdevs.Pathway.NiriTest.SnapShot",
      "--object-path",
      "/com/spiritdevs/SnapShot",
      "--method",
      "com.spiritdevs.SnapShot.Capture",
    ];
    ...
    const invalid = server.call(
      new Message({
        destination: "com.spiritdevs.Pathway.NiriTest.SnapShot",
        path: "/com/spiritdevs/SnapShot",
        interface: "com.spiritdevs.SnapShot",
        member: "Capture",
        signature: "s",
        body: ["not allowed"],
      }),

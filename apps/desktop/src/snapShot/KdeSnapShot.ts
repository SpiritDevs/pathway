...
import type { DesktopCaptureHelperState } from "@spiritdevs/contracts";
...
export const KDE_CAPTURE_EXECUTABLE = "pathway-kde-snap-shot";
const DESKTOP_FILE = "com.spiritdevs.Pathway.KdeCapture.desktop";
const MARKER = "X-Pathway-Capture-Helper=true";
...
export function kdeCapturePaths(paths: KdeCapturePaths) {
  return {
    executable: NodePath.join(paths.dataHome, "pathway", "kde-capture", KDE_CAPTURE_EXECUTABLE),
...
export class KdeCaptureSetup {
  ...
  async state(): Promise<DesktopCaptureHelperState> {
    try {
      ...
      if (!bundle)
        return {
          status: "error",
          message: "The capture helper is missing from this build. Update or reinstall Pathway.",
        };
  ...
  async perform(action: "install-kde-helper" | "remove-kde-helper") {
    ...
    if (action === "remove-kde-helper") {
      ...
      if (!bundle)
        throw new Error(
          "The capture helper is missing from this build. Update or reinstall Pathway.",
        );
...
export async function captureKdeWindow(
  ...
  const directory = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "pathway-kde-capture-"));
  let retained = false;
      ...
      feedback: {
        ...
        activate: async (title) => {
          ...
          const activation = await NodeFSP.mkdtemp(
            NodePath.join(NodeOS.tmpdir(), "pathway-kde-activate-"),
          );
          try {
            await run(
              executable,

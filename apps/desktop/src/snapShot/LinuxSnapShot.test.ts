...
const appId = "com.spiritdevs.Pathway";
const metadata = {
  title: "Editor",
  appName: "Text Editor",
...
beforeEach(async () => {
  ...
  directory = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "pathway-portal-test-"));
...
it("retains the authenticated connection until the compositor flight has landed", async () => {
  ...
  await feedback.activate("Pathway");
  ...
  expect(
    bus.calls
      ...
      .map((call) => call.body),
  ).toEqual([[false, true], ["Pathway"], [0.1, 0.8, 0.2, 0.1]]);
  expect(bus.disconnect).toHaveBeenCalledOnce();
  feedback.close();
  expect(bus.disconnect).toHaveBeenCalledOnce();
});
...
it("can activate without animation, and expires a renderer-abandoned capture", async () => {
  ...
  await expect(snapshot!.feedback!.activate("Pathway")).rejects.toThrow("No window");

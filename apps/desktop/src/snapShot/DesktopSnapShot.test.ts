...
const testLayer = (
  ...
  settingsGet: Effect.Effect<Option.Option<ClientSettings>> = Effect.succeed(initialSettings),
) =>
  Layer.mergeAll(
    Layer.succeed(
...
const makeSignedInService = DesktopSnapShot.make.pipe(
  Effect.tap((service) => service.setAccount("user-a")),
);
...
it.effect(
  "waits for a signed-in account before enabling capture and keeps the enabled preference",
  () => {
    registerShortcutMock.mockReset().mockReturnValue(true);
    return Effect.scoped(
      Effect.gen(function* () {
        ...
        assert.equal((yield* service.capture.pipe(Effect.flip)).operation, "signed-out");
        assert.deepEqual(yield* service.listPending, []);
        yield* service.setAccount("user-a");
    ...
    ).pipe(Effect.provide(testLayer("win32", {}, Option.some(enabledSettings()))));
  },
);

it.effect(
  "isolates pending files by account and restores only the same account after restart",
  () => {
    const ids = [
    ...
    const metadata = new Map(
      ids.map((id, index) => [
        `/state/snap-shots/${id}.json`,
        JSON.stringify({
          id,
          ...(index < 2 ? { ownerUserId: index === 0 ? "user-a" : "user-b" } : {}),
          name: "capture.png",
          mimeType: "image/png",
          sizeBytes: 3,
          source: {
    ...
    return Effect.scoped(
      Effect.gen(function* () {
        ...
        yield* service.setAccount("user-a");
        assert.deepEqual(
          (yield* service.listPending).map((capture) => capture.id),
          [ids[0]],
        );
        assert.equal((yield* service.read(ids[0]!)).ownerUserId, "user-a");
        for (const id of ids.slice(1)) {
          assert.equal((yield* service.read(id).pipe(Effect.flip)).operation, "read");
          assert.equal((yield* service.acknowledge(id).pipe(Effect.flip)).operation, "acknowledge");
        }
        yield* service.setAccount("user-b");
        assert.deepEqual(
          (yield* service.listPending).map((capture) => capture.id),
          [ids[1]],
        );
        assert.equal((yield* service.read(ids[0]!).pipe(Effect.flip)).operation, "read");
        assert.equal(
          (yield* service.acknowledge(ids[0]!).pipe(Effect.flip)).operation,
          "acknowledge",
        );
        assert.deepEqual(removed, []);
        const restarted = yield* DesktopSnapShot.make;
        assert.deepEqual(yield* restarted.listPending, []);
        yield* restarted.setAccount("user-a");
        assert.deepEqual(
          (yield* restarted.listPending).map((capture) => capture.id),
          [ids[0]],
        );
      ...
      Effect.provide(
        testLayer("win32", {
          readDirectory: () => Effect.succeed(ids.map((id) => `${id}.json`)),
          readFileString: (path) => Effect.sync(() => metadata.get(path)!),
...
it.effect.each(["list", "read", "acknowledge"] as const)(
  "rechecks account ownership when an asynchronous pending %s completes",
  (operation) => {
    ...
    return Effect.scoped(
      Effect.gen(function* () {
        ...
        assert.deepEqual(removed, []);
      }).pipe(Effect.ensuring(Effect.sync(() => release.resolve(metadata)))),
    ).pipe(
      Effect.provide(
        testLayer("win32", {
          readDirectory: () => Effect.succeed([`${id}.json`]),
          readFileString: () =>
            Effect.promise(() => {
              started.resolve();
...
it.effect.each(["win32", "darwin", "linux"] as const)(
  "does not start acquiring more %s pixels after the account changes during window lookup",
  (platform) => {
    ...
    const active = {
      ...
      owner: { name: "Editor", processId: 100 },
    ...
    if (platform === "linux") {
      ...
      activeWindowMock.mockImplementationOnce(async () => {
        lookupStarted.resolve();
        await lookupFinished.promise;
        return active;
      });
    ...
    return Effect.scoped(
      Effect.gen(function* () {
        ...
        assert.equal((yield* Fiber.join(capture)).operation, "signed-out");
...
it.effect.each(["win32", "darwin", "linux"] as const)(
  "keeps an in-flight %s capture with its original account without notifying the next account",
  (platform) => {
    ...
    return Effect.scoped(
      Effect.gen(function* () {
        ...
        fixture.first.context.resolve({ accessibleText: "Original account" });
        yield* Fiber.join(capture);
        assert.deepEqual(fixture.readyIds, []);
        assert.equal(fixture.state.handoffs, 0);
        const id = fixture.requestedIds[0]!;
        assert.equal((yield* service.read(id).pipe(Effect.flip)).operation, "read");
        assert.equal((yield* service.acknowledge(id).pipe(Effect.flip)).operation, "acknowledge");
        yield* service.setAccount("user-a");
        const restored = yield* service.read(id);
        assert.equal(restored.ownerUserId, "user-a");
        assert.equal(restored.source.windowTitle, fixture.first.title);
...
it.effect.each(["win32", "darwin", "linux"] as const)(
  ...
  (platform) => {
    ...
    const pathway = {
      id: 42,
      title: "Pathway",
      appIdentifier: "com.spiritdevs.Pathway.desktop",
      owner: { name: "Pathway", processId: 123 },
      bounds,
      png: Buffer.from([1, 2, 3]),
    };
    focusedWindowMock.mockReturnValue({
      getBounds: () => bounds,
      getTitle: () => pathway.title,
      isDestroyed: () => false,
      isMinimized: () => false,
      isVisible: () => true,
      show: vi.fn(),
    ...
    macCaptureMock.mockReset().mockImplementation(async () => {
      images.push(pathway.png);
      return { source: { name: pathway.title }, png: pathway.png };
    });
    ...
    linuxCaptureMock.mockResolvedValueOnce({
      ...
      window: {
        title: pathway.title,
        appName: pathway.owner.name,
        appIdentifier: pathway.appIdentifier,
        processId: pathway.owner.processId,
        bounds,
      },
      feedback: {
    ...
    return Effect.scoped(
      Effect.gen(function* () {
        ...
        const saved = yield* decodePendingMetadata(metadata);
        assert.equal(saved.source.windowTitle, pathway.title);
        assert.equal(saved.source.appName, pathway.owner.name);
...
it.effect("matches Windows accessibility windows on a scaled display", () => {
  ...
  return Effect.scoped(
    Effect.gen(function* () {
      ...
      yield* service.capture;
      const saved = yield* decodePendingMetadata(metadata);
      assert.notProperty(saved.source, "accessibleText");
      assert.deepEqual(
      ...
      Effect.gen(function* () {
        ...
        const warning = logs.find(
          (message) =>
            ...
            message[0] === "The compositor could not activate Pathway after the snapshot",
        );
      ...
      Effect.provide(
        Layer.mergeAll(
          testLayer("linux", {
            ...
            readFileString: () => Effect.sync(() => saved),
            remove: () =>
              Effect.sync(() => {
                order.push("delete");
              }),
...
it.each([
  { names: ["⠙ pathway"], expected: "Verified text" },
  { names: ["⠋ pathway", "⠙ pathway"], expected: undefined },
...
it.effect("flags revoked macOS permissions on read and re-registers once they return", () => {
  ...
  return Effect.scoped(
    Effect.gen(function* () {
      ...
      assert.equal(
        revoked.message,
        "Allow Screen Recording in System Settings, then restart Pathway.",
      );
...
it("resolves Pathway's macOS window instead of a same-process tooltip", async () => {
  const popover = {
    platform: "macos",
    id: 13182,
    title: "Tooltip",
    owner: { name: "Pathway", processId: process.pid, path: "/Applications/Pathway.app" },
    bounds: { x: 416, y: 92, width: 66, height: 20 },
  };
  const mainBounds = { x: 100, y: 50, width: 1000, height: 750 };
  activeWindowMock.mockReset().mockResolvedValue(popover);
  focusedWindowMock.mockReturnValue({
    isDestroyed: () => false,
    getMediaSourceId: () => "window:13180:0",
    getTitle: () => "Pathway conversation",
    getBounds: () => mainBounds,
  });
  try {
    assert.deepEqual(await DesktopSnapShot.resolveSnapShotActiveWindow("darwin"), {
      ...popover,
      id: 13180,
      title: "Pathway conversation",
      bounds: mainBounds,
    });
    const external = { ...popover, owner: { ...popover.owner, processId: process.pid + 1 } };
    activeWindowMock.mockResolvedValue(external);
    assert.deepEqual(await DesktopSnapShot.resolveSnapShotActiveWindow("darwin"), external);
  } finally {
    focusedWindowMock.mockReset();
  }
});
...
for (const fails of [false, true]) {
  it.effect(`tests macOS capture without publishing it and cleans up, failure=${fails}`, () => {
    const active = {
      ...
      id: 42,
      title: "Setup",
      owner: { name: "Pathway", processId: 123, path: "/Applications/Pathway.app" },
      bounds: { x: 0, y: 0, width: 800, height: 600 },
    };
    activeWindowMock.mockReset().mockResolvedValue(active);
    ...
    return Effect.scoped(
      Effect.gen(function* () {
        ...
        if (fails) {
          ...
          assert.equal(
            error.message,
            "Could not capture the active window. Try again, or restart Pathway if you just allowed Screen Recording.",
          );

...
import { SnapShotAccessibility, type SnapShotAccessibilityNode } from "@spiritdevs/contracts";
...
describe("accessibleWindowElementTree", () => {
  it("stops native collection before deeply nested controls exceed the wire decoder's stack", async () => {
    let childReads = 0;
    let element: Parameters<typeof accessibleWindowElementTree>[0] = {
      role: "unknown",
      children: async () => {
        childReads++;
        return [];
...
describe("compactAccessibilityTree", () => {
  it.each([128, 600])("produces decodable metadata for a %i-level control tree", (depth) => {
    let root: SnapShotAccessibilityNode = { role: "unknown", bounds: null, children: [] };
    for (let level = 1; level < depth; level++) {
      root = { role: "unknown", bounds: null, children: [root] };
    }
    const compacted = compactAccessibilityTree(root);
    const metadata = Schema.decodeUnknownSync(SnapShotAccessibility)({
      format: "element-tree",
    ...
    expect(metadata.truncated).toBe(depth > 128);
    let retainedDepth = 0;
    let current: SnapShotAccessibilityNode | undefined = compacted.root;
    while (current) {
      retainedDepth++;
      current = current.children[0];
    }
    expect(retainedDepth).toBe(Math.min(depth, 128));
  });
...
describe("findAccessibleWindow", () => {
  ...
  it.each(["⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏"])(
    ...
    (frame) => {
      const windows = [{ name: `${frame} pathway`, bounds: captured.bounds }];

      expect(findAccessibleWindow(windows, { ...captured, title: "⠋ pathway" }, "wayland")).toBe(
        windows[0],
      );
    },
  );
  ...
  it("rejects matching spinners when the window sizes differ", () => {
    expect(
      findAccessibleWindow(
        [{ name: "⠙ pathway", bounds: { ...captured.bounds, width: 400 } }],
        { ...captured, title: "⠋ pathway" },
        "wayland",
      ),
    ).toBeUndefined();
  });
  ...
  it("rejects ambiguous normalized titles even if one matches the captured spinner exactly", () => {
    const windows = [
      { name: "⠋ pathway", bounds: captured.bounds },
      { name: "⠙ pathway", bounds: captured.bounds },
    ];

    expect(
      findAccessibleWindow(windows, { ...captured, title: "⠋ pathway" }, "wayland"),
    ).toBeUndefined();
  });

  it("keeps exact title matching outside Wayland", () => {
    expect(
      findAccessibleWindow([{ name: "⠙ pathway", bounds: captured.bounds }], {
        ...captured,
        title: "⠋ pathway",
      }),
    ).toBeUndefined();
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* Effect.promise(async () => {
          ...
          const runtimeDirectory = await mkdtemp(join(tmpdir(), "pathway-wayland-"));
          ...
          try {
            ...
            expect(
              isWaylandSession("linux", { XDG_RUNTIME_DIR: "/nonexistent-pathway-test" }),
            ).toBe(false);
          } finally {
            if (server.listening) {
              await new Promise<void>((resolve, reject) => {

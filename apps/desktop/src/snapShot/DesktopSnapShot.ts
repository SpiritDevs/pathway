...
const MAC_ACCESSIBILITY_PERMISSION_MESSAGE =
  "Allow Accessibility in System Settings, then restart Pathway.";
const MAC_BOTH_PERMISSIONS_MESSAGE =
  "Allow Accessibility and Screen Recording in System Settings, then restart Pathway.";
...
export class DesktopSnapShotError extends Schema.TaggedErrorClass<DesktopSnapShotError>()(
  ...
  override get message(): string {
    switch (this.operation) {
      ...
      case "signed-out":
        return "Sign in to Pathway Cloud to take SnapShots.";
      case "disabled":
...
export class DesktopSnapShot extends Context.Service<
  DesktopSnapShot,
  {
    ...
    readonly setAccount: (userId: string | null) => Effect.Effect<void>;
...
>()("@spiritdevs/desktop/snapShot/DesktopSnapShot") {}
...
export class DesktopSnapShotSetupError extends Schema.TaggedErrorClass<DesktopSnapShotSetupError>()(
  ...
  override get message(): string {
    if (this.reason === "signed-out") return "Sign in to Pathway Cloud to set up SnapShots.";
    if (this.action === "test-mac-capture") {
      return this.reason === "unsupported-session"
        ? "Capture testing is only available on macOS."
        : "Could not capture the active window. Try again, or restart Pathway if you just allowed Screen Recording.";
    }
    if (this.action === "allow-screen-recording")
      return "Could not request Screen Recording access.";
    if (this.action === "allow-accessibility") return "Could not request Accessibility access.";
    if (this.action === "retry-shortcut")
      return "Could not register the capture shortcut. Try another shortcut.";
...
export async function resolveSnapShotActiveWindow(
  platform: NodeJS.Platform,
): Promise<ActiveWindow | undefined> {
  const active = await activeWindow(platform);
  if (platform !== "darwin" || active?.owner.processId !== process.pid) return active;
  // AppKit tooltips can appear ahead of the real window in CoreGraphics' list.
  // Electron knows the actual window when Pathway itself is foreground.
  const window =
    Electron.BrowserWindow.getFocusedWindow() ??
    Electron.BrowserWindow.getAllWindows().find(
      (candidate) => !candidate.isDestroyed() && candidate.isVisible(),
    );
  if (!window || window.isDestroyed()) return active;
  const match = /^window:(\d+):/.exec(window.getMediaSourceId());
  if (!match) return active;
  return { ...active, id: Number(match[1]), title: window.getTitle(), bounds: window.getBounds() };
}
...
async function captureSource({
  ...
  const destinationWindowBounds = destinationWindow?.getBounds();
  {
    ...
    const accessibilityRead =
      isCurrentAccount() && accessibleIdentity && settings.snapShotIncludeAccessibility
...
export const make = Effect.gen(function* () {
  ...
  const isCurrentAccount = (owner: string, revision: number) =>
    accountUserId === owner && accountRevision === revision;
  ...
  const prepareCapture = Effect.fn("desktop.snapShot.prepareCapture")(function* (
    ...
    return yield* Effect.gen(function* () {
      ...
      if (isCurrentAccount(ownerUserId, revision) && snapshot.animationStarted) {
  ...
  const checkShortcut = Effect.fn("desktop.snapShot.checkShortcut")(function* (
    ...
    if (mode === "portal" && niriSocketPath()) {
      return {
        available: false,
        message: "Configure the capture shortcut in your Niri config, not in Pathway.",
      };
    }
  ...
  const applySettings = Effect.fn("desktop.snapShot.applySettings")(function* (
    ...
    const enabled = settings.snapShotEnabled && accountUserId !== null;
    const mode = captureMode(environment.platform);
    const shortcut = settings.snapShotShortcut;
    if (enabled && settings.snapShotIncludeAccessibility && mode !== "unavailable") {
      accessibilityProcessPool.warm();
    } else {
      accessibilityProcessPool.cool();
    }
    if (enabled && environment.platform === "win32") {
      regionSnapShotPool.warm();
    } else {
      regionSnapShotPool.cool();
    }
    if (!enabled || !settings.snapShotFlash || mode === "unavailable") {
      flash.dispose();
    }
    if (!enabled || !settings.snapShotAnimations || mode === "unavailable") {
      transition.dispose();
      closeLinuxFeedback();
    }
    ...
    if (mode === "portal" && niriSocketPath()) {
      const registered = yield* Effect.tryPromise(async () => {
        ...
        return startNiriCaptureShortcut(linuxAppId, onCurrentShortcut, () => {
          void runPromise(
            setShortcutFailure("The Niri capture endpoint disconnected. Restart Pathway."),
          ).catch(() => undefined);
        });
      }).pipe(
        Effect.tap((stop) =>
      ...
      yield* Ref.set(stateRef, {
        ...
        shortcutMessage: registered
          ...
          : "Could not start the Niri capture endpoint. Another Pathway instance may be using it.",
        message: null,
      });
      return;
    }
  ...
  const setAccount = Effect.fn("desktop.snapShot.setAccount")(function* (userId: string | null) {
    if (accountUserId === userId) return;
    accountUserId = userId;
    accountRevision++;
    // Authentication is supplied by the trusted renderer after Clerk resolves. No
    // account is restored from disk, even when the enabled preference was saved.
  ...
  const setup = Effect.fn("desktop.snapShot.setup")(function* (action: DesktopSnapShotSetupAction) {
    if (accountUserId === null) {
      return yield* new DesktopSnapShotSetupError({ action, reason: "signed-out" });
    }
      ...
      yield* Effect.scoped(
        Effect.gen(function* () {
          const directory = yield* fileSystem.makeTempDirectoryScoped({
            prefix: "pathway-snapshot-test-",
          });
          yield* Effect.tryPromise(async () => {
            const active = await resolveSnapShotActiveWindow("darwin");
  ...
  return DesktopSnapShot.of({
    ...
    listPending: Effect.suspend(() => {
      const ownerUserId = accountUserId;
      const revision = accountRevision;
      if (ownerUserId === null) return Effect.succeed([]);
      return fileSystem.readDirectory(captureDirectory).pipe(
        Effect.catchTags({
          PlatformError: (cause) =>
    ...
    read: (id) =>
      Effect.gen(function* () {
        ...
        if (metadata.ownerUserId !== ownerUserId || !isCurrentAccount(ownerUserId, revision)) {
          return yield* new DesktopSnapShotError({ operation: "read", captureId: id });
        }

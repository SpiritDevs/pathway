...
export const setSnapShotAccount = DesktopIpc.makeIpcMethod({
  channel: IpcChannels.SET_SNAP_SHOT_ACCOUNT_CHANNEL,
  payload: Schema.NullOr(
    Schema.String.check(Schema.isMinLength(1), Schema.isMaxLength(255), Schema.isTrimmed()),
  ),
  result: Schema.Void,
  handler: Effect.fn("desktop.ipc.snapShot.setAccount")(function* (userId, event) {
    const { webContents } = yield* ensureTrustedSnapShotSender(event);
    ...
    if (accountRendererBinding?.webContents !== webContents) {
      ...
      accountRendererBinding = { webContents, dispose };
    }
    yield* snapShot.setAccount(userId);
  }),
});

...
#import "PathwayMarkdownTextShadowNode.h"
#import "PathwayMarkdownTextComponentDescriptor.h"
...
static void PathwayMarkdownTextApplyParagraphStyles(
    NSMutableAttributedString *attributedString,
    const std::vector<PathwayMarkdownTextParagraphStyleRange> &styleRanges)
{
...
static void PathwayMarkdownTextApplyAttachments(
    NSMutableAttributedString *attributedString,
    const std::vector<PathwayMarkdownTextAttachmentRange> &attachmentRanges,
    NSDictionary<NSString *, UIImage *> *images)
{
  for (const auto &attachmentRange : attachmentRanges) {
    ...
    const CGFloat attachmentSize = PathwayMarkdownTextAttachmentSize(attachmentRange);
    attachment.bounds = CGRectMake(
        0,
        PathwayMarkdownTextAttachmentBaselineOffset(attachmentRange),
        attachmentSize,
        attachmentSize);
...
@protocol PathwayMarkdownOutsideTapTarget <NSObject>
- (void)clearSelectionForOutsideTapWithHitView:(UIView *)hitView;
@end

@interface PathwayMarkdownOutsideTapCoordinator : NSObject <UIGestureRecognizerDelegate>

- (instancetype)initWithWindow:(UIWindow *)window;
- (void)addTarget:(id<PathwayMarkdownOutsideTapTarget>)target;
- (void)removeTarget:(id<PathwayMarkdownOutsideTapTarget>)target;

@end

static const void *PathwayMarkdownOutsideTapCoordinatorKey =
    &PathwayMarkdownOutsideTapCoordinatorKey;

@implementation PathwayMarkdownOutsideTapCoordinator {
  __weak UIWindow *_window;
  UITapGestureRecognizer *_recognizer;
  NSHashTable<id<PathwayMarkdownOutsideTapTarget>> *_targets;
}
...
- (void)removeTarget:(id<PathwayMarkdownOutsideTapTarget>)target
{
  ...
  if (objc_getAssociatedObject(window, PathwayMarkdownOutsideTapCoordinatorKey) == self) {
...
- (void)handleTap:(UITapGestureRecognizer *)sender
{
  ...
  for (id<PathwayMarkdownOutsideTapTarget> target in _targets.allObjects) {
...
static PathwayMarkdownOutsideTapCoordinator *
PathwayMarkdownOutsideTapCoordinatorForWindow(UIWindow *window)
{
  PathwayMarkdownOutsideTapCoordinator *coordinator =
      objc_getAssociatedObject(window, PathwayMarkdownOutsideTapCoordinatorKey);
  if (coordinator == nil) {
    coordinator = [[PathwayMarkdownOutsideTapCoordinator alloc] initWithWindow:window];
    objc_setAssociatedObject(
        window,
        PathwayMarkdownOutsideTapCoordinatorKey,
        coordinator,
...
@interface PathwayMarkdownText () <RCTPathwayMarkdownTextViewProtocol, UIGestureRecognizerDelegate, UITextViewDelegate>

@end

@interface PathwayMarkdownText () <PathwayMarkdownOutsideTapTarget>
@end

@implementation PathwayMarkdownText {
  UIView * _view;
  UITextView * _textView;
  PathwayMarkdownTextShadowNode::ConcreteState::Shared _state;
...
{
  return concreteComponentDescriptorProvider<PathwayMarkdownTextComponentDescriptor>();
}

- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps = std::make_shared<const PathwayMarkdownTextProps>();
    _props = defaultProps;
...
- (void)didMoveToWindow
{
  ...
  if (_outsideTapWindow != nil) {
    PathwayMarkdownOutsideTapCoordinator *coordinator =
        objc_getAssociatedObject(_outsideTapWindow, PathwayMarkdownOutsideTapCoordinatorKey);
    [coordinator removeTarget:self];
  }
  _outsideTapWindow = self.window;
  if (_outsideTapWindow != nil) {
    [PathwayMarkdownOutsideTapCoordinatorForWindow(_outsideTapWindow) addTarget:self];
  }
}
...
- (void)drawRect:(CGRect)rect
{
  ...
  const auto &props = *std::static_pointer_cast<PathwayMarkdownTextProps const>(_props);
  ...
  PathwayMarkdownTextApplyParagraphStyles(
      convertedAttrString,
      _state->getData().paragraphStyleRanges);
  PathwayMarkdownTextApplyAttachments(
      convertedAttrString,
  ...
  if (_eventEmitter != nullptr) {
    std::dynamic_pointer_cast<const facebook::react::PathwayMarkdownTextEventEmitter>(_eventEmitter)
    ->onTextLayout(facebook::react::PathwayMarkdownTextEventEmitter::OnTextLayout{static_cast<int>(self.tag), lines});
  };
}

- (void)loadAttachmentImages:(const std::vector<PathwayMarkdownTextAttachmentRange> &)attachmentRanges
{
  for (const auto &attachmentRange : attachmentRanges) {
    NSString *imageUri = [NSString stringWithUTF8String:attachmentRange.imageUri.c_str()];
    if ([imageUri hasPrefix:@"sf:"]) {
...
{
  const auto &oldViewProps = *std::static_pointer_cast<PathwayMarkdownTextProps const>(_props);
  const auto &newViewProps = *std::static_pointer_cast<PathwayMarkdownTextProps const>(props);
  ...
  if (oldViewProps.ellipsizeMode != newViewProps.ellipsizeMode) {
    if (newViewProps.ellipsizeMode == PathwayMarkdownTextEllipsizeMode::Head) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingHead;
    } else if (newViewProps.ellipsizeMode == PathwayMarkdownTextEllipsizeMode::Middle) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingMiddle;
    } else if (newViewProps.ellipsizeMode == PathwayMarkdownTextEllipsizeMode::Tail) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingTail;
    } else if (newViewProps.ellipsizeMode == PathwayMarkdownTextEllipsizeMode::Clip) {
...
{
  _state = std::static_pointer_cast<const PathwayMarkdownTextShadowNode::ConcreteState>(state);
  [self setNeedsDisplay];
}
...
- (PathwayMarkdownTextRun*)getTouchChild:(CGPoint)location
{
  ...
  for (UIView* child in self.subviews) {
    if (![child isKindOfClass:[PathwayMarkdownTextRun class]]) {
      continue;
    }

    PathwayMarkdownTextRun* textChild = (PathwayMarkdownTextRun*)child;
...
- (void)textViewDidChangeSelection:(UITextView *)textView
{
  ...
  std::dynamic_pointer_cast<const facebook::react::PathwayMarkdownTextEventEmitter>(_eventEmitter)
    ->onSelectionChange(facebook::react::PathwayMarkdownTextEventEmitter::OnSelectionChange{
...
Class<RCTComponentViewProtocol> PathwayMarkdownTextCls(void)
{
  return PathwayMarkdownText.class;
}

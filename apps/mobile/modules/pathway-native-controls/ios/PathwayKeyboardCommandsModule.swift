...
public final class PathwayKeyboardCommandsModule: Module {
  public func definition() -> ModuleDefinition {
    Name("PathwayKeyboardCommands")

    View(PathwayKeyboardCommandsView.self) {
      Prop("enabledCommands") { (view: PathwayKeyboardCommandsView, commands: [String]) in
...
public final class PathwayKeyboardCommandsView: ExpoView {
  ...
  @objc private func reclaimFirstResponderIfAvailable() {
    DispatchQueue.main.async { [weak self] in
      guard let self, self.window?.pathwayFirstResponder == nil else { return }
...
private extension UIView {
  var pathwayFirstResponder: UIResponder? {
    if isFirstResponder { return self }
    for subview in subviews {
      if let responder = subview.pathwayFirstResponder { return responder }
    }
    return nil
  }
}

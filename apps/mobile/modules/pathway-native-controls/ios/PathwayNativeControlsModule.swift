...
public final class PathwayNativeControlsModule: Module {
  public func definition() -> ModuleDefinition {
    ...
    Function("getShowcaseScene") { () -> String? in
      let scenePath = NSHomeDirectory() + "/Library/Caches/PathwayShowcaseScene"
    ...
    AsyncFunction("applyShowcaseOrientation") { (orientation: String) in
      ...
      for case let windowScene as UIWindowScene in UIApplication.shared.connectedScenes {
        windowScene.requestGeometryUpdate(.iOS(interfaceOrientations: mask)) { error in
          NSLog("PathwayNativeControls applyShowcaseOrientation(\(orientation)) failed: \(error)")
        }
    ...
    Function("markShowcaseReady") { (scene: String) in
      let readyPath = NSHomeDirectory() + "/Library/Caches/PathwayShowcaseReadyScene"
      try? scene.write(toFile: readyPath, atomically: true, encoding: .utf8)
    }

    View(PathwayHeaderButtonView.self) {
      Prop("label") { (view: PathwayHeaderButtonView, label: String) in
        view.setLabel(label)
      }
      Prop("systemImage") { (view: PathwayHeaderButtonView, systemImage: String) in

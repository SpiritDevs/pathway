...
describe("extractPairingUrlFromQrPayload", () => {
  ...
  it("unwraps mobile deep links that carry an encoded pairing url", () => {
    expect(
      extractPairingUrlFromQrPayload(
        "pathway://pair?pairingUrl=https%3A%2F%2Fremote.example.com%2Fpair%23token%3Dpairing-token",
      ),
    ).toBe("https://remote.example.com/pair#token=pairing-token");
  });
...
describe("parsePairingUrl", () => {
  it("reads hosted pairing links into backend host fields", () => {
    expect(
      parsePairingUrl(
        "https://app.spiritdevs.com/pair?host=https%3A%2F%2Fdesktop.example.com%2F#token=pairing-token",
      ),
    ).toEqual({
      host: "https://desktop.example.com",
      code: "pairing-token",
    });
  });
});

...
final class PathwayConversationUITests: XCTestCase {
    ...
    func testWorkingReturnToLatestAndDiffBubble() {
        ...
        let idle = NSPredicate(format: "exists == true AND hittable == true AND (value == nil OR value == %@)", "")
    ...
    func testImagePasteFromMenuAndMessageField() {
        ...
        XCTAssertTrue(failed.waitForExistence(timeout: 5))
        failed.tap()
        let failureDialog = app.alerts["Upload failed"]
        XCTAssertTrue(failureDialog.waitForExistence(timeout: 3))
        failureDialog.buttons["Cancel"].tap()
        XCTAssertTrue(failureDialog.waitForNonExistence(timeout: 3))
        XCTAssertTrue(failed.exists, "Cancelling the dialog must retain the failed attachment")
        XCTAssertEqual(field.value as? String, "Please review this screenshot.")
        failed.tap()
        XCTAssertTrue(failureDialog.waitForExistence(timeout: 3))
        failureDialog.buttons["Remove"].tap()
        XCTAssertTrue(failureDialog.waitForNonExistence(timeout: 3))

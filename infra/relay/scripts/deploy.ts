...
const publicConfigEnvEntries = (config: RelayPublicConfig) =>
  ({
    ...
    PATHWAY_MOBILE_OTLP_TRACES_URL: config.mobileTracingUrl,
    PATHWAY_MOBILE_OTLP_TRACES_DATASET: config.mobileTracingDataset,
    PATHWAY_MOBILE_OTLP_TRACES_TOKEN: config.mobileTracingToken,
    PATHWAY_RELAY_CLIENT_OTLP_TRACES_URL: config.clientTracingUrl,
    PATHWAY_RELAY_CLIENT_OTLP_TRACES_DATASET: config.clientTracingDataset,
    PATHWAY_RELAY_CLIENT_OTLP_TRACES_TOKEN: config.clientTracingToken,
  }) as const;
...
export function reconcileRootEnvRelayUrl(contents: string, relayUrl: string): string {
  return reconcileRootEnvPublicConfig(contents, {
    ...
    .split("\n")
    .filter((line) => !line.startsWith("PATHWAY_MOBILE_OTLP_TRACES_"))
    .filter((line) => !line.startsWith("PATHWAY_RELAY_CLIENT_OTLP_TRACES_"))
    .join("\n");
}
...
export const relayDeployCommand = Command.make(
  ...
  deploy,
).pipe(Command.withDescription("Deploy the Pathway relay through Alchemy."));

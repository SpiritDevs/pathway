...
describe("managed endpoint names", () => {
  it("uses the stage slug and a stable stage-scoped digest suffix", () => {
    ...
    expect(managedEndpointHostname("prod", "pathwayrelay.com", hash)).toBe(
      "prod-abcdef0123456789.pathwayrelay.com",
    );
    expect(managedEndpointTunnelName("dev_julius", hash)).toBe(
      "pathwayrelay-managedendpoint-dev-julius-abcdef0123456789",
    );
  });

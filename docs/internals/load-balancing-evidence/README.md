...
All changed Swift source and test files pass syntax parsing. A standalone Swift 6 harness compiled the actual selector, resolver, and environment client with surrounding type stubs; it passed 11 shared scoring fixtures, three direct-probe cleanup cases with zero subscriptions, and two checkout-selection cases. The native XCTest file adds 16 test functions covering 17 cases, but the XCTest suite and complete app build have not run.

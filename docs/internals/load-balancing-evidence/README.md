...
All changed Swift source and test files pass syntax parsing. A standalone Swift 6 harness compiled the actual selector, resolver, and environment client with surrounding type stubs; it passed 11 shared scoring fixtures and three scoped-probe cleanup cases. The native XCTest file adds 12 test functions covering 13 cases, but the XCTest suite and complete app build have not run.
...
3. Change machine weights and set one to Manual only. Recheck Auto. Verify preferences survive client restart and the excluded machine remains manually selectable.
4. Change model/options while a probe is outstanding. Select an account, branch/worktree, or attachment. Confirm a late result does not move that draft.
...
6. Interrupt the connection during launch, then retry and reload. Confirm the draft stays bound to its original destination and no second environment starts the work.
7. Repeat new-thread selection and explicit manual recovery on iOS, including a saved draft and a pending launch. Exercise direct remote and relay/tunnel connections.

The sampler reads a small whole-host snapshot on demand with a five-second shared cache. It adds no idle polling or process-tree subscription. Placement occurs on the client before the existing environment-local launch operation. Server deployments must support the new RPC to participate in Auto; manual placement remains available for older servers.

...
- Agent Threads → draft thread details: the compact meter expands into the same quota buckets and manual refresh completes. Unauthenticated Claude is hidden from this section. No provider turn was submitted.
- Top profile dropdown → Provider usage: account/environment summaries load and show the same quota buckets.

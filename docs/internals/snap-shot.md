...
`apps/desktop/src/snapShot/DesktopSnapShot.ts` owns opt-in capture, permission state, shortcut registration, native acquisition, feedback, and the pending capture queue. macOS and Windows capture the active window. Wayland selects a desktop-specific backend or the screenshot portal/picker. X11 is unavailable. [Linux capture](linux-snap-shot.md) describes compositor setup and identity matching.
...
The desktop saves the PNG and source metadata before announcing that a capture is ready. IPC lists pending metadata separately from image bytes. `SnapShotCoordinator` pins the current draft or environment/thread reference, handles draft promotion, compresses the image if necessary, and rescales accessibility coordinates to match. It acknowledges a capture only after verifying the saved draft. A failed draft write leaves the native pending copy available for retry.
...
Accessibility extraction runs in a separate, disposable process. Failure, timeout, or missing accessibility permission can produce a screenshot without app text. Portal captures do not guess window identity to attach accessibility from an unrelated window. The capture overlay and sound are optional; animation observes reduced motion and stops after delivery.

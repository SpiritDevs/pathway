...
The launcher is the only runtime writer of `service-state.json`. `pathway service install` and
`pathway service update` may replace the launcher and state while the unit is stopped. Server children
...
1. The active server installs `@spiritdevs/pathway@<target>` into a unique staging directory.
...
```sh
npx @spiritdevs/pathway@<version> service update
```

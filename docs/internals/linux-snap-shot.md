...
`<app-id>.SnapShot` on the session bus and exports `com.spiritdevs.SnapShot.Capture`; the config
...
Source in `apps/desktop/gnome-extension`, UUID `snap-shot@spiritdevs.com`. GNOME only discovers a newly
...
The extension trusts callers that own `com.spiritdevs.Pathway.SnapShot` (or the `.Development`
...
inside the extension with coordinates relative to Pathway's content area. Upstream found that Electron's restored-session
path could skip rebinding and leave callbacks behind on unregister, which is why
...
## Building helpers for development

Build each helper with `cargo build --release --manifest-path native/<backend>-snap-shot/Cargo.toml`
on Linux, where `<backend>` is `kde` or `hyprland`. Development reads the executable from that
crate's `target/release` directory. Release packaging builds the selected architecture under its
Rust target triple and stages the executable alongside the desktop resources.

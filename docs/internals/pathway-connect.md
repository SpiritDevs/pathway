...
[pathway-connect-auth-flow.html](./pathway-connect-auth-flow.html).
...
should set `PATHWAY_CLERK_PUBLISHABLE_KEY`, `PATHWAY_CLERK_JWT_TEMPLATE`,
`PATHWAY_CLERK_CLI_OAUTH_CLIENT_ID`, and `PATHWAY_RELAY_URL` before building. EAS preview and
...
5. Set `PATHWAY_CLERK_CLI_OAUTH_CLIENT_ID` in the repository-root `.env` file and release build
...
managed tunnel. The tunnel allocation and connector token survive ordinary server shutdowns so a restart can
reattach without recreating Cloudflare resources or waiting for DNS propagation. `pathway connect unlink`
records disabled intent immediately, stops a reachable running
...
Local desktop development uses `pathway-dev://app`, while packaged builds use `pathway://app`. Add the
matching origin to each Clerk instance's Backend API `allowed_origins` array as well. The development
Clerk instance should only need `pathway-dev://app`; the production Clerk instance should only need
`pathway://app`. `@clerk/electron` owns the native request adapter, encrypted Clerk token persistence,
...
PATHWAY_MACOS_PROVISIONING_PROFILE=/absolute/path/to/pathway.provisionprofile
# Optional: comma-separated override when Clerk's RP ID differs from the Frontend API hostname.
PATHWAY_CLERK_PASSKEY_RP_DOMAINS=example.clerk.accounts.dev,clerk.example.com
```

When `PATHWAY_CLERK_PASSKEY_RP_DOMAINS` is absent, the build derives the RP domain from
`PATHWAY_CLERK_PUBLISHABLE_KEY`. Signed macOS builds fail early if the Team ID, provisioning profile,
...
installed app executable with `VITE_DEV_SERVER_URL` and `PATHWAY_PORT` set. Rebuild the signed app
...
PATHWAY_PORT=13773 \
  "/Applications/Pathway (Alpha).app/Contents/MacOS/Pathway (Alpha)"
```
...
```sh
codesign --verify --deep --strict "/Applications/Pathway (Alpha).app"
codesign -d --entitlements :- "/Applications/Pathway (Alpha).app"
```
...
controls, rendered by `SettingsSidebarNav.tsx`: `PathwayConnectSidebarSignIn` in the footer shows a
**Sign in to Pathway Connect** button while signed out, and `PathwayConnectSidebarAvatar` shows a Clerk

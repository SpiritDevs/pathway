...
Pathway has one runtime boundary: a client talks to a Pathway server over HTTP and WebSocket, and the server
...
┌───────────────▼──────────────────────────────┐
│ Execution environment = one Pathway server        │
...
One running Pathway server instance. It owns provider availability and auth, model availability, projects
...
Access answers one question: how does the client speak WebSocket to a Pathway server? It does not answer
...
Managed Pathway Connect relay tunnels use `RelayConnectionTarget` and are the answer when the host is
...
[packages/ssh/src/tunnel.ts][sshtunnel], which resolves the target, launches or reuses the remote Pathway
...
Launch answers a different question: how does a Pathway server come to exist on the target machine? Keep
it separate from access.

- **Pre-existing server.** The operator already runs Pathway and the client connects directly or through a
  tunnel.

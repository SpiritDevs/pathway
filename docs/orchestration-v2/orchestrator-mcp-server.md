...
- create one or more ordinary top-level Pathway threads;
...
Creates a Pathway-owned child thread and immediately dispatches the supplied task
prompt.
...
Creates between one and twenty ordinary top-level Pathway threads:

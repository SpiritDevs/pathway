...
ARM64 by default for local Apple Silicon development; CI sets `Pathway_SHOWCASE_ANDROID_ABI=x86_64` so the
...
`pnpm --filter @spiritdevs/mobile showcase` starts Expo on its normal port, so it is not compatible with
...
The Pending rows use the production offline outbox and point at the real Pathway and React fixture

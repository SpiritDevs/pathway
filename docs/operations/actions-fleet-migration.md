...
Staging requires an existing package. A public registry lookup for `@spiritdevs/pathway` returned 404 on 2026-09-19; that means absent or not accessible to that lookup, not proof of ownership. Confirm the npm organization/package first. If it is new, build a reviewed Apple Silicon package and perform one explicit initial publication while signed in as an authorized maintainer, with 2FA. The automated workflow never falls back to direct publication.

...
Recordings capture one selected tab without audio. The encoder writes to disk while recording. A recording ends at ten minutes or approximately 500 MiB. Screenshots and videos use the environment's attachment storage and signed, expiring download URLs. The asset endpoint supports single byte ranges for video seeking. Capture ownership and metadata are stored in a per-task index, so reopening the capture list after a server restart can issue fresh download URLs. Opening the capture list again refreshes its download URLs. Each task retains at most 50 environment-browser captures totaling at most 1 GiB. Saving a capture or recovering the task’s index removes the oldest indexed capture files until both limits are met. Task deletion removes its indexed captures and index after browser teardown. Cleanup targets only files in that task’s capture index, leaving unrelated attachments and its website profile untouched. Download copies that must outlive these limits; no background cleanup scheduler is needed.

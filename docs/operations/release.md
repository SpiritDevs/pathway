...
- Reads the shared production Pathway Connect relay URL and Clerk client configuration before packaging clients.
...
## Pathway Connect relay deployment
...
through `/__pathway/channel` on the router domain so the user's channel cookie is
...
- Repository slug source:
  - `PATHWAY_DESKTOP_UPDATE_REPOSITORY` (format `owner/repo`), if set.
...
10. Complete the Clerk Native API and AASA setup in [Pathway Connect Clerk Setup](../internals/pathway-connect.md#desktop-passkeys).

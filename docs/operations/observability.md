...
Pathway has one server-side observability model:
...
stdout/stderr at `~/.pathway/ssh-launch/<state>/server.log`.
...
- SSH-managed launch persistence: `~/.pathway/ssh-launch/<state>/server.log`
...
`<home>/userdata/logs/server.trace.ndjson` (so `~/.pathway/userdata/...` by default, or
`/custom/path/userdata/...` with `--home-dir /custom/path`), a linked worktree dev run uses
`<worktree>/.pathway/userdata/logs/server.trace.ndjson`, and an implicit dev run outside a linked
worktree uses `~/.pathway/dev/logs/server.trace.ndjson`.
...
```bash
export PATHWAY_TRACE_MIN_LEVEL=Info
export PATHWAY_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `PATHWAY_OTLP_*`.
...
```bash
TRACE_FILE="${PATHWAY_HOME:-$HOME/.pathway}/userdata/logs/server.trace.ndjson"
```

A dev server started from a linked worktree defaults to that worktree's local home:

```bash
TRACE_FILE="$WORKTREE/.pathway/userdata/logs/server.trace.ndjson"
```

Only an implicit dev run outside a linked worktree uses the shared dev directory:

```bash
TRACE_FILE="$HOME/.pathway/dev/logs/server.trace.ndjson"
```
...
- `PATHWAY_OTLP_TRACES_URL` was not set
...
- `PATHWAY_TRACE_FILE`: override trace file path
- `PATHWAY_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `PATHWAY_TRACE_MAX_FILES`: rotated file count, default `10`
- `PATHWAY_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `PATHWAY_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `PATHWAY_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

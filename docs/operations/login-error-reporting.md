...
The server commits a report and scheduled delivery together, then acknowledges receipt. The screen confirms Cloud receipt, not inbox delivery. If `RESEND_API_KEY` is missing or blank, delivery logs a warning with the report's diagnostic fields in Convex and returns successfully without sending email or scheduling retries. The report remains stored with `sentAt: null` and no delivery attempt recorded. Setting a key later enables delivery for new reports; existing skipped reports can be sent by invoking the internal `loginErrorReports:deliver` action with their row ID.
...
The server accepts at most five reports per installation and 100 reports total per rolling hour. Report IDs deduplicate client retries, and the Resend idempotency key deduplicates email retries. Delivery retries six times over about half an hour. Inspect `loginErrorReports` rows with `sentAt: null` and `attempts: 6` for exhausted deliveries; correct the email configuration/provider problem, reset `attempts` to zero, and invoke the internal `loginErrorReports:deliver` action with the row ID. Delivered reports expire after seven days. Failed reports remain available for investigation.

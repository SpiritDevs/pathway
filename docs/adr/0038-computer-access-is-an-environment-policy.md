...
Under every policy, any client that can operate the thread can watch the preview, answer approvals and press Stop. Only an admin client (`access:write`) can change the policy, because the policy grants access to the host's desktop.

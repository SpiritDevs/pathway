...
This plan ports Synara's Computer Use feature into Pathway in full. The decisions behind it are in ADRs [0036](../adr/0036-computer-use-is-a-literal-mirror-of-synara.md)–[0045](../adr/0045-pathway-owns-computer-approvals.md), and the vocabulary is in the "Computer Use" section of [the glossary](../internals/glossary.md). Read both before building anything. This file describes how the work is divided and delivered. It is not a spec: the spec is Synara's code and tests.
...
- #1010 is excluded. From #1291, only the Cua build cache is taken ([0043](../adr/0043-computer-use-native-build-scope.md)).
- Licences:
...
1. **Effect everywhere in TypeScript** ([0042](../adr/0042-computer-use-is-rewritten-in-effect.md)). Read `.repos/effect-smol/LLMS.md` first. Model services with `Context.Service`/`Layer`, errors with `Schema.TaggedErrorClass`, processes with `effect/unstable/process`, and tools with `effect/unstable/ai` `Tool`. Copy the patterns of neighbouring Pathway code, such as `apps/server/src/preview`, `apps/server/src/mcp/toolkits/*` and `apps/desktop/src/snapShot`. Swift and Rust stay native.
...
3. **Renames.**
   ...
   - The "AppSnap" helper → `pathway-helper` ([0044](../adr/0044-one-general-native-helper-for-macos.md)).
...
| P4   | `feat/computer-use-p4-tools-policy` | The MCP `computer` toolkit (`computer_*`, `computer_browser_*`, space tools, guidance, progress guard, turn presence), Pathway-owned approvals ([0045](../adr/0045-pathway-owns-computer-approvals.md)), the access policy ([0038](../adr/0038-computer-access-is-an-environment-policy.md)), the autonomy ceiling ([0039](../adr/0039-computer-autonomy-is-an-environment-dropdown.md)/[0040](../adr/0040-computer-autonomy-is-an-environment-ceiling-over-thread-mode.md)), WS handlers and RPC, and the frame route. |

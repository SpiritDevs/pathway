...
- Shake detection starts enabled on iPhone and can be disabled in the confirmation card, report sheet, or Settings. It first opens a compact confirmation card; only tapping Report bug opens the full form and begins or resumes a draft. Dismissing the card leaves drafts unchanged. Keep the pre-card screenshot in memory for the optional attachment, and release it when the sheet closes. Ignore repeated shakes while either step is open. The preference belongs to the device. Shaking never submits a report.
...
| Integrated UI verification | Passed on iPhone 17 Pro with offline fixture data: Settings, report form, investigation off/on, model selection, and Simulator Shake opening a compact confirmation card. A follow-up UI test verified card dismissal without a draft, the shake toggle, and the full form opening only after confirmation. Screenshots and recordings are in `.github/pr-assets/apple-bug-reporting`. No live Cloud submission or provider run was performed. |

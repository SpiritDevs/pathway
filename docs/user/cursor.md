...
The adapter currently uses the SDK's local-agent runtime so runs operate in the selected Pathway
workspace. Cursor cloud agents need repository and cloud-environment configuration that Pathway does
not expose yet.
...
- thread-scoped Pathway MCP tools;
- image attachments;
...
overrides also control Cursor Auto-review. Auto-review is not represented as an interactive Pathway
approval flow.
...
```bash
pnpm --filter @spiritdevs/pathway record:cursor-replay -- --scenario simple
```

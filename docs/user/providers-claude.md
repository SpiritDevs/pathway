...
This guide is for people who want to use more than one Claude setup in Pathway. For Codex, see
[Codex](./providers-codex.md). For first-time setup, see [Install Pathway](./install.md).
...
In Pathway Settings, your Claude provider can stay like this:
...
An empty `CLAUDE_CONFIG_DIR path` means Pathway uses Claude Code's normal config directory.

When you set this field, Pathway points Claude Code at that directory with the
...
`~/.claude_personal_home/.claude`, which is not where Pathway looks.

Then add another Claude provider in Pathway:
...
Pathway only offers Claude providers that use the same config directory for an existing thread. A
...
multiple files under its config directory, so Pathway keeps separate config directories isolated
...
Add or edit a Claude provider in Pathway Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. Pathway stores the value as a server secret and does not
...
Pathway does not need a special Claude Code Router provider. Treat the router as a Claude

...
This guide is for people who want to use more than one Codex account in Pathway. For Claude, see
[Claude](./providers-claude.md). For first-time setup, see [Install Pathway](./install.md).
...
- both accounts can see the same Pathway/Codex sessions
...
In Pathway Settings, name it something obvious:
...
In Pathway Settings, add another Codex provider:
...
Pathway shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Pathway treats it

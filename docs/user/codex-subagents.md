...
When Codex reports a subagent's model and reasoning effort, Pathway updates its configuration without changing the conversation title or dropping inherited session options such as service tier. Nested subagents use their immediate parent's configuration as the initial default. Unreported configuration is not treated as a confirmed model or effort in the subagent roster.

The Agents panel uses the subagent's reported nickname when available, with its task title or prompt as a fallback. Opening a child conversation directly does not prevent later subagent activations from reporting their configuration and usage.

...
On web and desktop, Auto checks permission to start threads through the active machine connection. An expired connection token does not exclude a machine whose active connection can still start work. This requires an updated server on that machine; older servers keep their existing permission check. If Auto cannot check access or resources, the location dropdown explains which check failed. Select Auto again to retry both checks. If it asks you to reconnect, reconnect that machine before retrying. Read-only connections remain excluded.

...
Pathway connects to your Git hosting provider so you can create pull requests, review code, and manage repositories without leaving the app.

## Supported Providers

Pathway works with the platforms your team already uses:
...
- Choose **Review with an agent** from a pull request's menu to select the agent, model, reasoning,
  ...
  thread list. Each review runs in its own worktree at the pull request head, so it does not move or
  reuse a checkout where other work is in progress. Findings can be published as inline host
  comments or kept as draft markers for you to inspect and submit from **Code**.
...
1. Install the GitHub CLI on the machine running Pathway:
   ```bash
   brew install gh
   ```
2. Sign in:
   ...
   gh auth login
   ```
3. Open **Settings → Source Control** in Pathway and verify GitHub shows as authenticated
...
machine running Pathway.

Recommended, a Bitbucket access token:

```bash
export PATHWAY_BITBUCKET_ACCESS_TOKEN="your-access-token"
```
...
```bash
export PATHWAY_BITBUCKET_EMAIL="you@example.com"
export PATHWAY_BITBUCKET_API_TOKEN="your-token"
```

If both are set, the access token wins. Restart Pathway and verify the connection in **Source
Control settings**.
...
**Git is required** – Pathway uses Git for all local operations. Ensure `git` is installed on your server.

**Server-side setup** – Authentication happens on the machine running Pathway (the server), not your local browser. If you're using a hosted or team instance, your administrator may have already configured providers.

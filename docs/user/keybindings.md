...
The same configuration lives in `~/.pathway/userdata/keybindings.json` on the machine running the
server, if you prefer editing it directly. Pathway writes the built-in defaults into that file on

...
import {
  ...
  resolveRemotePathwayCliPackageSpec,
  runSshCommand,
} from "./command.ts";
...
describe("ssh command", () => {
  ...
  it.effect("resolves the remote Pathway package spec from the desktop release channel", () =>
    Effect.sync(() => {
      assert.equal(
        resolveRemotePathwayCliPackageSpec({
          appVersion: "0.0.17",
      ...
      assert.equal(
        ...
        "@spiritdevs/pathway@0.0.17-nightly.20260415.44",
      );
      assert.equal(
        resolveRemotePathwayCliPackageSpec({

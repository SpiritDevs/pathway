...
import { extractJsonObject, fromLenientJson } from "@spiritdevs/shared/schemaJson";
...
export interface RemotePathwayRunnerOptions {
...
export interface SshEnvironmentManagerOptions {
  ...
  readonly resolveCliRunner?: Effect.Effect<RemotePathwayRunnerOptions>;
}
...
function sshRunnerLogFields(runner: RemotePathwayRunnerOptions | undefined) {
...
remote_node_satisfies_engine() {
  Pathway_NODE_ENGINE_RANGE=@@Pathway_NODE_ENGINE_RANGE@@
  if [ -z "$Pathway_NODE_ENGINE_RANGE" ]; then
    return 0
  fi
  node - "$Pathway_NODE_ENGINE_RANGE" <<'NODE'
@@Pathway_NODE_ENGINE_CHECK_SCRIPT@@
NODE
}
...
ensure_remote_node_path() {
  ...
  if ! command -v node >/dev/null 2>&1 && [ -d "$NVM_DIR/versions/node" ]; then
    for Pathway_NODE_BIN in "$NVM_DIR"/versions/node/*/bin; do
      if [ -x "$Pathway_NODE_BIN/node" ]; then
        PATH="$Pathway_NODE_BIN:$PATH"
        export PATH
      fi
    done
  fi
...
set -eu
@@Pathway_NODE_ENV_SCRIPT@@
ensure_remote_node_path || true
Pathway_NODE_SCRIPT_PATH=@@Pathway_NODE_SCRIPT_PATH@@
if [ -n "$Pathway_NODE_SCRIPT_PATH" ]; then
  ...
  exec node "$Pathway_NODE_SCRIPT_PATH" "$@"
fi
if command -v pathway >/dev/null 2>&1; then
  exec pathway "$@"
fi
if command -v npx >/dev/null 2>&1; then
  exec npx --yes @@PATHWAY_PACKAGE_SPEC@@ "$@"
fi
if command -v npm >/dev/null 2>&1; then
  exec npm exec --yes @@PATHWAY_PACKAGE_SPEC@@ -- "$@"
fi
printf 'Remote host is missing the Pathway CLI and could not install @@PATHWAY_PACKAGE_SPEC@@ because node/npm/npx are unavailable on PATH. Install Node or configure a supported version manager for non-interactive shells.\\n' >&2
exit 1
`;
...
@@Pathway_NODE_ENV_SCRIPT@@
STATE_KEY="$1"
STATE_DIR="$HOME/.pathway/ssh-launch/$STATE_KEY"
DEFAULT_SERVER_HOME="$HOME/.pathway"
DEFAULT_RUNTIME_FILE="$DEFAULT_SERVER_HOME/userdata/server-runtime.json"
PORT_FILE="$STATE_DIR/port"
PID_FILE="$STATE_DIR/pid"
MANAGED_FILE="$STATE_DIR/managed"
LOG_FILE="$STATE_DIR/server.log"
RUNNER_FILE="$STATE_DIR/run-pathway.sh"
RUNNER_NEXT="$STATE_DIR/run-pathway.next.$$"
mkdir -p "$STATE_DIR"
cleanup_runner_next() {
  rm -f "$RUNNER_NEXT"
}
...
pick_port() {
  node - "$PORT_FILE" "@@Pathway_DEFAULT_REMOTE_PORT@@" "@@Pathway_REMOTE_PORT_SCAN_WINDOW@@" <<'NODE'
@@Pathway_PICK_PORT_SCRIPT@@
NODE
}
wait_ready() {
  node - "$REMOTE_PORT" "$1" "@@Pathway_READY_PROBE_TIMEOUT_MS@@" <<'NODE'
@@Pathway_WAIT_READY_SCRIPT@@
NODE
}
wait_for_pid_exit() {
  PID_TO_WAIT="$1"
...
if [ -n "$DEFAULT_REMOTE_PORT" ]; then
  ...
  if wait_ready "@@Pathway_REUSE_READY_TIMEOUT_MS@@"; then
...
if [ "$REMOTE_MANAGED" = "external" ]; then
  if [ -z "$REMOTE_PORT" ] || ! wait_ready "@@Pathway_REUSE_READY_TIMEOUT_MS@@"; then
    REMOTE_PID=""
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  fi
  ...
  if [ "$RUNNER_CHANGED" -eq 1 ]; then
    ...
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  elif ! wait_ready "@@Pathway_REUSE_READY_TIMEOUT_MS@@"; then
    kill "$REMOTE_PID" 2>/dev/null || true
    wait_for_pid_exit "$REMOTE_PID"
    REMOTE_PID=""
    REMOTE_PORT=""
...
if [ -z "$REMOTE_PORT" ]; then
  ...
  nohup env PATHWAY_NO_BROWSER=1 "$RUNNER_FILE" serve --host 127.0.0.1 --port "$REMOTE_PORT" --base-dir "$DEFAULT_SERVER_HOME" >>"$LOG_FILE" 2>&1 < /dev/null &
  REMOTE_PID="$!"
  printf '%s\\n' "$REMOTE_PID" >"$PID_FILE"
  printf '%s\\n' "$REMOTE_PORT" >"$PORT_FILE"
  printf 'managed\\n' >"$MANAGED_FILE"
  if ! wait_ready "@@Pathway_READY_TIMEOUT_MS@@"; then
    printf 'Remote Pathway server did not become ready on 127.0.0.1:%s.\\n' "$REMOTE_PORT" >&2
...
STATE_DIR="$HOME/.pathway/ssh-launch/@@Pathway_STATE_KEY@@"
DEFAULT_SERVER_HOME="$HOME/.pathway"
RUNNER_FILE="$STATE_DIR/run-pathway.sh"
mkdir -p "$STATE_DIR"
cat >"$RUNNER_FILE" <<'SH'
...
export function buildRemotePathwayRunnerScript(input?: RemotePathwayRunnerOptions): string {
  const packageSpec = shellSingleQuote(input?.packageSpec?.trim() || "@spiritdevs/pathway@latest");
  const nodeScriptPath = input?.nodeScriptPath?.trim() || "";
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_RUNNER_SCRIPT, {
      PATHWAY_PACKAGE_SPEC: packageSpec,
      Pathway_NODE_SCRIPT_PATH: shellSingleQuote(nodeScriptPath),
      Pathway_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    }),
  );
}

export function buildRemoteNodeEnvScript(input?: RemotePathwayRunnerOptions): string {
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_NODE_ENV_SCRIPT, {
      Pathway_NODE_ENGINE_RANGE: shellSingleQuote(input?.nodeEngineRange?.trim() || ""),
      Pathway_NODE_ENGINE_CHECK_SCRIPT: stripTrailingNewlines(buildRemoteNodeEngineCheckScript()),
    }),
  );
}

export function buildRemoteLaunchScript(input?: RemotePathwayRunnerOptions): string {
  return applyScriptPlaceholders(REMOTE_LAUNCH_SCRIPT, {
    Pathway_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    Pathway_RUNNER_SCRIPT: stripTrailingNewlines(buildRemotePathwayRunnerScript(input)),
    Pathway_PICK_PORT_SCRIPT: stripTrailingNewlines(REMOTE_PICK_PORT_SCRIPT),
    Pathway_WAIT_READY_SCRIPT: stripTrailingNewlines(REMOTE_WAIT_READY_SCRIPT),
    Pathway_DEFAULT_REMOTE_PORT: String(DEFAULT_REMOTE_PORT),
    Pathway_REMOTE_PORT_SCAN_WINDOW: String(REMOTE_PORT_SCAN_WINDOW),
    Pathway_READY_TIMEOUT_MS: String(REMOTE_READY_TIMEOUT_MS),
    Pathway_REUSE_READY_TIMEOUT_MS: String(REMOTE_REUSE_READY_TIMEOUT_MS),
    Pathway_READY_PROBE_TIMEOUT_MS: String(SSH_READY_PROBE_TIMEOUT_MS),
  });
}

export function buildRemotePairingScript(
  target: DesktopSshEnvironmentTarget,
  input?: RemotePathwayRunnerOptions,
): string {
  return applyScriptPlaceholders(REMOTE_PAIRING_SCRIPT, {
    Pathway_STATE_KEY: remoteStateKey(target),
    Pathway_RUNNER_SCRIPT: stripTrailingNewlines(buildRemotePathwayRunnerScript(input)),
  });
}
...
export const launchOrReuseRemoteServer = Effect.fn("ssh/tunnel.launchOrReuseRemoteServer")(
  function* (
    ...
    runner?: RemotePathwayRunnerOptions,
  ): Effect.fn.Return<
...
const makeSshEnvironmentManager = Effect.fn("ssh/tunnel.SshEnvironmentManager.make")(function* (
  ...
  const createTunnelEntry = Effect.fn("ssh/tunnel.ensureTunnelEntry.create")(function* (input: {
    readonly key: string;
    readonly resolvedTarget: DesktopSshEnvironmentTarget;
    readonly runner?: RemotePathwayRunnerOptions;
...
>()("@spiritdevs/ssh/tunnel/SshEnvironmentManager") {

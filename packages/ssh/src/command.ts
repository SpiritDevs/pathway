...
import type { DesktopSshEnvironmentTarget, DesktopUpdateChannel } from "@spiritdevs/contracts";
...
const PUBLISHABLE_PATHWAY_VERSION_PATTERN = /^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/u;
...
export function resolveRemotePathwayCliPackageSpec(input: {
  readonly appVersion: string;
  readonly updateChannel: DesktopUpdateChannel;
  readonly isDevelopment?: boolean;
}): string {
  const appVersion = input.appVersion.trim();
  if (!input.isDevelopment && PUBLISHABLE_PATHWAY_VERSION_PATTERN.test(appVersion)) {
    return `@spiritdevs/pathway@${appVersion}`;
  }

  if (input.isDevelopment) {
    return "@spiritdevs/pathway@nightly";
  }

  return input.updateChannel === "nightly"
    ? "@spiritdevs/pathway@nightly"
    : "@spiritdevs/pathway@latest";
}

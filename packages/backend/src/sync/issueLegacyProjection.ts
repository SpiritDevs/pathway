/**
 * Compatibility projection from company-replica issue entities into the environment-era issue
 * contracts. Web and server both consume these pure mappings so the two read boundaries cannot
 * drift while the legacy contracts remain in service.
 *
 * @module sync/issueLegacyProjection
 */
import { mergeEffectiveWorkflow, type EffectiveStatus } from "./workflow.ts";
import type {
...
export function issueThreadLinksFromReplica(
...
/** The replica-owned fields of the legacy list snapshot. */
export interface IssueCollectionProjection {

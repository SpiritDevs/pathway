 ...
 * through the Pathway proxy. One class handles both platforms because the hub
...
export function createDeviceStreamClient(
  ...
  const requestKeyframe = () => {
    if (platform === "ios") {
      // AVCC has no reset-video command. Reopen only video, keeping HID connected.
      videoGeneration++;
      controller?.abort();
  ...
  const readIosVideo = async () => {
    const generation = ++videoGeneration;
    // A decoder callback belongs to one AVCC response generation.
    closeDecoder();

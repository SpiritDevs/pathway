...
describe("buildProjectGroups", () => {
  it("preserves every physical clone as a selectable member in repository modes", () => {
    const projects = [
      ...
      makeProject("pathway-2", "/work/pathway-2"),
      makeProject("pathway-3", "/work/pathway-3"),
    ];
  ...
  it("uses a shared custom title as the repository group's label", () => {
    const projects = [
      makeProject("first", "/work/pathway", { title: "Custom project" }),
      makeProject("second", "/work/pathway-2", { title: "Custom project" }),
    ];
  ...
  it("keeps the repository label when shared titles match its repository name", () => {
    const projects = [
      makeProject("first", "/work/pathway", { title: "pathway" }),
      makeProject("second", "/work/pathway-2", { title: "pathway" }),
    ];
  ...
  it("keeps physical clones in separate groups when requested", () => {
    ...
    expect(groups.map((group) => group.label)).toEqual(["pathway", "pathway-2", "pathway-3"]);
  });
  ...
  it("applies a physical-project override without dropping its siblings", () => {
    ...
    const third = makeProject("pathway-3", "/work/pathway-3");
  ...
  it("dedupes stale registrations at one physical path using the freshest project", () => {
    const stale = makeProject("stale", "/work/pathway", {
      repositoryIdentity: null,
      updatedAt: "2026-07-01T00:00:00.000Z",
    });
    const fresh = makeProject("fresh", "/work/pathway/", {
  ...
  it("uses repository identity from a duplicate registration when the winner lacks it", () => {
    const identified = makeProject("identified", "/work/pathway", {
      updatedAt: "2026-07-01T00:00:00.000Z",
    });
    const freshUnidentified = makeProject("fresh", "/work/pathway/", {
      repositoryIdentity: null,
      updatedAt: "2026-07-02T00:00:00.000Z",
    });
    const sibling = makeProject("sibling", "/work/pathway-2");
  ...
  it("uses the freshest identity-bearing duplicate when the winner lacks identity", () => {
    ...
    const staleIdentified = makeProject("stale-identified", "/work/pathway", {
      repositoryIdentity: staleIdentity,
      updatedAt: "2026-07-01T00:00:00.000Z",
    });
    const freshIdentified = makeProject("fresh-identified", "/work/pathway/", {
      updatedAt: "2026-07-02T00:00:00.000Z",
    });
    const winner = makeProject("winner", "/work/pathway", {
      repositoryIdentity: null,

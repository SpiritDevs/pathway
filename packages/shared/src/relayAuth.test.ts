...
describe("Clerk relay auth", () => {
  it("derives a custom Frontend API hostname from a Clerk publishable key", () => {
    expect(
      clerkFrontendApiHostnameFromPublishableKey(clerkPublishableKey("clerk.spiritdevs.com")),
    ).toBe("clerk.spiritdevs.com");
    expect(clerkFrontendApiUrlFromPublishableKey(clerkPublishableKey("clerk.spiritdevs.com"))).toBe(
  ...
  it("reports semantic frontend API failures without inventing a cause", () => {
    ...
    const pathFrontendApi = "clerk.spiritdevs.com/path";
  ...
  it("allows standard Clerk hosts and an exact configured custom hostname", () => {
    ...
    expect(isAllowedClerkFrontendApiHostname("clerk.spiritdevs.com", "clerk.spiritdevs.com")).toBe(
      true,
    );
    expect(isAllowedClerkFrontendApiHostname("attacker.example", "clerk.spiritdevs.com")).toBe(
      false,
    );
    expect(
      isAllowedClerkFrontendApiHostname("nested.clerk.spiritdevs.com", "clerk.spiritdevs.com"),
    ).toBe(false);
  });
});
